...
3. Prefer `delivery.channel:"web"` unless the user explicitly needs the configured messaging channel/Discord/other output.
4. Create with:
...
Bad: "Send the user a morning brief."
...
Symptoms:
- Job complete, but the user does not see the result.

Fix:
- Verify whether job prompt actually sends/returns output.
- For the configured messaging channel, confirm a real `send_telegram` call or delivery tool log when required.
...
A completed run is not proof that the user received the right thing.

For the configured messaging channel/Discord/WhatsApp or other external delivery:
...
### the configured messaging channel delivery
- Job success is not proof of the configured messaging channel delivery.
- The prompt must explicitly call `send_telegram` or use a workflow that sends to the configured messaging channel.
