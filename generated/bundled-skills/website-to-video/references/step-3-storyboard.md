...
Pick from the inventory in [capabilities.md](capabilities.md) and implementation patterns in [techniques.md](../../hyperframes-animation/techniques.md). Examples of composable beats:
...
- **[techniques.md](../../hyperframes-animation/techniques.md)** — primitive animation techniques with code patterns. Pick for beats; these are starting points to adapt, not templates to copy.
- **[animate-text.md](../../hyperframes-animation/adapters/animate-text.md)** — named seek-safe text animation effects and their implementation contract. Assign a specific effect ID to every headline, label, and copy element in every beat — not generic "fades in" descriptions.
...
**Global guardrails** — read [video-composition.md](../../hyperframes-creative/references/video-composition.md) first. It defines the medium rules: density, color presence, scale, frame composition, and how design.md is brand truth not layout spec. Then apply these capture-specific additions:
...
When planning beats, decide which ones deserve an HTML-in-Canvas treatment vs. a standard GSAP animation. If you want it, name it in the storyboard — Step 5 will read [`html-in-canvas-patterns.md`](../../hyperframes-animation/adapters/html-in-canvas-patterns.md) for implementation. You don't need to specify the API details here.
...
Every text element in this beat must name a specific effect from the [animate-text adapter](../../hyperframes-animation/adapters/animate-text.md). Pick what fits the brand and this beat's mood—do not default to the same effect every beat.
