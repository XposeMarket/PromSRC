...
Read [references/windows-runtime.md](references/windows-runtime.md) when wrapper discovery, Node, Chrome, or encoder behavior differs from a standalone project.

- **FFmpeg not found:** First rerun with `<PROMETHEUS_ROOT>/scripts/run-hyperframes.js`. It exposes the encoders already bundled with Prometheus; no system-wide install is needed. Treat failure through that wrapper as the real blocker.
