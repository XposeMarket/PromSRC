...
function mediaRuntimeProbe(command, packageName) {
  const ambient = runCommand(command, ["-version"]);
  if (ambient.status === 0) return { result: ambient, source: "PATH" };
  const bundled = bundledBinary(packageName);
  if (!bundled) return { result: ambient, source: null };
  return { result: runCommand(bundled, ["-version"]), source: "Prometheus bundle" };
}
...
function runDoctor() {
  ...
  const ffmpegRuntime = mediaRuntimeProbe("ffmpeg", "@ffmpeg-installer/ffmpeg");
  const ffmpegProbe = ffmpegRuntime.result;
  checks.push({
    name: "ffmpeg on PATH",
    ok: ffmpegProbe.status === 0,
    detail:
      ffmpegProbe.status === 0
        ? `${firstLine(ffmpegProbe.stdout)} (${ffmpegRuntime.source})`
        : "ffmpeg not found on PATH or in the Prometheus bundle",
    fix:
      ffmpegProbe.status === 0
        ? ""
        : "run through <PROMETHEUS_ROOT>/scripts/run-hyperframes.js or install ffmpeg",
  });

  const ffprobeRuntime = mediaRuntimeProbe("ffprobe", "@ffprobe-installer/ffprobe");
  const ffprobeProbe = ffprobeRuntime.result;
  checks.push({
    name: "ffprobe on PATH",
    ok: ffprobeProbe.status === 0,
    detail:
      ffprobeProbe.status === 0
        ? `${firstLine(ffprobeProbe.stdout)} (${ffprobeRuntime.source})`
        : "ffprobe not found on PATH or in the Prometheus bundle",
    fix:
      ffprobeProbe.status === 0
        ? ""
        : "run through <PROMETHEUS_ROOT>/scripts/run-hyperframes.js or install ffprobe",
  });
