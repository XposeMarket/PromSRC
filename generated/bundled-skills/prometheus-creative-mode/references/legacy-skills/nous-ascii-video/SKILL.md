...
## Prometheus Creative Video Integration

When the user wants premium ASCII visuals inside Prometheus Creative Video, use this skill as the heavy render engine and then hand the result back to Prometheus:

1. Prefer `creative_render_ascii_asset` for the integrated Prometheus path. It runs the bundled Python/ffmpeg ASCII renderer, writes job metadata/logs, imports the MP4 into the Creative asset library, and can place it into the active canvas as a selectable video layer.
2. Render dense source-backed ASCII, audio-reactive glyph fields, shader chains, and tonemapped output with this Python/ffmpeg pipeline when a bespoke one-off script is needed.
3. Let HTML Motion / HyperFrames handle the finishing pass: crisp text, captions, CTA, layout, overlays, transitions, safe-area framing, and final Prometheus export.
4. Use browser-native HTML ASCII only for lightweight previews, editable smart-layer effects, quick logo treatments, or when the user explicitly wants the whole effect authored in HTML.

Default judgment: Python owns the cinematic ASCII raster quality; Prometheus HTML Motion owns the editable composition and export workflow around it.
