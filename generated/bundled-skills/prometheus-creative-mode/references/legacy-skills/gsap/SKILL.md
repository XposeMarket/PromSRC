...
Inside Prometheus Creative HTML Motion clips, GSAP guidance is allowed only when the HTML remains self-contained and deterministic. Prefer timeline-driven, seek-safe animation. Avoid wall-clock animations, external plugins, remote scripts, and `autoAlpha` when exact snapshot/export visibility matters; use `opacity` plus explicit timeline state instead.
