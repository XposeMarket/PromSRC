...
Inside Prometheus, use this skill as a website-to-video planning and design reference. The default build target is Prometheus Creative Video mode, not a standalone HyperFrames Studio project.

- Use Prometheus browser/asset tools for capture and inspection when available.
- Build the video with `prometheus-hyperframes-bridge` and Prometheus Creative HTML Motion tools.
- Replace standalone gates like `npx hyperframes capture`, `lint`, `validate`, `snapshot`, `preview`, and `render` with the nearest Prometheus tools: `creative_create_html_motion_clip`, `creative_lint_html_motion_clip`, `creative_render_html_motion_snapshot`, `creative_validate_layout`, `creative_quality_report`, and `creative_export_html_motion_clip`.
- Deliver a Prometheus canvas/creative export path for Creative Mode work. Only deliver a HyperFrames Studio URL when the user explicitly asks for a standalone HyperFrames project.
