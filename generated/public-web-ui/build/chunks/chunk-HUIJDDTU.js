import{b as q,c as j}from"./chunk-FD3LBQ2D.js";import{L,j as H}from"./chunk-ZQ77UU3S.js";import{a as C}from"./chunk-35CAQ6TV.js";import{H as h,I as o,O as k,P as S}from"./chunk-U7YV2ZI6.js";import{qa as I,ra as P,sa as E,va as D}from"./chunk-NJ4SR5XR.js";import{a as _}from"./chunk-GRAK6S3F.js";import{a as N}from"./chunk-36KIJFV6.js";function g(e,t=""){let n=Number(e||0);return Number.isFinite(n)?Math.abs(n)>=1e9?`${Math.round(n/1e8)/10}B${t}`:Math.abs(n)>=1e6?`${Math.round(n/1e5)/10}M${t}`:Math.abs(n)>=1e3?`${Math.round(n/100)/10}K${t}`:`${Math.round(n).toLocaleString()}${t}`:t?`0${t}`:"0"}function A(e){if(!e)return"";let t=new Date(e);return Number.isNaN(t.getTime())?"":t.toLocaleString([],{month:"short",day:"numeric",hour:"numeric",minute:"2-digit"})}function Q(e){return String(e?.title||e?.goal||e?.userRequest||e?.summary||e?.id||"Latest goal").trim()}function Y(e){return String(e?.summary||e?.result||e?.assistantSummary||e?.description||e?.lastAssistantMessage||"").trim()}function F(e){let t=String(e||"").toLowerCase();return["running","pending","executing"].includes(t)?'<span class="pm-pill running">running</span>':["failed","rejected","denied"].includes(t)?'<span class="pm-pill orange">failed</span>':["complete","completed","done","approved","auto"].includes(t)?'<span class="pm-pill active">complete</span>':`<span class="pm-pill gray">${o(t||"unknown")}</span>`}function R(e,t){let n=String(e||"").toLowerCase();return String(t||"").toLowerCase().includes("proposal")||n.includes("proposal")?"proposal":n.includes("delete")||n.includes("remove")?"delete":n.includes("type")||n.includes("fill")?"type":n.includes("click")||n.includes("press")?"click":n.includes("command")||n==="shell"?"cmd":n.includes("write")||n.includes("edit")||n.includes("create")||n.includes("append")?"edit":n.includes("read")||n.includes("list")||n.includes("search")||n.includes("stat")||n.includes("grep")?"read":"other"}function V(e){let t={total:0,read:0,edit:0,delete:0,type:0,click:0,cmd:0,proposal:0,approved:0,rejected:0,pending:0};for(let n of e||[])for(let r of n.tools||[]){t.total++;let s=R(r.toolName,r.actionType);t[s]!==void 0&&t[s]++;let l=String(r.approvalStatus||"").toLowerCase();l==="approved"?t.approved++:l==="rejected"?t.rejected++:l==="pending"&&t.pending++}return t}function X(e,t=3){let n=new Map;for(let r of e||[]){let s=String(r.toolName||"tool");n.set(s,(n.get(s)||0)+1)}return[...n.entries()].sort((r,s)=>s[1]-r[1]).slice(0,t)}function be(e){let t=String(e?.priority||"medium").toLowerCase();return`<span class="pm-proposal-badge ${t==="critical"||t==="high"?"orange":t==="low"?"gray":"blue"}">${o(t.toUpperCase())}</span>`}function fe(e){let t=String(e?.status||"pending").toLowerCase();return t==="pending"?'<span class="pm-proposal-status pending">PENDING</span>':t==="executing"||t==="repairing"?'<span class="pm-proposal-status running">RUNNING</span>':t==="executed"||t==="approved"?'<span class="pm-proposal-status complete">APPROVED</span>':t==="denied"||t==="failed"?'<span class="pm-proposal-status denied">DENIED</span>':`<span class="pm-proposal-status">${o(t.toUpperCase())}</span>`}function $e(e,t=2){let n=Array.isArray(e?.affectedFiles)?e.affectedFiles.slice(0,t):[],r=Array.isArray(e?.affectedFiles)?e.affectedFiles.length-n.length:0,s=n.map(l=>`<span>${o(l?.action||"touch")}: ${o(l?.path||"")}</span>`);return r>0&&s.push(`<span>+${r} more</span>`),s.length?`<div class="pm-proposal-files">${s.join("")}</div>`:""}function U(e,t){let n=String(t?.status||t?.state||"").trim().toLowerCase();if(n)return n.replace(/\s+/g,"_");let r=String(e?.status||"").trim().toLowerCase();return["approved","executing","repairing","executed"].includes(r)?"approved":"pending"}function J(e,t){return t?.approved===!0||t?.isApproved===!0||U(e,t)==="approved"}function ke(e){let t=Array.isArray(e?.executionSteps)?e.executionSteps:[];return t.length?`<section class="pm-card pm-more-section"><div class="pm-card-head">Approved Execution Steps</div><div class="pm-proposal-steps">${t.map((n,r)=>{let s=String(n?.title||n?.description||`Step ${r+1}`),l=String(n?.kind||"").trim(),d=String(n?.successCriteria||n?.success_criteria||"").trim(),c=U(e,n),p=J(e,n),a=c.replace(/[^a-z0-9_-]/g,"_");return`<div class="pm-proposal-step ${p?"is-approved":""} is-${a}" data-step-status="${o(c)}">
  ...
  </section>`:""}function Z(e,t,n="stdout"){let r=String(e||"").trim(),s=r?document.querySelector(`[data-pm-process-output="${L(r)}"]`):null,l=r?document.querySelector(`[data-pm-process-run="${L(r)}"]`):null;if(!s||!l||!t)return;let d=l.getAttribute("data-pm-process-tab")||"combined";if(d!=="combined"&&d!==n)return;let c=s.scrollHeight-s.scrollTop-s.clientHeight<40,a=`${s.textContent==="No output yet."?"":s.textContent}${t}`;s.textContent=a.length>128*1024?`[Older live output hidden; open the saved log for more.]
${a.slice(-128*1024)}`:a,c&&(s.scrollTop=s.scrollHeight)}function ee(){if(window.__pmMobileProcessRunLiveInstalled)return;window.__pmMobileProcessRunLiveInstalled=!0;let e=window.wsEventBus||_;e?.on?.("process_run_output",(t={})=>{let n=String(t.run?.runId||t.runId||"").trim();Z(n,String(t.chunk||""),String(t.stream||"stdout"))}),["process_run_started","process_run_update","process_run_exited"].forEach(t=>{e?.on?.(t,(n={})=>{let r=n.run||{},s=String(r.runId||n.runId||"").trim(),l=s?document.querySelector(`[data-pm-process-run="${L(s)}"]`):null;if(!l)return;let d=String(r.state||r.status||"").toLowerCase(),c=l.querySelector(".pm-process-pill"),p=l.querySelector(".pm-process-live-state");c&&d&&(c.textContent=d,c.className=`pm-process-pill ${d}`),p&&(p.textContent=d==="exited"?"completed":"streaming")})})}ee();function te(e,t=120){return`<div class="pm-memory-orbit" aria-hidden="true"><div class="pm-memory-core"></div>${(Array.isArray(e)?e:[]).slice(0,t).map((s,l)=>{let d=Number(s?.degree||0),c=l*137.5*Math.PI/180,p=8+Math.sqrt(l+1)*7.2,a=50+Math.cos(c)*Math.min(p,43),m=50+Math.sin(c)*Math.min(p,43),i=d>6?"#ff8a2a":d>3?"#a78bfa":"#55c4ff";return`<i style="left:${a.toFixed(2)}%;top:${m.toFixed(2)}%;background:${i};"></i>`}).join("")}</div>`}function ne(){let e=document.getElementById("memory-view");if(!e||e.dataset.pmIdsParked==="1")return()=>{};let t=[];return e.querySelectorAll("[id]").forEach(n=>{let r=n.getAttribute("id");!r||!r.startsWith("memory-")||(n.setAttribute("data-pm-original-id",r),n.setAttribute("id",`desktop-${r}`),t.push(n))}),e.dataset.pmIdsParked="1",()=>{t.forEach(n=>{let r=n.getAttribute("data-pm-original-id");r&&(n.setAttribute("id",r),n.removeAttribute("data-pm-original-id"))}),delete e.dataset.pmIdsParked}}function M(){return'<div class="pm-more-skeleton"><span></span><span></span><span></span></div>'}function re(e={}){return Number(e.totalTokens??e.total??0)||0}function se(e={}){return Number(e.modelCalls??e.messages??0)||0}function T(e,t,{retryOnWsOpen:n=!0}={}){if(!e||typeof t!="function")return;let r=!1,s=!1,l=!1,d=typeof e._pmCleanup=="function"?e._pmCleanup:null,c=async()=>{if(!(l||s||r)){s=!0;try{r=await t()!==!1}finally{s=!1}}},p=setTimeout(()=>requestAnimationFrame(c),0),a=()=>{r||setTimeout(c,120)};n&&_?.on?.("ws:open",a),e._pmCleanup=()=>{l=!0,clearTimeout(p),n&&_?.off?.("ws:open",a),d?.()}}function oe(e,{navigate:t}){let n=`<span class="pm-spacer"></span><button class="pm-icon-btn" id="pm-more-refresh" aria-label="Refresh" style="background:var(--pm-surface);border:1px solid var(--pm-border);">${h.refresh}</button>`;e.innerHTML=`
    ${k({title:"More",online:!0,extras:n,hideTitle:!0,hideBrand:!0})}
    <div class="pm-body pm-more-page" id="pm-more-body">
      ${M()}
    </div>
  `,S(e,{});let r=e.querySelector("#pm-more-body"),s=d=>{let c=Array.isArray(d?.audit?.runs)?d.audit.runs:[],p=Array.isArray(d?.memory?.recent)?d.memory.recent:[];r.innerHTML=`
  ...
  `}function le(e,t){let n=Math.max(0,Number(e)||0);if(n<=0)return 0;let r=n/Math.max(1,Number(t)||1);return Math.min(1,.12+.88*Math.sqrt(r))}function ce(e){let t=String(e?.dataset?.date||""),n=Math.max(0,Number(e?.dataset?.tokens||0));if(!t)return;document.getElementById("pm-token-activity-popover")?.remove();let r=document.createElement("div");r.id="pm-token-activity-popover",r.className="pm-token-activity-popover";let s=new Date(`${t}T00:00:00`).toLocaleDateString(void 0,{weekday:"short",month:"short",day:"numeric",year:"numeric"});r.innerHTML=`<strong>${o(s)}</strong><span>${o(g(n))} tokens</span>`,document.body.appendChild(r);let l=e.getBoundingClientRect(),d=8,c=r.offsetWidth||164,p=r.offsetHeight||52,a=l.left+l.width/2-c/2,m=l.top-p-d;m<d&&(m=l.bottom+d),a=Math.max(d,Math.min(a,window.innerWidth-c-d)),m=Math.max(d,Math.min(m,window.innerHeight-p-d)),r.style.left=`${a}px`,r.style.top=`${m}px`}function pe(e){let t=Array.from(e?.querySelectorAll?.(".pm-hub-token-cell[data-date]")||[]);if(!t.length)return;let n=null,r=null,s=()=>{r&&clearTimeout(r),r=null,document.getElementById("pm-token-activity-popover")?.remove()},l=a=>{a&&(r&&clearTimeout(r),ce(a))},d=(a,m)=>document.elementFromPoint(a,m)?.closest?.(".pm-hub-token-cell[data-date]"),c=()=>{n=null,window.removeEventListener("pointermove",p,!0),window.removeEventListener("pointerup",c,!0),window.removeEventListener("pointercancel",c,!0),r=setTimeout(s,850)},p=a=>{if(n!==a.pointerId)return;let m=d(a.clientX,a.clientY);m&&l(m)};t.forEach(a=>{a.addEventListener("pointerdown",m=>{m.pointerType==="mouse"&&m.button!==0||(n=m.pointerId,l(a),window.addEventListener("pointermove",p,!0),window.addEventListener("pointerup",c,!0),window.addEventListener("pointercancel",c,!0))}),a.addEventListener("pointerenter",m=>{m.pointerType==="mouse"&&l(a)}),a.addEventListener("pointerleave",m=>{m.pointerType==="mouse"&&n===null&&s()})})}function de(e={}){let t=Array.isArray(e?.daily)?e.daily:[];if(!t.length)return'<div class="pm-hub-token-empty">No token activity recorded yet.</div>';let n=new Date(`${t[0].date}T00:00:00`),r=Number.isFinite(n.getTime())?n.getDay():0,s=t.map(i=>Math.max(0,Number(i.tokens||i.count||0))),l=Math.max(1,...s),d=Math.ceil((r+t.length)/7),c=["S","M","T","W","T","F","S"].map(i=>`<span>${i}</span>`).join(""),p="";for(let i=0;i<r;i++)p+='<i class="empty"></i>';t.forEach(i=>{let u=Math.max(0,Number(i.tokens||i.count||0)),v=`${i.date}: ${g(u)} tokens`,b=u>0?"true":"false",y=le(u,l).toFixed(3);p+=`<i class="pm-hub-token-cell" data-date="${o(i.date)}" data-tokens="${u}" data-active="${b}" style="--pm-token-alpha:${y}" aria-label="${o(v)}"></i>`});let a=[],m=new Set;return t.forEach((i,u)=>{let[v,b]=String(i.date||"").split("-"),y=`${v}-${b}`;if(!v||!b||m.has(y))return;m.add(y);let f=new Date(`${i.date}T00:00:00`);a.push(`<span style="grid-column:${Math.floor((r+u)/7)+1}">${o(f.toLocaleDateString(void 0,{month:"short"}))}</span>`)}),`
  ...
  `,S(e,{});let r=e.querySelector("#pm-hub-body"),s=0,l=async()=>{try{let d=++s;r.innerHTML=M();let c=await P(),p=ae(),a=c.goals[0],m=c.models||{},i=c.tools||{},u=c.tokenActivity||{daily:[],stats:{}},v=u.stats||{},b=Number(v.totalTokens||re(m))||0,y=Number(v.peakTokens||0)||0,f=Number(v.activeDays||i.activeDays||m.activeDays||0)||0,O=Number(v.current??v.currentStreak??i.currentStreak??m.currentStreak??0)||0,G=Number(v.longest??v.longestStreak??i.longestStreak??m.longestStreak??0)||0,W=Number(i.toolCalls||i.total||0)||0,z=Array.isArray(c.topModels)?c.topModels:[],K=Array.isArray(c.topTools)?c.topTools:[];return r.innerHTML=`
      ...
      `,pe(r),c.goalsLoaded===!1&&E().then($=>{if(d!==s||!r.isConnected)return;let x=r.querySelector("#pm-hub-latest-goal");x&&(x.outerHTML=B($[0]||null))}).catch(()=>{}),!0}catch(d){return r.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${h.target}</div><h2>Could not load Hub</h2><p>${o(d.message||"")}</p></div>`,!1}};e.querySelector("#pm-hub-refresh")?.addEventListener("click",l),T(e,l)}async function ue(e,{navigate:t}){let n=`<span class="pm-spacer"></span><button class="pm-icon-btn" id="pm-audit-refresh" aria-label="Refresh" style="background:var(--pm-surface);border:1px solid var(--pm-border);">${h.refresh}</button>`;e.innerHTML=`
    ${k({title:"Audit",leftIcon:"back",onBack:()=>t("#mobile/more"),online:!0,extras:n,hideTitle:!0,hideBrand:!0})}
    <div class="pm-body pm-more-page" id="pm-audit-body">${M()}</div>
  `,S(e,{onBack:()=>t("#mobile/more")});let r=e.querySelector("#pm-audit-body"),s="",l=[],d=()=>{let p=V(l);r.innerHTML=`
      ...
      ${l.length?`<div class="pm-audit-run-list">${l.slice(0,40).map(i=>{let u=s===i.key,v=Array.isArray(i.tools)?i.tools.slice().sort((y,f)=>String(f.timestamp||"").localeCompare(String(y.timestamp||""))):[],b=X(v);return`<article class="pm-card pm-audit-run-card" data-run-key="${o(i.key)}">
    ...
    `,r.querySelectorAll("[data-run-key]").forEach(i=>i.addEventListener("click",()=>{let u=i.getAttribute("data-run-key")||"";s=s===u?"":u,d()}));let a=r.querySelector("#pm-audit-search"),m=r.querySelector("#pm-audit-clear");a?.addEventListener("input",()=>{let i=String(a.value||"").trim().toLowerCase();r.querySelectorAll(".pm-audit-run-card").forEach(u=>{u.hidden=i&&!u.textContent.toLowerCase().includes(i)})}),m?.addEventListener("click",c)},c=async()=>{try{return r.innerHTML=M(),l=await D(200),d(),!0}catch(p){return r.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${h.clipboard}</div><h2>Could not load Audit</h2><p>${o(p.message||"")}</p></div>`,!1}};e.querySelector("#pm-audit-refresh")?.addEventListener("click",c),T(e,c)}async function ye(e,{navigate:t}){let n=ne(),r=`<span class="pm-spacer"></span><button class="pm-icon-btn" type="button" onclick="refreshMemoryGraph(true)" aria-label="Refresh" style="background:var(--pm-surface);border:1px solid var(--pm-border);">${h.refresh}</button>`;e.innerHTML=`
