import{a as xt,b as _t,c as Mt,d as Tt}from"./chunk-VGVLXHST.js";import{c as wt,d as kt,e as At,f as xe,g as _e}from"./chunk-GJZMANL6.js";import{C as ee,D as gt,E as yt,F as ht,G as ft,M as St,N as $t,O as te,a as rt,b as nt,d as W,e as Se,f as it,g as ot,h as de,i as ct,j as lt,l as X,m as dt,o as pt,r as $e,s as ut,t as mt,u as pe,v as we,w as bt,x as ke,y as vt,z as Ae}from"./chunk-XCIG7NTZ.js";import{b as Y}from"./chunk-J5HXEABW.js";import{a as C}from"./chunk-35CAQ6TV.js";import{F as h,G as p,M as fe,N as le}from"./chunk-HDAINNAZ.js";import{e as J,j as ce}from"./chunk-NQO4BGNT.js";import"./chunk-5RLMNBA7.js";import"./chunk-DAASNOK5.js";import"./chunk-PDCVPUEN.js";import{a as Ye,b as Xe,c as et}from"./chunk-ACPZYBFH.js";import"./chunk-MGDZYTA3.js";import{e as he}from"./chunk-CUCDWB4G.js";import"./chunk-YGZSSPM3.js";import{c as Ne,ca as Fe,da as ge,ea as je,fa as Ve,ga as Qe,ha as ye,ia as ie,ja as Ue,ka as Ge,la as oe,m as Be,ma as Ke,na as We,oa as Ze,pa as Je,s as Oe}from"./chunk-4MLV3F4Z.js";import"./chunk-YMT6MSCC.js";import{e as tt,f as at,g as st}from"./chunk-XREJVKMI.js";import"./chunk-4WCZDNBS.js";import"./chunk-3EPMIDRU.js";import"./chunk-JF4LWGNM.js";import"./chunk-IPNQ4FF4.js";import{a as R}from"./chunk-GBLBNUG2.js";import"./chunk-36KIJFV6.js";import"./chunk-EPSJJCWL.js";var ue={running:{label:"running",cls:"running"},idle:{label:"idle",cls:"gray"},scheduled:{label:"scheduled",cls:"orange"},team:{label:"team",cls:"active"},failed:{label:"failed",cls:"orange"}};function Ht(r){let e=ue[r.status]||ue.idle;return`
  ...
  `,le(r,{});let m=r.querySelector("#pm-subagents-body"),o=r.querySelector("#pm-subagents-count");async function c({force:b=!1}={}){let v=[];try{v=await Fe({force:b})}catch(A){m.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${h.robot}</div><h2>Couldn\u2019t load subagents</h2><p>${p(A.message||"Network error")}</p></div>`,o.textContent="0 agents";return}if(o.textContent=`${v.length} agent${v.length===1?"":"s"}`,!v.length){m.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${h.robot}</div><h2>No subagents yet</h2><p>Create agents from the desktop Settings \u2192 Agents page.</p></div>`;return}let n=v[0],S=`
  ...
  `,le(r,{onBack:()=>u?.("#mobile/subagents")});let o=r.querySelector("#pm-detail-body"),c=null;try{c=await ge(e)}catch(d){o.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${h.robot}</div><h2>Couldn\u2019t load subagent</h2><p>${p(d.message||"Network error")}</p></div>`;return}if(!c){o.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${h.robot}</div><h2>Subagent not found</h2><p>${p(e)} isn\u2019t available right now.</p></div>`;return}let g=ue[c.status]||ue.idle,b=["Overview","Chat","Memory","Runs","Heartbeat"],v=`pm-sa-model-${c.id}`,n=`pm-sa-voice-${c.id}`,S=String(c.effectiveModel||c.model||"").trim();o.innerHTML=`
  ...
  `,document.body.appendChild(u);let m=()=>u.remove();u.addEventListener("click",o=>{(o.target===u||o.target.getAttribute("data-close"))&&m()}),u.querySelector("#pm-dispatch-submit").addEventListener("click",async()=>{let o=String(u.querySelector("#pm-dispatch-task").value||"").trim();if(!o){C("Describe the task first","error");return}let c=u.querySelector("#pm-dispatch-submit");c.disabled=!0,c.innerHTML="\u2026sending";try{let g=await Ze(r,o);if(g?.success){let b=String(g.result?.result||g.result?.summary||"").slice(0,140);C(b?`Done \xB7 ${b}`:"Task complete","success"),m()}else C(g?.error||"Dispatch failed","error"),c.disabled=!1,c.innerHTML=`${h.send} Dispatch`}catch(g){C(g.message||"Dispatch failed","error"),c.disabled=!1,c.innerHTML=`${h.send} Dispatch`}}),setTimeout(()=>u.querySelector("#pm-dispatch-task")?.focus(),50)}async function It(r,e){let[u,m]=await Promise.all([je(e),Ve(e)]),o=[{key:"agent",title:"AGENT.md",content:u,exists:!!u,empty:"No AGENT.md is set for this agent yet."},{key:"memory",title:"MEMORY.md",content:m.content,exists:m.exists,empty:"No personal memory file exists for this agent yet."}],c="",g=()=>{r.innerHTML=`<section class="pm-subagent-memory" aria-label="Subagent memory files">
    ...
    </section>`,r.querySelectorAll("[data-memory-file]").forEach(b=>b.addEventListener("click",()=>{let v=b.getAttribute("data-memory-file")||"";c=c===v?"":v,g()})),r.querySelectorAll("[data-memory-copy]").forEach(b=>b.addEventListener("click",()=>{let v=o.find(n=>n.key===b.getAttribute("data-memory-copy"));Ae(v?.content||"",b)}))};g()}function Nt(r){return String(r||"Task").replace(/^\s*\[\s*(?:subagent|agent)\s*\]\s*/i,"").trim()||"Task"}function Bt(r){let e=String(r||""),u=e.match(/\*\*Ready artifact\*\*\s*[-:]\s*`([^`\n]+)`/i),m=e.match(/\bSHA-?256:\s*`?([a-f0-9]{32,})`?/i);if(!u||!m)return null;let o=e.match(/\bSize:\s*\*{0,2}([\d,.]+\s*(?:bytes?|kb|mb|gb))\*{0,2}/i),c=e.indexOf(`
`,m.index+m[0].length);return{raw:e.slice(u.index,c===-1?e.length:c),path:String(u[1]||"").trim(),sha256:String(m[1]||"").trim(),size:String(o?.[1]||"").trim()}}function Lt(r,{compact:e=!1}={}){let u=String(r||"").trim();if(!u)return"";let m=Bt(u),o=m?u.replace(m.raw,"").trim():u,c=e&&o.length>420?`${o.slice(0,417).trimEnd()}...`:o,g=c?`<div class="pm-sa-run-summary markdown-body">${lt(c)}</div>`:"";return m?`${g}
    ...
    `,T()},A=a=>(Array.isArray(a?.recoveryConversation)?a.recoveryConversation:[]).map((i,d)=>({id:`recovery_${a?.id||a?.taskId||"task"}_${d}`,role:i?.role==="user"?"user":"agent",content:String(i?.content||""),body:{text:String(i?.content||""),attachments:Array.isArray(i?.attachmentPreviews)?i.attachmentPreviews:[]},attachmentPreviews:Array.isArray(i?.attachmentPreviews)?i.attachmentPreviews:[],createdAt:Number(i?.timestamp||Date.now())||Date.now()})),x=(a,i,d)=>{let l=A(a),f=l.length?l.map(D=>we(D,{sender:"Recovery"})).join(""):'<div style="font-size:12px;color:var(--pm-muted);padding:8px 2px;">No recovery messages yet.</div>',_=`pm-sa-run-${String(i||"").replace(/[^a-zA-Z0-9_-]/g,"_")}`,w=b(i);return`<section class="pm-sa-run-recovery-panel">
    ...
    </section>`},q=a=>{let i=String(a.id||a.taskId||""),d=String(a.status||a.taskStatus||"").toLowerCase(),l=xt(d),f=String(a.resultPreview||a.finalSummary||a.pauseAnalysis?.message||a.prompt||"").trim(),_=Nt(a.taskName||a.title||"Task"),w=a.startedAt||a.createdAt,D=a.completedAt||a.finishedAt,F=D&&w?wt(D-w):"",M=m===i,E=o[i]?.task,H=o[i]?.loading,L=!!(E?.canRecover||a.canRecover||["needs_assistance","awaiting_user_input","paused","stalled","failed"].includes(d));return`
      <article class="pm-card pm-sa-run-card" data-sa-run-id="${p(i)}" style="padding:14px 16px;cursor:pointer;border-color:${M?"var(--pm-orange)":"var(--pm-border)"};">
        ...
        ${M?`<div style="margin-top:12px;padding-top:12px;border-top:1px solid var(--pm-border);display:flex;flex-direction:column;gap:12px;cursor:default;">
          ${H||!E?'<div class="pm-card-body">Loading run details...</div>':`
            ...
            ${L||E.recoveryConversation?.length?x(E,i,L):""}
      ...
      </article>`};async function P(a){if(m=m===a?"":a,m&&!o[a]?.task){o[a]={loading:!0},S();try{let i=await Ue(e,a);o[a]={task:i.task||null,run:i.run||null,evidenceBus:i.evidenceBus||null}}catch(i){o[a]={task:null,error:i?.message||"Failed to load run"},C(i?.message||"Failed to load run","error")}}S()}function T(){r.querySelector("#pm-sa-runs-refresh")?.addEventListener("click",async a=>{a.stopPropagation(),u=await ie(e,50),S()}),r.querySelectorAll("[data-sa-run-id]").forEach(a=>{a.addEventListener("click",async i=>{i.target.closest("button, textarea, input, a, summary")||await P(a.getAttribute("data-sa-run-id"))})}),r.querySelectorAll("[data-sa-run-copy]").forEach(a=>{a.addEventListener("click",i=>{i.preventDefault(),i.stopPropagation(),Ae(a.getAttribute("data-sa-run-copy")||"",a)})}),r.querySelectorAll("[data-sa-run-composer]").forEach(a=>{let i=String(a.getAttribute("data-sa-run-composer")||"").trim();if(!i)return;let d=`pm-sa-run-${i.replace(/[^a-zA-Z0-9_-]/g,"_")}`,l=b(i),f=async(w,{queued:D=!1}={})=>{let F=String(w?.text||"").trim(),M=Array.isArray(w?.files)?w.files:[];if(!F&&!M.length)return;if(l.busy){if(l.queue.length>=8){v(i,"Queue is full. Wait for the current reply to finish.","error"),C("Recovery queue is full.","error");return}l.queue.push({text:F,files:M}),v(i,`Queued reply ${l.queue.length}/8.`,"queued"),C("Recovery reply queued.","info"),n(i);return}l.busy=!0,l.controller=new AbortController,l.status="",l.tone="",n(i),v(i,D?"Sending queued reply\u2026":"Sending recovery reply\u2026","busy");let E=!1;try{let H=M;M.length&&(v(i,`Uploading ${M.length===1?"attachment":"attachments"}\u2026`,"busy"),H=(await $e(M,{signal:l.controller.signal})).map((V,I)=>({...M[I]||{},name:V.name||M[I]?.name||"attachment",kind:V.isImage?"image":V.isVideo?"video":M[I]?.kind||"file",workspacePath:V.workspacePath||M[I]?.workspacePath,path:V.workspacePath||M[I]?.path,dataUrl:M[I]?.dataUrl,mimeType:M[I]?.mimeType,sizeLabel:M[I]?.sizeLabel})));let L=await Ge(e,i,F||(H.length?"Please review the attached file(s).":""),H,{signal:l.controller.signal});L?.task&&(o[i]={task:L.task,run:L.run||null,evidenceBus:L.evidenceBus||null}),u=await ie(e,50),E=!0,v(i,L?.resumed?"Run resumed.":"Reply sent.","success"),C(L?.resumed?"Run resumed":"Reply sent","success")}catch(H){if(l.controller?.signal?.aborted||H?.name==="AbortError")v(i,"Stopped. The run was not changed.","stopped"),C("Recovery reply stopped.","info");else{let U=H?.message||"Recovery reply failed.";v(i,U,"error"),C(U,"error")}}finally{let H=E&&l.queue.length?l.queue.shift():null;l.busy=!1,l.controller=null,S(),H&&f(H,{queued:!0})}},_=_e(a,d,{placeholder:"Reply to this run...",draftKey:`subagent_run:${e}:${i}`,isBusy:()=>l.busy,onAbort:()=>{l.busy&&(l.controller?.abort?.(),n(i))},onSubmit:w=>f(w)});g.set(i,_)})}S()}async function Rt(r,e){r.innerHTML='<div class="pm-card" style="text-align:center;padding:24px;color:var(--pm-muted);">Loading heartbeat\u2026</div>';let{status:u,markdown:m}=await Qe(e),o=u?.lastTickAt||u?.last_tick_at||u?.timestamp;r.innerHTML=`
    <div class="pm-card">
  ...
  `;let o=r.querySelector("#pm-sa-chat-list"),c=r.querySelector(".pm-sa-chat-scrollport"),g=r.querySelector("#pm-sa-chat-queue");mt(o,()=>{});let b=r.querySelector("#pm-sa-chat-goal");ot(b,it.activeSessionId,{fallbackToLast:!0});let v=[],n=null,S=null,A=0,x="",q=!1,P=!1,T=[],a=[],i=null,d=null,l=0,f=null,_=0,w=!0,D=()=>!!(S||n?.streaming||q),F=(t={})=>{let s=X(t),y=String(s.sessionId||s.sourceSessionId||"").trim();return!!s.id&&(y===m||String(s.agentId||"").trim()===String(e.id))},M=(t={})=>{if(!F(t))return!1;let s=X(t),y=a.findIndex(k=>String(k?.approvalRequest?.id||"")===s.id),$={role:"agent",content:"",createdAt:Date.now(),approvalRequest:s};return y>=0?a[y]={...a[y],approvalRequest:{...a[y].approvalRequest||{},...s}}:a.push($),a=a.slice(-8),!0},E=(t,s,y={})=>{let $=String(t||"").trim();if(!$)return!1;let k=a.findIndex(O=>String(O?.approvalRequest?.id||"")===$);return k<0?!1:(a[k].approvalRequest=X({...a[k].approvalRequest||{},...y.approval||y,id:$,status:s}),!0)},H=async()=>{let t=await Oe("pending").catch(()=>[]);(Array.isArray(t)?t:[]).forEach(M)};function L(){g&&(g.hidden=T.length===0,g.innerHTML=T.length?`<div class="pm-mobile-queued-list">${T.map((t,s)=>`
