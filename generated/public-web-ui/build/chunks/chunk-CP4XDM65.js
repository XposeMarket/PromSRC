function g(r){return r?String(r).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}var ee=g;function F(r){let t=Date.now()-r;return t<6e4?"just now":t<36e5?`${Math.floor(t/6e4)}m ago`:t<864e5?`${Math.floor(t/36e5)}h ago`:`${Math.floor(t/864e5)}d ago`}function P(r,t=0){let n=Number(r);return Number.isFinite(n)?`${n.toFixed(t)}%`:"--%"}function R(r,t){let n=Number(r),e=Number(t);return!Number.isFinite(n)||!Number.isFinite(e)||e<=0?"-- / -- GB":`${n.toFixed(1)} / ${e.toFixed(1)} GB`}function E(r){let t=Number(r);return Number.isFinite(t)?`${Math.max(0,Math.min(100,t))}%`:"0%"}function q(r,t){let n=document.getElementById(r);n&&(n.textContent=String(t||""))}function te(r){let t=String(r||"").trim(),n=g(t);return`<span class="t-think-sizer" aria-hidden="true">${n}</span><span class="t-think-text" data-text="${n}">${n}</span>`}function re(r,t){let n=String(t||"").trim(),e=r?.querySelector?.(".t-think-text");if(!e||!n)return!1;let i=String(e.textContent||"").trim();if(!i||i===n)return!1;r.querySelectorAll?.(".t-think-text").forEach(s=>{s!==e&&s.remove()});let o=e.cloneNode(!0);o.classList.remove("is-enter-start"),o.classList.add("is-exit"),o.textContent=n,o.setAttribute("data-text",n),e.classList.remove("is-exit"),e.classList.add("is-enter-start");let a=r.querySelector?.(".t-think-sizer");a&&n.length>String(a.textContent||"").length&&(a.textContent=n),r.appendChild(o),e.offsetWidth;let c=()=>{e.isConnected!==!1&&e.classList.remove("is-enter-start")};return typeof requestAnimationFrame=="function"?requestAnimationFrame(c):typeof setTimeout=="function"&&setTimeout(c,0),typeof setTimeout=="function"&&setTimeout(()=>{o.isConnected!==!1&&o.remove(),e.isConnected!==!1&&e.classList.remove("is-enter-start")},420),!0}function O(r,t){let n=document.getElementById(r);n&&(n.style.width=E(t))}function M(r,t,n="info",e=5e3,i={}){let o=typeof i?.key=="string"?i.key.trim():"";if(o)for(let u of document.querySelectorAll(".__sc-toast"))u.dataset.scToastKey===o&&u.remove();let a=n==="warn"?"warning":["info","success","error","warning"].includes(n)?n:"info",c={info:"\u2139\uFE0F",success:"\u2713",error:"\u26A0\uFE0F",warning:"\u26A0\uFE0F"},s=document.createElement("div"),l=24+[...document.querySelectorAll(".__sc-toast")].reduce((u,f)=>u+f.offsetHeight+8,0);if(s.className=`__sc-toast __sc-toast--${a}`,o&&(s.dataset.scToastKey=o),s.style.cssText=`position:fixed;bottom:${l}px;right:24px;z-index:99999;`,s.innerHTML=`
    <span class="__sc-toast-icon" aria-hidden="true">${c[a]}</span>
    <div class="__sc-toast-copy">
      <div class="__sc-toast-title">${g(r)}</div>
      ${t?`<div class="__sc-toast-body">${g(String(t))}</div>`:""}
    </div>
    <button class="__sc-toast-close" type="button" aria-label="Dismiss">&times;</button>
  `,!document.getElementById("__sc-toast-style")){let u=document.createElement("style");u.id="__sc-toast-style",u.textContent="@keyframes scToastIn{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}",document.head.appendChild(u)}document.body.appendChild(s);let d=Math.max(0,Math.min(5e3,Number.isFinite(Number(e))?Number(e):5e3));setTimeout(()=>{s.style.transition="opacity 0.3s",s.style.opacity="0",setTimeout(()=>s.remove(),300)},d),s.querySelector(".__sc-toast-close")?.addEventListener("click",()=>s.remove())}function D(r,t){M(r,t,"info")}function H(r,t,n,e={}){let{title:i="Confirm",confirmText:o="Confirm",cancelText:a="Cancel",danger:c=!1,details:s=""}=e,m=document.createElement("div");m.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.35);display:flex;align-items:center;justify-content:center;animation:scToastIn 0.15s ease";let l=document.createElement("div");l.style.cssText="background:var(--panel);border:1.5px solid var(--line);border-radius:14px;padding:24px 24px 18px;max-width:560px;width:92%;box-shadow:0 8px 40px rgba(0,0,0,0.18);font-family:var(--font)",l.innerHTML=`
    <div style="font-size:15px;font-weight:800;margin-bottom:10px">${g(i)}</div>
    <div style="font-size:13px;color:var(--muted);line-height:1.6;margin-bottom:18px">${g(r)}</div>
    ${s?`<pre style="margin:0 0 18px;padding:12px 14px;border-radius:10px;border:1px solid var(--line);background:var(--panel-2);font-size:11px;line-height:1.65;color:var(--text);white-space:pre-wrap;word-break:break-word;font-family:'Cascadia Code','Fira Code','Consolas',monospace">${g(s)}</pre>`:""}
    <div style="display:flex;gap:8px;justify-content:flex-end">
      <button id="__sc-confirm-cancel" style="border:1px solid var(--line);background:var(--panel-2);color:var(--muted);border-radius:8px;padding:7px 16px;font-size:12px;font-weight:700;cursor:pointer">${g(a)}</button>
      <button id="__sc-confirm-ok" style="border:none;background:${c?"#dc2626":"var(--brand)"};color:#fff;border-radius:8px;padding:7px 16px;font-size:12px;font-weight:700;cursor:pointer">${g(o)}</button>
    </div>
  `,m.appendChild(l),document.body.appendChild(m);let d=()=>m.remove();l.querySelector("#__sc-confirm-cancel").onclick=()=>{d(),n&&n()},l.querySelector("#__sc-confirm-ok").onclick=()=>{d(),t&&t()},m.addEventListener("click",u=>{u.target===m&&(d(),n&&n())})}var b=[];function j(r,t="log"){let n=new Date().toLocaleTimeString();b.push({text:`[${n}] ${String(r??"")}`,type:String(t||"log").replace(/[^a-z0-9_-]/gi,"")||"log"}),b.length>100&&b.shift();let e=document.getElementById("log-panel");e&&(e.replaceChildren(...b.map(i=>{let o=document.createElement("div");return o.className=`log-line ${i.type}`,o.textContent=i.text,o})),e.scrollTop=e.scrollHeight)}var p=Object.freeze({bg:"transparent",bgSoft:"transparent",surface:"transparent",surfaceSecondary:"transparent",border:"currentColor",borderStrong:"currentColor",text:"currentColor",muted:"currentColor",accent:"currentColor",accentStrong:"currentColor",success:"currentColor",warning:"currentColor",danger:"currentColor"});function k(r,t){return String(r||"").replace(/[<>{};\r\n]/g,"").trim()||t}function A(){let r=document.documentElement,t=typeof getComputedStyle=="function"?getComputedStyle(r):null,n=(i,o)=>{for(let a of i){let c=t?.getPropertyValue(a)?.trim();if(c)return k(c,o)}return o},e={isDark:r.getAttribute("data-theme")==="dark",bg:n(["--bg","--pm-chat-page-bg"],p.bg),bgSoft:n(["--bg-soft"],p.bgSoft),surface:n(["--panel","--composer-panel"],p.surface),surfaceSecondary:n(["--panel-2","--composer-bg"],p.surfaceSecondary),border:n(["--line","--composer-border"],p.border),borderStrong:n(["--line-strong"],p.borderStrong),text:n(["--text","--fg","--composer-text"],p.text),muted:n(["--muted","--composer-muted"],p.muted),accent:n(["--brand","--pm-custom-accent"],p.accent),accentStrong:n(["--brand-2"],p.accentStrong),success:n(["--ok"],p.success),warning:n(["--warn"],p.warning),danger:n(["--err"],p.danger)};return e.series=[e.accent,e.accentStrong,e.success,e.warning,e.danger,e.muted],e.vars={"--prom-bg":e.bg,"--prom-bg-soft":e.bgSoft,"--prom-surface":e.surface,"--prom-surface-secondary":e.surfaceSecondary,"--prom-border":e.border,"--prom-border-strong":e.borderStrong,"--prom-text":e.text,"--prom-muted":e.muted,"--prom-accent":e.accent,"--prom-accent-strong":e.accentStrong,"--prom-success":e.success,"--prom-warning":e.warning,"--prom-danger":e.danger,"--prom-series-1":e.series[0],"--prom-series-2":e.series[1],"--prom-series-3":e.series[2],"--prom-series-4":e.series[3],"--prom-series-5":e.series[4],"--prom-series-6":e.series[5],"--bg":e.bg,"--bg-soft":e.bgSoft,"--panel":e.surface,"--panel-2":e.surfaceSecondary,"--line":e.border,"--line-strong":e.borderStrong,"--text":e.text,"--fg":e.text,"--muted":e.muted,"--brand":e.accent,"--brand-2":e.accentStrong,"--ok":e.success,"--warn":e.warning,"--err":e.danger},e}function Y(r){if(r&&typeof r=="object"&&r.vars)return r;let t={isDark:typeof r=="boolean"?r:!!r?.isDark,...p};return t.series=[t.accent,t.accentStrong,t.success,t.warning,t.danger,t.muted],t.vars=Object.fromEntries([["--prom-bg",t.bg],["--prom-bg-soft",t.bgSoft],["--prom-surface",t.surface],["--prom-surface-secondary",t.surfaceSecondary],["--prom-border",t.border],["--prom-border-strong",t.borderStrong],["--prom-text",t.text],["--prom-muted",t.muted],["--prom-accent",t.accent],["--prom-accent-strong",t.accentStrong],["--prom-success",t.success],["--prom-warning",t.warning],["--prom-danger",t.danger],...t.series.map((n,e)=>[`--prom-series-${e+1}`,n]),["--bg",t.bg],["--bg-soft",t.bgSoft],["--panel",t.surface],["--panel-2",t.surfaceSecondary],["--line",t.border],["--line-strong",t.borderStrong],["--text",t.text],["--fg",t.text],["--muted",t.muted],["--brand",t.accent],["--brand-2",t.accentStrong],["--ok",t.success],["--warn",t.warning],["--err",t.danger]]),t}function U(r){let t=r?.vars&&typeof r.vars=="object"?r.vars:{};return Object.entries(t).map(([n,e])=>`${n}:${k(e,"transparent")}`).join(";")}function B(r,t,n){let e=Y(n),i=w({background:"transparent",primaryColor:e.surface,primaryTextColor:e.text,primaryBorderColor:e.borderStrong,lineColor:e.muted,secondaryColor:e.surfaceSecondary,secondaryTextColor:e.text,secondaryBorderColor:e.border,tertiaryColor:e.bgSoft,tertiaryTextColor:e.text,tertiaryBorderColor:e.border,textColor:e.text,mainBkg:e.surface,nodeBorder:e.borderStrong,clusterBkg:e.surfaceSecondary,clusterBorder:e.border,edgeLabelBackground:"transparent"}),o=w({text:e.text,muted:e.muted,border:e.border,series:e.series}),a=`:root{${U(e)}color-scheme:${e.isDark?"dark":"light"}}*{margin:0;padding:0;box-sizing:border-box}html,body{background:transparent!important;color:var(--prom-text);color-scheme:${e.isDark?"dark":"light"};max-width:100%;overflow-x:hidden}body{min-height:0;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}`;return r==="chart"?`<!DOCTYPE html><html><head><meta charset="UTF-8">
...
</head><body>${t}</body></html>`}function x(r){return String(r||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;")}function w(r){return JSON.stringify(r??null).replace(/</g,"\\u003c")}function W(r,t={}){let n=String(t.visualId||""),e=t.state&&typeof t.state=="object"?t.state:{},i=`<script>(function(){
var visualId=${w(n)},last=0,state=${w(e)}||{};
...
if('ResizeObserver'in window){var ro=new ResizeObserver(send);if(document.documentElement)ro.observe(document.documentElement);if(document.body)ro.observe(document.body)}addEventListener('load',function(){restoreControls();send();post('prometheus:visual-ready')});setTimeout(send,50);setTimeout(send,250);setTimeout(send,1000)})();<\/script>`,o=String(r||"");return/<head\b[^>]*>/i.test(o)?o.replace(/<head\b[^>]*>/i,a=>`${a}${i}`):`${i}${o}`}function X(){if(window.__PROM_VISUAL_MESSAGE_BRIDGE_INSTALLED__)return;window.__PROM_VISUAL_MESSAGE_BRIDGE_INSTALLED__=!0;let r=()=>{let t=A();document.querySelectorAll('iframe[data-prom-visual="true"]').forEach(n=>{let e=String(n.getAttribute("data-visual-id")||"");if(!(!e||!n.contentWindow))try{n.contentWindow.postMessage({type:"prometheus:visual-theme",visualId:e,theme:t},"*")}catch{}})};document.addEventListener("prom-theme-change",()=>setTimeout(r,0)),document.addEventListener("prom-appearance-change",()=>setTimeout(r,0)),window.addEventListener("message",t=>{let n=t?.data;if(!n||!String(n.type||"").startsWith("prometheus:visual-"))return;let e=Array.from(document.querySelectorAll('iframe[data-prom-visual="true"]')).find(o=>o.contentWindow===t.source);if(!e)return;let i=String(e.getAttribute("data-visual-id")||"");if(String(n.visualId||"")===i){if(n.type==="prometheus:visual-resize"){let o=Number(n.height);if(!Number.isFinite(o))return;let a=Math.min(1e4,Math.max(120,Math.ceil(o))),c=Math.ceil(e.getBoundingClientRect().height||0);if(Math.abs(c-a)<=1)return;e.style.height=`${a}px`,e.style.minHeight=`${a}px`;return}if(n.type==="prometheus:visual-state"&&n.state&&typeof n.state=="object"){window.dispatchEvent(new CustomEvent("prometheus:visual-state-change",{detail:{visualId:i,state:n.state}}));return}n.type==="prometheus:visual-followup"&&n.prompt&&window.dispatchEvent(new CustomEvent("prometheus:visual-followup",{detail:{visualId:i,prompt:String(n.prompt),title:String(n.title||"")}}))}})}function S(r){if(!r?.getAttribute)return"";let t=r.closest?.(".visual-block"),n=String(r.getAttribute("data-visual-id")||"").trim();return n?[n,String(r.getAttribute("data-visual-version")||"1"),String(t?.getAttribute("data-vis-lang")||""),String(t?.getAttribute("data-vis-code")||"")].join("\0"):""}function y(r){if(!r?.querySelector&&!r?.matches)return"";let t=r.matches?.('iframe[data-prom-visual="true"]')?r:r.querySelector?.('iframe[data-prom-visual="true"]');return S(t)}function G(r,t){return!r||!t||r.nodeType!==t.nodeType?!1:r.nodeType!==1?!0:String(r.tagName||"").toLowerCase()===String(t.tagName||"").toLowerCase()}function K(r,t){let e=r.matches?.('iframe[data-prom-visual="true"]')?new Set(["srcdoc","style"]):new Set;Array.from(r.attributes||[]).forEach(i=>{e.has(i.name)||t.hasAttribute(i.name)||r.removeAttribute(i.name)}),Array.from(t.attributes||[]).forEach(i=>{e.has(i.name)||r.getAttribute(i.name)!==i.value&&r.setAttribute(i.name,i.value)})}function v(r,t,n,e=null){let i=Array.from(t||[]),o=Array.from(n||[]),a=Math.min(i.length,o.length),c=0;for(let s=0;s<a;s+=1){let m=i[s],l=o[s],d=z(m,l);if(d){c+=d.reused;continue}let u=l.cloneNode(!0);r.replaceChild(u,m)}for(let s=a;s<o.length;s+=1)r.insertBefore(o[s].cloneNode(!0),e);for(let s=a;s<i.length;s+=1)i[s].remove();return c}function J(r,t){return r.length===t.length&&r.every((n,e)=>n===t[e])}function L(r,t){let n=Array.from(r.childNodes||[]),e=Array.from(t.childNodes||[]),i=n.map(y).filter(Boolean),o=e.map(y).filter(Boolean);if(i.length&&J(i,o)){let a=0,c=0,s=0;for(let m of o){let l=n.findIndex((f,h)=>h>=a&&y(f)===m),d=e.findIndex((f,h)=>h>=c&&y(f)===m);if(l<0||d<0)return v(r,n,e);s+=v(r,n.slice(a,l),e.slice(c,d),n[l]);let u=z(n[l],e[d]);if(!u)return v(r,n,e);s+=u.reused,a=l+1,c=d+1}return s+=v(r,n.slice(a),e.slice(c)),s}return v(r,n,e)}function z(r,t){return G(r,t)?r.nodeType===3||r.nodeType===8?(r.nodeValue!==t.nodeValue&&(r.nodeValue=t.nodeValue),{reused:0}):r.matches?.('iframe[data-prom-visual="true"]')?S(r)===S(t)?{reused:1}:null:(K(r,t),{reused:L(r,t)}):null}function I(r,t){return!r?.childNodes||!t?.childNodes?0:L(r,t)}function Q(r,t){if(!r)return 0;let n=String(t||"");if(typeof document>"u"||typeof document.createElement!="function"||typeof r.appendChild!="function")return r.innerHTML=n,0;let e=document.createElement("template");e.innerHTML=n;let i=!!r.querySelector?.('iframe[data-prom-visual="true"]'),o=!!e.content.querySelector?.('iframe[data-prom-visual="true"]');return!i&&!o?(r.innerHTML=n,0):I(r,e.content)}function Z(r,t,n=0){let e=`${r}\0${n}\0${t}`,i=2166136261;for(let o=0;o<e.length;o+=1)i^=e.charCodeAt(o),i=Math.imul(i,16777619);return`visual_local_${(i>>>0).toString(36)}`}function V(r,t,n={}){X();let e=n.artifact&&typeof n.artifact=="object"?n.artifact:null,i=String(e?.id||n.visualId||Z(r,t,n.ordinal||0)),o=`vis_${i.replace(/[^a-z0-9_-]/gi,"_")}`,a=A(),c=W(B(r,t,a),{visualId:i,state:e?.state||n.state||{}}),s=x(c),m=r.replace(/"/g,""),l=x(t),d=r==="chart"?240:r==="html"?180:220;return`<div class="visual-block visual-block--inline" id="${o}-wrap" data-vis-lang="${m}" data-vis-code="${l}" data-vis-surface="inline">
  <iframe
    id="${o}"
    ...
    loading="lazy"
  ></iframe>
</div>`}function $(r){let t=String(r||""),n=typeof window<"u"?window.DOMPurify:null;return!n||typeof n.sanitize!="function"?g(t):n.sanitize(t,{USE_PROFILES:{html:!0},FORBID_TAGS:["script","style","iframe","object","embed","form","input","button","textarea","select","option","svg","math","link","meta","base"],FORBID_ATTR:["style","srcdoc","formaction","xlink:href"],ALLOW_DATA_ATTR:!1,ALLOW_ARIA_ATTR:!0,RETURN_TRUSTED_TYPE:!1})}function _(r,t={}){if(!r)return"";try{let n=[],e=`PROMVISUAL${Math.random().toString(36).slice(2)}X`,i=/```(chart|svg|html|mermaid)\n([\s\S]*?)```/g,o=0,a=Array.isArray(t.visualArtifacts)?t.visualArtifacts.filter(d=>d?.type==="visual"):[],c=String(r).replace(i,(d,u,f)=>{let h=n.length,T=u.toLowerCase(),N=a.find(C=>Number(C.ordinal)===o&&String(C.renderer||"")===T)||null;return n.push({lang:T,code:f.trim(),partial:!1,artifact:N,ordinal:o}),o+=1,`${e}${h}END`}),s=/```(chart|svg|html|mermaid)\n([\s\S]*)$/,m=c.match(s);if(m){let d=n.length;n.push({lang:m[1].toLowerCase(),code:m[2],partial:!0}),c=c.slice(0,m.index)+`${e}${d}END`}let l=$(marked.parse(c,{breaks:!0,gfm:!0,mangle:!1,headerIds:!1}));if(n.length){let d=new RegExp(`${e}(\\d+)END`,"g");l=l.replace(d,(u,f)=>{let h=n[+f];return h?h.partial?"":V(h.lang,h.code,{artifact:h.artifact,ordinal:h.ordinal}):""}),l=l.replace(/<p>\s*(<div class="visual-block"[\s\S]*?<\/div>)\s*<\/p>/g,"$1")}return l}catch{return g(r)}}window.escHtml=g;window.escapeHtml=g;window.sanitizeHtml=$;window.renderMd=_;window.timeAgo=F;window.fmtPercent=P;window.fmtMemoryGb=R;window.meterWidth=E;window.setText=q;window.setMeter=O;window.showToast=M;window.bgtToast=D;window.showConfirm=H;window.log=j;window.buildVisualSrcdoc=B;window.buildVisualIframe=V;window.preserveVisualIframes=I;window.setInnerHTMLPreservingVisuals=Q;window.renderMd=_;export{g as a,ee as b,F as c,P as d,R as e,E as f,q as g,te as h,re as i,O as j,M as k,D as l,H as m,j as n,B as o,I as p,Q as q,V as r,$ as s,_ as t};
