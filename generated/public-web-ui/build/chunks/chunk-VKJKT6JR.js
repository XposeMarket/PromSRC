import{A as We,C as j,H as Qe,I as Ue,J as Oe,M as Ge,c as Ce,f as He,g as Re,h as ne,i as De,j as Pe,l as ee,m as je,o as ze,p as Be,q as de,r as Fe,s as Ie,t as Ne,u as ie,v as Ve}from"./chunk-PT3732BB.js";import{b as Z}from"./chunk-J5HXEABW.js";import{a as I}from"./chunk-35CAQ6TV.js";import{H as g,I as u,O as Ee,P as ce}from"./chunk-YIZUMKTO.js";import{e as X}from"./chunk-QUP5IS2R.js";import{$ as Me,Ia as qe,N as oe,O as he,P as le,Q as be,R as ye,S as we,T as $e,U as xe,V as Se,W as Ae,X as re,Z as ke,_ as Le,aa as Te,ba as _e,c as ge,s as fe}from"./chunk-EIYUY45O.js";import{a as T}from"./chunk-GBLBNUG2.js";function Je(e){let i=e.house==="blue"?"#4a82d1":"#a4682b";return`
  ...
  `,ce(e,{});let b=e.querySelector("#pm-teams-body"),n=e.querySelector("#pm-teams-count"),a=e.querySelector("#pm-teams-refresh");async function v({force:y=!1}={}){let c=[];try{c=await he({force:y})}catch(S){b.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${g.users}</div><h2>Couldn\u2019t load teams</h2><p>${u(S.message||"Network error")}</p></div>`,n.textContent="0 teams";return}if(n.textContent=`${c.length} team${c.length===1?"":"s"}`,!c.length){b.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${g.users}</div><h2>No teams yet</h2><p>Create your first team from the desktop app.</p></div>`;return}let w=c.find(S=>S.featured)||c[0],r=null;try{r=await le(w.id)}catch{}let $=r?`
    ...
    `:"";b.innerHTML=`
      <div class="pm-team-grid">${c.map(Je).join("")}</div>
      ${$}
  ...
  `,ce(e,{onBack:()=>f("#mobile/teams")});let b=e.querySelector("#pm-detail-body"),n=null;try{n=await le(i)}catch(d){b.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${g.users}</div><h2>Couldn\u2019t load team</h2><p>${u(d.message||"Network error")}</p></div>`;return}if(!n){b.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${g.users}</div><h2>Team not found</h2><p>This team isn\u2019t available right now.</p></div>`;return}let a=["Context","Subagents","Workspace","Memory","Runs","Team Chat"];b.innerHTML=`
    ...
    <div class="pm-tabs" role="tablist">
      ${a.map((d,A)=>`<button class="${A===0?"active":""}" data-tab="${d}">${u(d)}</button>`).join("")}
    </div>
    ...
    <div class="pm-team-preview">
      ...
      <div class="pm-chip-row">
        ${n.members.map(d=>`<span class="pm-member-chip"><span class="pm-avatar" style="background:${d.color}">${d.avatar}</span>${u(d.name)}</span>`).join("")}
      </div>
    </div>
  ...
  `;let v=b.querySelector("#pm-context-slot"),o=b.querySelector("#pm-tab-slot");async function y(d){try{o?._pmCleanup?.()}catch{}if(o&&(o._pmCleanup=null),b.querySelectorAll(".pm-tabs button").forEach(A=>A.classList.toggle("active",A.getAttribute("data-tab")===d)),d==="Context"){v.style.display="",o.innerHTML="";return}v.style.display="none",o.innerHTML=`<div class="pm-card" style="text-align:center;padding:24px;color:var(--pm-muted);">Loading ${u(d)}\u2026</div>`;try{d==="Subagents"?await et(o,n):d==="Runs"?await at(o,i):d==="Team Chat"?await ot(o,i):d==="Workspace"?await dt(o,i):d==="Memory"&&await pt(o,i,n)}catch(A){o.innerHTML=`<div class="pm-card"><div class="pm-card-head">${g.users} Error</div><div class="pm-card-body">${u(A.message||"Failed to load")}</div></div>`}}b.querySelectorAll(".pm-tabs button").forEach(d=>{d.addEventListener("click",()=>y(d.getAttribute("data-tab")))});let c=a.find(d=>d.toLowerCase().replace(/\s+/g,"-")===String(h||"").toLowerCase());c&&c!=="Context"&&y(c);let w=b.querySelector("[data-toggle-purpose]"),r=b.querySelector("[data-purpose]");w&&r&&w.addEventListener("click",()=>{r.getAttribute("data-collapsed")==="1"?(r.style.webkitLineClamp="unset",r.style.display="block",r.setAttribute("data-collapsed","0"),w.textContent="Show less \u25B4"):(r.style.display="-webkit-box",r.style.webkitLineClamp="6",r.setAttribute("data-collapsed","1"),w.textContent="Show more \u25BE")});async function $(d,A,D){let W=d.innerHTML;d.disabled=!0,d.style.opacity="0.6";try{let L=await A();if(!L||L.success===!1)throw new Error(L?.error||"Failed");return I(D,"success"),L}catch(L){throw I(L.message||"Action failed","error"),L}finally{d.disabled=!1,d.style.opacity="",d.innerHTML=W}}b.querySelectorAll("[data-act]").forEach(d=>{let A=d.getAttribute("data-act");d.addEventListener("click",async()=>{if(A==="start")await $(d,()=>be(i),"Run started").catch(()=>{});else if(A==="pause"){let D=n.paused;try{await $(d,()=>D?we(i):ye(i),D?"Team resumed":"Team paused"),n.paused=!D,d.innerHTML=n.paused?`${g.play} Resume`:`${g.pause} Pause`}catch{}}else if(A==="review")await $(d,()=>$e(i),"Manager review triggered").catch(()=>{});else if(A==="delete"){if(!window.confirm(`Delete team "${n.name}"? This cannot be undone.`))return;try{await $(d,()=>xe(i),"Team deleted"),oe(),f("#mobile/teams")}catch{}}})});let S=b.querySelector("[data-save-ref]");S&&S.addEventListener("click",async()=>{let d=b.querySelector("#pm-ref-title"),A=b.querySelector("#pm-ref-body"),D=(d.value||"").trim(),W=(A.value||"").trim();if(!D||!W){I("Title and content required","error");return}try{await $(S,()=>qe(i,D,W),"Reference saved"),d.value="",A.value=""}catch{}}),e._pmCleanup=()=>{try{o?._pmCleanup?.()}catch{}}}var Ye={working:{label:"working",cls:"running"},active:{label:"active",cls:"active"},ready:{label:"ready",cls:"active"},idle:{label:"idle",cls:"gray"},blocked:{label:"blocked",cls:"orange"},paused:{label:"paused",cls:"gray"},awaiting:{label:"awaiting",cls:"orange"},offline:{label:"offline",cls:"gray"}};function Ze(e){if(!e||e<0)return"\u2014";let i=Math.floor(e/1e3);return i<60?`${i}s`:`${Math.floor(i/60)}m ${i%60}s`}async function et(e,i){let f=null;try{f=await Se(i.id)}catch{}let h=f?.memberStates||{},b=Array.isArray(f?.activeDispatches)?f.activeDispatches:[],n=new Map;for(let v of b){let o=String(v.agentId||v.subagentId||"").trim();o&&n.set(o,v)}let a=i.members.filter(v=>v.id!=="manager").map(v=>{let o=h[v.id]||{},y=Ye[String(o.status||"idle").toLowerCase()]||Ye.idle,c=n.get(v.id);return`
      <article class="pm-card">
        ...
        <div style="display:flex;justify-content:space-between;align-items:center;font-size:12px;color:var(--pm-muted);">
          <span>${c?"\u{1F4E1} dispatched":"Last update"}</span>
          <span>${Z(o.lastUpdateAt||c?.startedAt)}</span>
        </div>
      </article>
  ...
  `).join("")}function st(e){return e?{type:String(e.type||e.event||""),...e.data||{}}:null}function rt(e,i,f){if(!e)return;let h=new Set,b=new Set;e.querySelectorAll(".pm-agent-chat-msg").forEach((n,a)=>{n.querySelector(".pm-trace-drawer.open")&&h.add(a),n.querySelectorAll("details.pm-trace-tool-group[open]").forEach((o,y)=>{b.add(`${a}:${o.getAttribute("data-pm-trace-group")||y}`)})}),e.innerHTML=i.map(f).join(""),e.querySelectorAll(".pm-agent-chat-msg").forEach((n,a)=>{let v=n.querySelector(".pm-trace-drawer"),o=n.querySelector('[data-expandable="trace"]');v&&h.has(a)&&(v.classList.add("open"),o?.classList.add("expanded")),n.querySelectorAll("details.pm-trace-tool-group").forEach((y,c)=>{let w=`${a}:${y.getAttribute("data-pm-trace-group")||c}`;b.has(w)&&!y.closest('.pm-trace-drawer[data-trace-completed="1"]')&&y.setAttribute("open","")})}),We(e)}var pe={};function nt(e,i){let f=String(e||"pm-agent-chat");return`
    ...
    </form>`}function it(e,i,{placeholder:f,isBusy:h,onSubmit:b,onAbort:n,draftKey:a="",voiceTarget:v=null,onVoiceSubmit:o=null,openCameraCapture:y=null}){let c=String(i||"pm-agent-chat"),w=e.querySelector(`#${c}-form`),r=e.querySelector(`#${c}-input`),$=e.querySelector(`#${c}-send-btn`),S=e.querySelector(`#${c}-attach-btn`),d=e.querySelector(`#${c}-mic-btn`),A=e.querySelector(`#${c}-voice-shell`),D=e.querySelector(`#${c}-voice-close`),W=e.querySelector(`#${c}-voice-camera`),L=e.querySelector(`#${c}-voice-inline`),G=e.querySelector(`#${c}-file-input`),K=e.querySelector(`#${c}-attach-tray`),N=String(a||"").trim(),_=null;N&&(pe[N]||(pe[N]={text:"",pending:[]}),_=pe[N],Array.isArray(_.pending)||(_.pending=[]));let P=_?_.pending:[],k=!1,q=null,te=null,H=null,z=0;r&&_?.text&&(r.value=_.text);let Q=()=>{if(!r)return;let l=Math.max(320,Math.round(window.visualViewport?.height||window.innerHeight||640)),m=Math.max(96,Math.min(280,Math.floor(l*.5)-86)),E=Number(r.dataset.maxHeight||m);r.style.height="auto",r.style.height=`${Math.min(E,Math.max(30,r.scrollHeight||30))}px`,r.style.overflowY=r.scrollHeight>E?"auto":"hidden"},U=()=>!!(String(r?.value||"").trim()||P.length),V=()=>v&&typeof o=="function"&&A&&L,Y=()=>{w&&(w.classList.toggle("is-focused",document.activeElement===r),w.classList.toggle("has-text",!!String(r?.value||"").trim()),w.classList.toggle("has-attachments",P.length>0))},J=()=>{if(!(!A||!L)){A.hidden=!0;try{L._pmCleanup?.()}catch{}L.innerHTML="",Ue(),j?.target?.kind==="subagent"&&j.target.agentId===v?.agentId&&(j.target=null,j.subagentSubmit=null)}},ae=async({autoStart:l=!0}={})=>{if(!V()){I("Voice mode is not available for this composer.","error");return}let m={kind:"subagent",agentId:String(v.agentId||"").trim(),label:String(v.label||v.name||"Subagent").trim(),voice:v.voice&&typeof v.voice=="object"?v.voice:null};j.target=m,j.targetSessionId=`subagent_chat_${m.agentId}`,j.targetSessionLabel=m.label,j.targetSessionChannel="subagent",j.targetSessionForced=!0,j.subagentSubmit=async E=>o({text:String(E||"").trim(),files:[]}),Oe(m.voice),A.hidden=!1,L.innerHTML="",await Qe(L,{inline:!0,inlineChatSessionId:j.targetSessionId,inlineChatSessionLabel:m.label,autoStart:l,openCameraCapture:y,cameraButton:W})},O=()=>{K&&(K.hidden=P.length===0,K.innerHTML=Be(P,!0),K.querySelectorAll("[data-remove-attachment]").forEach(l=>{l.addEventListener("click",()=>{let m=Number(l.getAttribute("data-remove-attachment"));Number.isFinite(m)&&P.splice(m,1),O(),R()})}),Y())},R=()=>{let l=!!h?.(),m=l&&!U();w&&(w.classList.toggle("is-busy",l),w.setAttribute("aria-busy",l?"true":"false"),w.dataset.composerState=l?m?"stopping":"busy":"idle"),r&&(r.placeholder=l?"Queue a message...":f),$&&($.disabled=!1,$.classList.toggle("is-abort",m),$.classList.toggle("is-voice",!l&&!U()&&V()),$.title=m?"Stop":!l&&!U()&&V()?"Start voice mode":l?"Queue message":"Send",$.setAttribute("aria-label",m?"Stop":!l&&!U()&&V()?"Start voice mode":l?"Queue message":"Send"),$.setAttribute("aria-busy",l?"true":"false"),$.innerHTML=m?'<svg viewBox="0 0 24 24" width="20" height="20" fill="none" stroke="currentColor" stroke-width="2"><rect x="7" y="7" width="10" height="10" rx="2"/></svg>':!l&&!U()&&V()?`<img class="pm-send-voice-icon" src="${Ce}" alt="" aria-hidden="true" />`:g.send),Y()},t=()=>{let l=String(r?.value||"").trim(),m=P.splice(0,P.length);if(r&&(r.value="",_&&(_.text=""),Q()),k||q||H){k=!1,z+=1,H&&clearTimeout(H),H=null;let E=q;q=null;try{E?.abort?.()}catch{try{E?.stop?.()}catch{}}d?.classList.remove("listening")}return O(),R(),{text:l,files:m}},s=({refocus:l=!0}={})=>{k=!1,z+=1,H&&clearTimeout(H),H=null;let m=q;q=null;try{m?.stop?.()}catch{try{m?.abort?.()}catch{}}d?.classList.remove("listening"),l&&r?.focus(),Q(),R()},p=(l,m=140)=>{H&&clearTimeout(H),H=null,!(!k||q)&&(H=setTimeout(()=>{H=null,x(l)},m))},x=l=>{if(!(!k||q||!r))try{let m=new l,E=z,B=String(r.value||"").trimEnd();q=m,m.lang=navigator.language||"en-US",m.interimResults=!0,m.continuous=!0,m.onstart=()=>{E===z&&d?.classList.add("listening")},m.onresult=F=>{if(E!==z)return;let C="",me="";for(let se=0;se<F.results.length;se+=1){let ve=String(F.results[se]?.[0]?.transcript||"");F.results[se].isFinal?C+=ve:me+=ve}let ue=`${C}${me}`.trim();r.value=`${B}${B&&ue?" ":""}${ue}`,_&&(_.text=r.value||""),Q(),R()},m.onerror=F=>{if(E!==z)return;let C=String(F?.error||"unknown");["not-allowed","service-not-allowed","audio-capture"].includes(C)?(k=!1,I(C==="audio-capture"?"The microphone is not available.":"Microphone permission was denied.","error")):["no-speech","aborted"].includes(C)||console.warn("[mobile agent chat] dictation cycle error:",C)},m.onend=()=>{if(E===z){if(q===m&&(q=null),Q(),R(),!k){d?.classList.remove("listening");return}p(l)}},m.start()}catch(m){q=null,k=!1,d?.classList.remove("listening"),I(m?.message||"Could not start dictation.","error")}};r?.addEventListener("input",()=>{_&&(_.text=r.value||""),Q(),R()}),r?.addEventListener("focus",Y),r?.addEventListener("blur",()=>setTimeout(Y,0)),r?.addEventListener("keydown",l=>{l.key==="Enter"&&!l.shiftKey&&(l.preventDefault(),w?.requestSubmit?.())}),r?.addEventListener("paste",async l=>{let m=Array.from(l.clipboardData?.files||[]);if(!m.length)return;String(l.clipboardData?.getData?.("text/plain")||"").trim()||l.preventDefault();let E=await Promise.all(m.slice(0,8).map(de));P.push(...E.filter(Boolean)),O(),R()}),S?.addEventListener("click",()=>G?.click()),d?.addEventListener("click",()=>{let l=window.SpeechRecognition||window.webkitSpeechRecognition;if(!l){I("Speech dictation is not available in this browser.","error");return}if(k){s();return}te=l,k=!0,z+=1,d.classList.add("listening"),I("Listening until you tap the mic again.","info"),x(l)}),D?.addEventListener("click",J),W?.addEventListener("click",()=>{typeof y=="function"?y():G?.click()}),G?.addEventListener("change",async()=>{let l=Array.from(G.files||[]).slice(0,8);if(G.value="",!l.length)return;let m=await Promise.all(l.map(de));P.push(...m.filter(Boolean)),O(),R()}),w?.addEventListener("submit",async l=>{if(l.preventDefault(),h?.()&&!U()){n?.(),R();return}let m=t();if(!m.text&&!m.files.length){await ae({autoStart:!0});return}await b?.(m),R()});let M=e._pmCleanup;return e._pmCleanup=()=>{s({refocus:!1}),J(),M?.()},requestAnimationFrame(()=>{O(),Q(),R()}),{input:r,update:R,consume:t,pending:P}}async function ot(e,i){e.innerHTML=`
  ...
  `;let f=e.querySelector("#pm-team-chat-list"),h=e.querySelector("#pm-team-chat-queue"),b=e.querySelector("#pm-team-chat-goal");Ne(f,()=>{}),Re(b,He.activeSessionId,{fallbackToLast:!0});let n=[],a=null,v=null,o=0,y="",c=!1,w=!1,r=[],$=[],S=null;function d(t={}){let s=t?.body&&typeof t.body=="object"?t.body:{},p=t?.metadata&&typeof t.metadata=="object"?t.metadata:{},x=String(t?.from||t?.role||"").toLowerCase(),M=x==="user"||x==="you"||x==="human",l=String(t?.content||t?.message||t?.text||s.text||"");return{...t,role:M?"user":"agent",from:M?"user":x||"manager",fromLabel:t?.fromLabel||t?.fromName||s.sender||(M?"You":"Manager"),content:l,body:{...s,text:l},createdAt:t?.createdAt||t?.timestamp||t?.ts||Date.now(),processEntries:Array.isArray(t?.processEntries)?t.processEntries:Array.isArray(p.processEntries)?p.processEntries:[]}}function A(t){let s=d(t);try{return Ve(s,{sender:s.fromLabel,live:t===a,keepLiveTraceVisible:t===a})}catch(p){console.warn("[mobile team chat] rich message render failed:",p);let x=s.role==="user";return`<div class="pm-msg ${x?"from-user":"from-ai"} pm-agent-chat-msg">
        <div class="pm-bubble">
          ${x?"":`<span class="pm-sender">${u(s.fromLabel)}</span>`}
          <div class="markdown-body">${Pe(s.content)}</div>
        </div>
      </div>`}}let D=()=>!!(v||a?.streaming||c),W=(t={})=>{let s=ee(t),p=String(s.sessionId||s.sourceSessionId||"").trim();return!!s.id&&(p.startsWith(`team_dm_manager_${i}___`)||p.startsWith(`team_dm_member_${i}___`)||p===`team_chat_${i}`||String(s.teamId||s.toolArgs?.teamId||"").trim()===String(i))},L=(t={})=>{if(!W(t))return!1;let s=ee(t),p=$.findIndex(M=>String(M?.approvalRequest?.id||"")===s.id),x={role:"agent",from:"manager",fromLabel:"Manager",content:"",createdAt:Date.now(),approvalRequest:s};return p>=0?$[p]={...$[p],approvalRequest:{...$[p].approvalRequest||{},...s}}:$.push(x),$=$.slice(-8),!0},G=(t,s,p={})=>{let x=String(t||"").trim();if(!x)return!1;let M=$.findIndex(l=>String(l?.approvalRequest?.id||"")===x);return M<0?!1:($[M].approvalRequest=ee({...$[M].approvalRequest||{},...p.approval||p,id:x,status:s}),!0)},K=async()=>{let t=await fe("pending").catch(()=>[]);(Array.isArray(t)?t:[]).forEach(L)};function N(){h&&(h.hidden=r.length===0,h.innerHTML=r.length?`<div class="pm-mobile-queued-list">${r.map((t,s)=>`
           ...
           </div>`).join("")}</div>`:"",De(),h.querySelectorAll("[data-team-queue-edit]").forEach(t=>X(t,()=>{})),h.querySelectorAll("[data-team-queue-menu]").forEach(t=>X(t,()=>{let s=Number(t.getAttribute("data-team-queue-menu"));if(!Number.isInteger(s))return;let p=h.querySelector(`[data-team-queue-menu-popover="${s}"]`);if(!p)return;let x=!!p.hidden;ne(h),p.hidden=!x})),h.querySelectorAll("[data-team-queue-steer]").forEach(t=>X(t,()=>{let s=Number(t.getAttribute("data-team-queue-steer"));if(Number.isFinite(s)&&s>=0&&s<r.length){let[p]=r.splice(s,1);p&&r.unshift(p)}ne(h),N(),_()})),h.querySelectorAll("[data-team-queue-remove]").forEach(t=>X(t,()=>{let s=Number(t.getAttribute("data-team-queue-remove"));Number.isFinite(s)&&r.splice(s,1),ne(h),N()})))}function _(){if(D()||!r.length){S?.update?.();return}let t=r.shift();N(),R(t).catch(s=>I(s?.message||"Send failed","error"))}function P(t){let s=a&&!a._done?a:null;n=Array.isArray(t)?t.slice():[],s&&(n.some(x=>String(x.content||x.message||x.text||"").trim()&&String(x.content||x.message||x.text||"").trim()===String(s.content||"").trim())||n.push(s))}function k(){let t=$.filter(p=>String(p?.approvalRequest?.status||"pending")==="pending"),s=[...n,...t];if(!s.length){f.innerHTML='<div style="text-align:center;color:var(--pm-muted);padding:24px 8px;font-size:13px;">No messages yet. Send the first one.</div>';return}rt(f,s,A),f.querySelectorAll("[data-pm-approval-action][data-pm-approval-id]").forEach(je),Ge(f),f.scrollTop=f.scrollHeight}try{P(await re(i,80)),await K(),k()}catch(t){f.innerHTML=`<div style="color:var(--pm-red);padding:16px;">${u(t.message||"Failed to load chat")}</div>`}async function q({forceHistory:t=!1}={}){try{let s=await ke(i,y?o:0);s.stream?.streamId&&s.stream.streamId!==y&&(y=s.stream.streamId,o=0),s.stream?.streamId&&!a&&s.active&&(a={role:"manager",from:"manager",fromLabel:"Manager",content:"",_progress:"Reconnecting...",createdAt:Date.now(),workStartedAt:Date.now(),streaming:!0,processEntries:[]},n.push(a));for(let p of s.events||[])p.streamId&&(y=p.streamId),o=Math.max(o,Number(p.seq||0)),a||(a={role:"manager",from:"manager",fromLabel:"Manager",content:"",_progress:"Reconnecting...",createdAt:Date.now(),workStartedAt:Date.now(),streaming:!0,processEntries:[]},n.push(a)),ie(a,st(p),"Manager");(t||!s.active||a?._done)&&(P(await re(i,80)),await K(),s.active||(a=null)),k()}catch{}}let te=()=>q({forceHistory:!0}),H=()=>{document.hidden||q({forceHistory:!0})},z=async(t={})=>{if(String(t.teamId||"")===String(i))try{P(await re(i,80)),a=null,k()}catch{}},Q=(t={})=>{String(t.teamId||"")===String(i)&&(c||(t.streamId&&t.streamId!==y&&(y=t.streamId,o=0),o=Math.max(o,Number(t.seq||0)),a||(a={role:"manager",from:"manager",fromLabel:"Manager",content:"",_progress:"Thinking...",createdAt:Date.now(),workStartedAt:Date.now(),streaming:!0,processEntries:[]},n.push(a)),ie(a,{type:String(t.event||""),...t.data||{}},"Manager"),k()))},U=async(t={})=>{let s=t.approval?ee(t.approval,t):await ze(t);L(s)&&k()},V=t=>(s={})=>{let p=t==="approval_approved"?"approved":t==="approval_denied"?"rejected":t==="approval_expired"?"expired":"failed";G(s.approvalId||s.id||s.approval?.id,p,s)&&k()},Y=V("approval_approved"),J=V("approval_denied"),ae=V("approval_expired"),O=V("approval_failed");T?.on?.("ws:open",te),T?.on?.("team_chat_message",z),T?.on?.("team_chat_stream_event",Q),T?.on?.("approval_created",U),T?.on?.("approval_approved",Y),T?.on?.("approval_denied",J),T?.on?.("approval_expired",ae),T?.on?.("approval_failed",O),document.addEventListener("visibilitychange",H),e._pmCleanup=()=>{if(!w){w=!0;try{v?.abort?.()}catch{}T?.off?.("ws:open",te),T?.off?.("team_chat_message",z),T?.off?.("team_chat_stream_event",Q),T?.off?.("approval_created",U),T?.off?.("approval_approved",Y),T?.off?.("approval_denied",J),T?.off?.("approval_expired",ae),T?.off?.("approval_failed",O),document.removeEventListener("visibilitychange",H)}},q();async function R(t){let s=String(t?.text||"").trim(),p=Array.isArray(t?.files)?t.files:[],x=String(t?.source||"").trim(),M=s||(p.length?"Please review the attached file(s).":"");if(!M&&!p.length)return;if(D()){r.push({text:s,files:p,source:x,speak:t?.speak===!0,voice:t?.voice===!0}),N(),S?.update?.();return}let l=M,m=p;if(p.length){let B=await Fe(p);l=`${M}${Ie(B)}`,m=B.map((F,C)=>({...p[C]||{},name:F.name||p[C]?.name||"attachment",kind:F.isImage?"image":F.isVideo?"video":p[C]?.kind||"file",workspacePath:F.workspacePath||p[C]?.workspacePath,path:F.workspacePath||p[C]?.path,dataUrl:p[C]?.dataUrl,mimeType:p[C]?.mimeType,sizeLabel:p[C]?.sizeLabel}))}let E={role:"user",from:"user",content:M,body:{text:M,attachments:m},attachmentPreviews:m,createdAt:Date.now()};a={role:"manager",from:"manager",fromLabel:"Manager",content:"",_progress:"Manager is thinking...",createdAt:Date.now(),workStartedAt:Date.now(),streaming:!0,processEntries:[]},n.push(E,a),k(),c=!0,S?.update?.(),v=Le(i,{message:l},{onEvent:B=>{ie(a,B,"Manager"),k()},onError:B=>{B?.name!=="AbortError"&&(a.content=a.content||`Error: ${B?.message||"stream failed"}`,a._progress="",a.streaming=!1,a.workEndedAt=Date.now(),c=!1,v=null,S?.update?.(),k(),I(B?.message||"Send failed","error"))},onDone:async()=>{a&&(a._progress="",a.streaming=!1,a.workEndedAt=a.workEndedAt||Date.now(),a.workDurationMs=Math.max(0,a.workEndedAt-Number(a.workStartedAt||a.createdAt||a.workEndedAt))),c=!1,v=null,S?.update?.(),await q({forceHistory:!0}),_()}})}S=it(e,"pm-team-chat",{placeholder:"Message the team manager...",draftKey:"team:manager",isBusy:D,onAbort:()=>{try{v?.abort?.()}catch{}a&&(a._progress="Stopping...",a.streaming=!1),v=null,c=!1,k()},onSubmit:R}),N()}function lt(e){let i=String(e||"").toLowerCase();return/\.(md|markdown|txt)$/.test(i)?"\u{1F4DD}":/\.(js|ts|tsx|jsx|mjs|cjs)$/.test(i)?"\u{1F4DC}":/\.(json|yaml|yml|toml)$/.test(i)?"\u{1F527}":/\.(png|jpg|jpeg|gif|svg|webp)$/.test(i)?"\u{1F5BC}\uFE0F":/\.(mp4|mov|webm|mkv)$/.test(i)?"\u{1F3AC}":/\.(mp3|wav|ogg|flac)$/.test(i)?"\u{1F3B5}":/\.(html|htm)$/.test(i)?"\u{1F310}":/\.(pdf)$/.test(i)?"\u{1F4C4}":"\u{1F4C3}"}function ct(e){return!e||e<1024?`${e||0} B`:e<1024*1024?`${(e/1024).toFixed(1)} KB`:`${(e/(1024*1024)).toFixed(2)} MB`}async function dt(e,i){e.innerHTML='<div class="pm-card" style="text-align:center;padding:24px;color:var(--pm-muted);">Loading workspace\u2026</div>';let f;try{f=await Me(i)}catch(a){e.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${g.doc}</div><h2>Couldn\u2019t load workspace</h2><p>${u(a.message||"")}</p></div>`;return}let h=f.files||[];if(!h.length){e.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${g.doc}</div><h2>Workspace is empty</h2><p>Files written by team subagents will appear here.</p></div>`;return}e.innerHTML=`
...
\u2026(truncated)`:""}</pre>
        `,n.querySelector("#pm-ws-close").addEventListener("click",()=>{n.style.display="none",n.innerHTML=""}),n.scrollIntoView({behavior:"smooth",block:"nearest"})}catch(o){n.innerHTML=`<div class="pm-card-body" style="color:var(--pm-red);">${u(o.message||"Failed to load file")}</div>`}})})}async function pt(e,i,f){e.innerHTML='<div class="pm-card" style="text-align:center;padding:24px;color:var(--pm-muted);">Loading memory\u2026</div>';let h;try{h=await _e()}catch(c){e.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${g.brain}</div><h2>Couldn\u2019t load memory</h2><p>${u(c.message||"")}</p></div>`;return}let b=Array.isArray(h?.nodes)?h.nodes.slice():[],n=String(i).toLowerCase(),a=String(f?.name||"").toLowerCase(),v=c=>{let w=String(c.sourcePath||"").toLowerCase(),r=String(c.projectId||"").toLowerCase();return r&&r.includes(n)||w&&(w.includes(n)||a&&w.includes(a))},o=b.filter(v),y=(o.length?o:b).sort((c,w)=>String(w.timestamp||"").localeCompare(String(c.timestamp||""))).slice(0,30);if(!y.length){e.innerHTML=`<div class="pm-empty"><div class="pm-empty-icon">${g.brain}</div><h2>No memory yet</h2><p>As the team works, reflections and memory entries land here.</p></div>`;return}e.innerHTML=`
    ...
    ${y.map(c=>`
      <article class="pm-card" style="padding:12px 14px;">
        <div style="display:flex;align-items:center;gap:8px;margin-bottom:4px;">
          <strong style="flex:1;font-size:13px;line-height:1.3;">${u(c.label||"Memory")}</strong>
          <span class="pm-pill gray" style="font-family:ui-monospace,monospace;">${u(c.sourceTypeLabel||c.sourceType||"memory")}</span>
        </div>
        ${c.summary?`<div class="pm-card-body" style="margin-bottom:4px;">${u(String(c.summary).slice(0,240))}${String(c.summary).length>240?"\u2026":""}</div>`:""}
        <div style="display:flex;justify-content:space-between;font-size:11px;color:var(--pm-muted);">
          <span style="overflow:hidden;text-overflow:ellipsis;white-space:nowrap;max-width:70%;font-family:ui-monospace,monospace;">${u(c.sourcePath||"")}</span>
          <span>${c.timestamp?Z(new Date(c.timestamp).getTime()):""}</span>
        </div>
      </article>
    `).join("")}
