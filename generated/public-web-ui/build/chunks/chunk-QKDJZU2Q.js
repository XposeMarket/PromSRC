function g(r){return r?String(r).replace(/&/g,"&amp;").replace(/</g,"&lt;").replace(/>/g,"&gt;").replace(/"/g,"&quot;"):""}var oe=g;function P(r){let e=Date.now()-r;return e<6e4?"just now":e<36e5?`${Math.floor(e/6e4)}m ago`:e<864e5?`${Math.floor(e/36e5)}h ago`:`${Math.floor(e/864e5)}d ago`}function D(r,e=0){let n=Number(r);return Number.isFinite(n)?`${n.toFixed(e)}%`:"--%"}function q(r,e){let n=Number(r),t=Number(e);return!Number.isFinite(n)||!Number.isFinite(t)||t<=0?"-- / -- GB":`${n.toFixed(1)} / ${t.toFixed(1)} GB`}function A(r){let e=Number(r);return Number.isFinite(e)?`${Math.max(0,Math.min(100,e))}%`:"0%"}function H(r,e){let n=document.getElementById(r);n&&(n.textContent=String(e||""))}function se(r){let e=String(r||"").trim(),n=g(e);return`<span class="t-think-sizer" aria-hidden="true">${n}</span><span class="t-think-text" data-text="${n}">${n}</span>`}function ie(r,e){let n=String(e||"").trim(),t=r?.querySelector?.(".t-think-text");if(!t||!n)return!1;let o=String(t.textContent||"").trim();if(!o||o===n)return!1;r.querySelectorAll?.(".t-think-text").forEach(i=>{i!==t&&i.remove()});let s=t.cloneNode(!0);s.classList.remove("is-enter-start"),s.classList.add("is-exit"),s.textContent=n,s.setAttribute("data-text",n),t.classList.remove("is-exit"),t.classList.add("is-enter-start");let a=r.querySelector?.(".t-think-sizer");a&&n.length>String(a.textContent||"").length&&(a.textContent=n),r.appendChild(s),t.offsetWidth;let c=()=>{t.isConnected!==!1&&t.classList.remove("is-enter-start")};return typeof requestAnimationFrame=="function"?requestAnimationFrame(c):typeof setTimeout=="function"&&setTimeout(c,0),typeof setTimeout=="function"&&setTimeout(()=>{s.isConnected!==!1&&s.remove(),t.isConnected!==!1&&t.classList.remove("is-enter-start")},420),!0}function O(r,e){let n=document.getElementById(r);n&&(n.style.width=A(e))}function k(r,e,n="info",t=5e3,o={}){let s=typeof o?.key=="string"?o.key.trim():"";if(s)for(let u of document.querySelectorAll(".__sc-toast"))u.dataset.scToastKey===s&&u.remove();let a=n==="warn"?"warning":["info","success","error","warning"].includes(n)?n:"info",c={info:"\u2139\uFE0F",success:"\u2713",error:"\u26A0\uFE0F",warning:"\u26A0\uFE0F"},i=document.createElement("div"),l=24+[...document.querySelectorAll(".__sc-toast")].reduce((u,f)=>u+f.offsetHeight+8,0);if(i.className=`__sc-toast __sc-toast--${a}`,s&&(i.dataset.scToastKey=s),i.style.cssText=`position:fixed;bottom:${l}px;right:24px;z-index:99999;`,i.innerHTML=`
  ...
  `,!document.getElementById("__sc-toast-style")){let u=document.createElement("style");u.id="__sc-toast-style",u.textContent="@keyframes scToastIn{from{opacity:0;transform:translateY(12px)}to{opacity:1;transform:translateY(0)}}",document.head.appendChild(u)}document.body.appendChild(i);let d=Math.max(0,Math.min(5e3,Number.isFinite(Number(t))?Number(t):5e3));setTimeout(()=>{i.style.transition="opacity 0.3s",i.style.opacity="0",setTimeout(()=>i.remove(),300)},d),i.querySelector(".__sc-toast-close")?.addEventListener("click",()=>i.remove())}function j(r,e){k(r,e,"info")}function Y(r,e,n,t={}){let{title:o="Confirm",confirmText:s="Confirm",cancelText:a="Cancel",danger:c=!1,details:i=""}=t,m=document.createElement("div");m.style.cssText="position:fixed;inset:0;z-index:99998;background:rgba(0,0,0,0.35);display:flex;align-items:center;justify-content:center;animation:scToastIn 0.15s ease";let l=document.createElement("div");l.style.cssText="background:var(--panel);border:1.5px solid var(--line);border-radius:14px;padding:24px 24px 18px;max-width:560px;width:92%;box-shadow:0 8px 40px rgba(0,0,0,0.18);font-family:var(--font)",l.innerHTML=`
    ...
    ${i?`<pre style="margin:0 0 18px;padding:12px 14px;border-radius:10px;border:1px solid var(--line);background:var(--panel-2);font-size:11px;line-height:1.65;color:var(--text);white-space:pre-wrap;word-break:break-word;font-family:'Cascadia Code','Fira Code','Consolas',monospace">${g(i)}</pre>`:""}
    <div style="display:flex;gap:8px;justify-content:flex-end">
      ...
      <button id="__sc-confirm-ok" style="border:none;background:${c?"#dc2626":"var(--brand)"};color:#fff;border-radius:8px;padding:7px 16px;font-size:12px;font-weight:700;cursor:pointer">${g(s)}</button>
    </div>
  `,m.appendChild(l),document.body.appendChild(m);let d=()=>m.remove();l.querySelector("#__sc-confirm-cancel").onclick=()=>{d(),n&&n()},l.querySelector("#__sc-confirm-ok").onclick=()=>{d(),e&&e()},m.addEventListener("click",u=>{u.target===m&&(d(),n&&n())})}var x=[];function U(r,e="log"){let n=new Date().toLocaleTimeString();x.push({text:`[${n}] ${String(r??"")}`,type:String(e||"log").replace(/[^a-z0-9_-]/gi,"")||"log"}),x.length>100&&x.shift();let t=document.getElementById("log-panel");t&&(t.replaceChildren(...x.map(o=>{let s=document.createElement("div");return s.className=`log-line ${o.type}`,s.textContent=o.text,s})),t.scrollTop=t.scrollHeight)}var p=Object.freeze({bg:"transparent",bgSoft:"transparent",surface:"transparent",surfaceSecondary:"transparent",border:"currentColor",borderStrong:"currentColor",text:"currentColor",muted:"currentColor",accent:"currentColor",accentStrong:"currentColor",success:"currentColor",warning:"currentColor",danger:"currentColor"});function B(r,e){return String(r||"").replace(/[<>{};\r\n]/g,"").trim()||e}function L(){let r=document.documentElement,e=typeof getComputedStyle=="function"?getComputedStyle(r):null,n=(o,s)=>{for(let a of o){let c=e?.getPropertyValue(a)?.trim();if(c)return B(c,s)}return s},t={isDark:r.getAttribute("data-theme")==="dark",bg:n(["--bg","--pm-chat-page-bg"],p.bg),bgSoft:n(["--bg-soft"],p.bgSoft),surface:n(["--panel","--composer-panel"],p.surface),surfaceSecondary:n(["--panel-2","--composer-bg"],p.surfaceSecondary),border:n(["--line","--composer-border"],p.border),borderStrong:n(["--line-strong"],p.borderStrong),text:n(["--text","--fg","--composer-text"],p.text),muted:n(["--muted","--composer-muted"],p.muted),accent:n(["--brand","--pm-custom-accent"],p.accent),accentStrong:n(["--brand-2"],p.accentStrong),success:n(["--ok"],p.success),warning:n(["--warn"],p.warning),danger:n(["--err"],p.danger)};return t.series=[t.accent,t.accentStrong,t.success,t.warning,t.danger,t.muted],t.vars={"--prom-bg":t.bg,"--prom-bg-soft":t.bgSoft,"--prom-surface":t.surface,"--prom-surface-secondary":t.surfaceSecondary,"--prom-border":t.border,"--prom-border-strong":t.borderStrong,"--prom-text":t.text,"--prom-muted":t.muted,"--prom-accent":t.accent,"--prom-accent-strong":t.accentStrong,"--prom-success":t.success,"--prom-warning":t.warning,"--prom-danger":t.danger,"--prom-series-1":t.series[0],"--prom-series-2":t.series[1],"--prom-series-3":t.series[2],"--prom-series-4":t.series[3],"--prom-series-5":t.series[4],"--prom-series-6":t.series[5],"--bg":t.bg,"--bg-soft":t.bgSoft,"--panel":t.surface,"--panel-2":t.surfaceSecondary,"--line":t.border,"--line-strong":t.borderStrong,"--text":t.text,"--fg":t.text,"--muted":t.muted,"--brand":t.accent,"--brand-2":t.accentStrong,"--ok":t.success,"--warn":t.warning,"--err":t.danger},t}function W(r){if(r&&typeof r=="object"&&r.vars)return r;let e={isDark:typeof r=="boolean"?r:!!r?.isDark,...p};return e.series=[e.accent,e.accentStrong,e.success,e.warning,e.danger,e.muted],e.vars=Object.fromEntries([["--prom-bg",e.bg],["--prom-bg-soft",e.bgSoft],["--prom-surface",e.surface],["--prom-surface-secondary",e.surfaceSecondary],["--prom-border",e.border],["--prom-border-strong",e.borderStrong],["--prom-text",e.text],["--prom-muted",e.muted],["--prom-accent",e.accent],["--prom-accent-strong",e.accentStrong],["--prom-success",e.success],["--prom-warning",e.warning],["--prom-danger",e.danger],...e.series.map((n,t)=>[`--prom-series-${t+1}`,n]),["--bg",e.bg],["--bg-soft",e.bgSoft],["--panel",e.surface],["--panel-2",e.surfaceSecondary],["--line",e.border],["--line-strong",e.borderStrong],["--text",e.text],["--fg",e.text],["--muted",e.muted],["--brand",e.accent],["--brand-2",e.accentStrong],["--ok",e.success],["--warn",e.warning],["--err",e.danger]]),e}function X(r){let e=r?.vars&&typeof r.vars=="object"?r.vars:{};return Object.entries(e).map(([n,t])=>`${n}:${B(t,"transparent")}`).join(";")}function z(r,e,n){let t=W(n),o=S({background:"transparent",primaryColor:t.surface,primaryTextColor:t.text,primaryBorderColor:t.borderStrong,lineColor:t.muted,secondaryColor:t.surfaceSecondary,secondaryTextColor:t.text,secondaryBorderColor:t.border,tertiaryColor:t.bgSoft,tertiaryTextColor:t.text,tertiaryBorderColor:t.border,textColor:t.text,mainBkg:t.surface,nodeBorder:t.borderStrong,clusterBkg:t.surfaceSecondary,clusterBorder:t.border,edgeLabelBackground:"transparent"}),s=S({text:t.text,muted:t.muted,border:t.border,series:t.series}),a=`:root{${X(t)}color-scheme:${t.isDark?"dark":"light"}}*{margin:0;padding:0;box-sizing:border-box}html,body{background:transparent!important;color:var(--prom-text);color-scheme:${t.isDark?"dark":"light"};max-width:100%;overflow-x:hidden}body{min-height:0;font-family:system-ui,-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif}`;return r==="chart"?`<!DOCTYPE html><html><head><meta charset="UTF-8">
...
</head><body>${e}</body></html>`}function y(r){return String(r||"").replace(/&/g,"&amp;").replace(/"/g,"&quot;")}function S(r){return JSON.stringify(r??null).replace(/</g,"\\u003c")}function G(r,e={}){let n=String(e.visualId||""),t=e.state&&typeof e.state=="object"?e.state:{},o=`<script>(function(){
var visualId=${S(n)},last=0,state=${S(t)}||{};
...
if('ResizeObserver'in window){var ro=new ResizeObserver(send);if(document.documentElement)ro.observe(document.documentElement);if(document.body)ro.observe(document.body)}addEventListener('load',function(){restoreControls();send();post('prometheus:visual-ready')});setTimeout(send,50);setTimeout(send,250);setTimeout(send,1000)})();<\/script>`,s=String(r||"");return/<head\b[^>]*>/i.test(s)?s.replace(/<head\b[^>]*>/i,a=>`${a}${o}`):`${o}${s}`}function K(){if(window.__PROM_VISUAL_MESSAGE_BRIDGE_INSTALLED__)return;window.__PROM_VISUAL_MESSAGE_BRIDGE_INSTALLED__=!0;let r=()=>{let e=L();document.querySelectorAll('iframe[data-prom-visual="true"]').forEach(n=>{let t=String(n.getAttribute("data-visual-id")||"");if(!(!t||!n.contentWindow))try{n.contentWindow.postMessage({type:"prometheus:visual-theme",visualId:t,theme:e},"*")}catch{}})};document.addEventListener("prom-theme-change",()=>setTimeout(r,0)),document.addEventListener("prom-appearance-change",()=>setTimeout(r,0)),window.addEventListener("message",e=>{let n=e?.data;if(!n||!String(n.type||"").startsWith("prometheus:visual-"))return;let t=Array.from(document.querySelectorAll('iframe[data-prom-visual="true"]')).find(s=>s.contentWindow===e.source);if(!t)return;let o=String(t.getAttribute("data-visual-id")||"");if(String(n.visualId||"")===o){if(n.type==="prometheus:visual-resize"){let s=Number(n.height);if(!Number.isFinite(s))return;let a=Math.min(1e4,Math.max(120,Math.ceil(s))),c=Math.ceil(t.getBoundingClientRect().height||0);if(Math.abs(c-a)<=1)return;t.style.height=`${a}px`,t.style.minHeight=`${a}px`;return}if(n.type==="prometheus:visual-state"&&n.state&&typeof n.state=="object"){window.dispatchEvent(new CustomEvent("prometheus:visual-state-change",{detail:{visualId:o,state:n.state}}));return}n.type==="prometheus:visual-followup"&&n.prompt&&window.dispatchEvent(new CustomEvent("prometheus:visual-followup",{detail:{visualId:o,prompt:String(n.prompt),title:String(n.title||"")}}))}})}function C(r){if(!r?.getAttribute)return"";let e=r.closest?.(".visual-block"),n=String(r.getAttribute("data-visual-id")||"").trim();return n?[n,String(r.getAttribute("data-visual-version")||"1"),String(e?.getAttribute("data-vis-lang")||""),String(e?.getAttribute("data-vis-code")||"")].join("\0"):""}function w(r){if(!r?.querySelector&&!r?.matches)return"";let e=r.matches?.('iframe[data-prom-visual="true"]')?r:r.querySelector?.('iframe[data-prom-visual="true"]');return C(e)}function J(r,e){return!r||!e||r.nodeType!==e.nodeType?!1:r.nodeType!==1?!0:String(r.tagName||"").toLowerCase()===String(e.tagName||"").toLowerCase()}function Q(r,e){let t=r.matches?.('iframe[data-prom-visual="true"]')?new Set(["srcdoc","style"]):new Set;Array.from(r.attributes||[]).forEach(o=>{t.has(o.name)||e.hasAttribute(o.name)||r.removeAttribute(o.name)}),Array.from(e.attributes||[]).forEach(o=>{t.has(o.name)||r.getAttribute(o.name)!==o.value&&r.setAttribute(o.name,o.value)})}function b(r,e,n,t=null){let o=Array.from(e||[]),s=Array.from(n||[]),a=Math.min(o.length,s.length),c=0;for(let i=0;i<a;i+=1){let m=o[i],l=s[i],d=V(m,l);if(d){c+=d.reused;continue}let u=l.cloneNode(!0);r.replaceChild(u,m)}for(let i=a;i<s.length;i+=1)r.insertBefore(s[i].cloneNode(!0),t);for(let i=a;i<o.length;i+=1)o[i].remove();return c}function Z(r,e){return r.length===e.length&&r.every((n,t)=>n===e[t])}function I(r,e){let n=Array.from(r.childNodes||[]),t=Array.from(e.childNodes||[]),o=n.map(w).filter(Boolean),s=t.map(w).filter(Boolean);if(o.length&&Z(o,s)){let a=0,c=0,i=0;for(let m of s){let l=n.findIndex((f,h)=>h>=a&&w(f)===m),d=t.findIndex((f,h)=>h>=c&&w(f)===m);if(l<0||d<0)return b(r,n,t);i+=b(r,n.slice(a,l),t.slice(c,d),n[l]);let u=V(n[l],t[d]);if(!u)return b(r,n,t);i+=u.reused,a=l+1,c=d+1}return i+=b(r,n.slice(a),t.slice(c)),i}return b(r,n,t)}function V(r,e){return J(r,e)?r.nodeType===3||r.nodeType===8?(r.nodeValue!==e.nodeValue&&(r.nodeValue=e.nodeValue),{reused:0}):r.matches?.('iframe[data-prom-visual="true"]')?C(r)===C(e)?{reused:1}:null:(Q(r,e),{reused:I(r,e)}):null}function _(r,e){return!r?.childNodes||!e?.childNodes?0:I(r,e)}function ee(r,e){if(!r)return 0;let n=String(e||"");if(typeof document>"u"||typeof document.createElement!="function"||typeof r.appendChild!="function")return r.innerHTML=n,0;let t=document.createElement("template");t.innerHTML=n;let o=!!r.querySelector?.('iframe[data-prom-visual="true"]'),s=!!t.content.querySelector?.('iframe[data-prom-visual="true"]');return!o&&!s?(r.innerHTML=n,0):_(r,t.content)}function te(r,e,n=0){let t=`${r}\0${n}\0${e}`,o=2166136261;for(let s=0;s<t.length;s+=1)o^=t.charCodeAt(s),o=Math.imul(o,16777619);return`visual_local_${(o>>>0).toString(36)}`}function $(r,e,n={}){K();let t=n.artifact&&typeof n.artifact=="object"?n.artifact:null,o=String(t?.id||n.visualId||te(r,e,n.ordinal||0)),s=`vis_${o.replace(/[^a-z0-9_-]/gi,"_")}`,a=L(),c=G(z(r,e,a),{visualId:o,state:t?.state||n.state||{}}),i=y(c),m=r.replace(/"/g,""),l=y(e),d=r==="chart"?240:r==="html"?180:220;return`<div class="visual-block visual-block--inline" id="${s}-wrap" data-vis-lang="${m}" data-vis-code="${l}" data-vis-surface="inline">
  <iframe
    id="${s}"
    data-prom-visual="true"
    data-visual-id="${y(o)}"
    data-visual-version="${y(t?.version||1)}"
    srcdoc="${i}"
    ...
    loading="lazy"
  ></iframe>
</div>`}function N(r){let e=String(r||""),n=typeof window<"u"?window.DOMPurify:null;return!n||typeof n.sanitize!="function"?g(e):n.sanitize(e,{USE_PROFILES:{html:!0},FORBID_TAGS:["script","style","iframe","object","embed","form","input","button","textarea","select","option","svg","math","link","meta","base"],FORBID_ATTR:["style","srcdoc","formaction","xlink:href"],ALLOW_DATA_ATTR:!1,ALLOW_ARIA_ATTR:!0,RETURN_TRUSTED_TYPE:!1})}var re=600,ne=2e5,v=new Map;function F(r,e={}){if(!r)return"";let n=String(r);if(n.length<=ne&&!/```(chart|svg|html|mermaid)\n/.test(n)&&!(Array.isArray(e.visualArtifacts)&&e.visualArtifacts.length)){let o=v.get(n);if(o!==void 0)return v.delete(n),v.set(n,o),o;let s=M(n,e);return v.set(n,s),v.size>re&&v.delete(v.keys().next().value),s}return M(n,e)}function M(r,e={}){try{let n=[],t=`PROMVISUAL${Math.random().toString(36).slice(2)}X`,o=/```(chart|svg|html|mermaid)\n([\s\S]*?)```/g,s=0,a=Array.isArray(e.visualArtifacts)?e.visualArtifacts.filter(d=>d?.type==="visual"):[],c=String(r).replace(o,(d,u,f)=>{let h=n.length,T=u.toLowerCase(),R=a.find(E=>Number(E.ordinal)===s&&String(E.renderer||"")===T)||null;return n.push({lang:T,code:f.trim(),partial:!1,artifact:R,ordinal:s}),s+=1,`${t}${h}END`}),i=/```(chart|svg|html|mermaid)\n([\s\S]*)$/,m=c.match(i);if(m){let d=n.length;n.push({lang:m[1].toLowerCase(),code:m[2],partial:!0}),c=c.slice(0,m.index)+`${t}${d}END`}let l=N(marked.parse(c,{breaks:!0,gfm:!0,mangle:!1,headerIds:!1}));if(n.length){let d=new RegExp(`${t}(\\d+)END`,"g");l=l.replace(d,(u,f)=>{let h=n[+f];return h?h.partial?"":$(h.lang,h.code,{artifact:h.artifact,ordinal:h.ordinal}):""}),l=l.replace(/<p>\s*(<div class="visual-block"[\s\S]*?<\/div>)\s*<\/p>/g,"$1")}return l}catch{return g(r)}}window.escHtml=g;window.escapeHtml=g;window.sanitizeHtml=N;window.renderMd=F;window.timeAgo=P;window.fmtPercent=D;window.fmtMemoryGb=q;window.meterWidth=A;window.setText=H;window.setMeter=O;window.showToast=k;window.bgtToast=j;window.showConfirm=Y;window.log=U;window.buildVisualSrcdoc=z;window.buildVisualIframe=$;window.preserveVisualIframes=_;window.setInnerHTMLPreservingVisuals=ee;window.renderMd=F;export{g as a,oe as b,P as c,D as d,q as e,A as f,H as g,se as h,ie as i,O as j,k,j as l,Y as m,U as n,z as o,_ as p,ee as q,$ as r,N as s,F as t};
