  ...
  const chatAiTurn = {
    ...
    const stream = context.streamChat({ message: finalText, sessionId: targetSessionId, callerContext, clientRequestId: workerClientRequestId }, {
      ...
      onThought: (m) => {
        ...
        if (text) {
          context._appendMobileProcess(chatAiTurn, 'think', text);
          context._appendMobileLiveTrace(chatAiTurn, 'think', text);
        }
        context._notifyMobileChatVoiceUpdate(targetSessionId, { reason: 'voice_thought' });
      },
      onToolCall: (evt) => {
    ...
    if (context._isMobileRealtimeAgentMode()) {
      ...
      try {
        if (listenMode === 'always_listening') {
          ...
          context._mobileRealtimeAgentPttPress(sid);
        }
      } catch (err) {
        ...
        context._voiceDebug('realtime-agent-start-failed', { message: err?.message || String(err) });
        context.pmToast(`Realtime agent failed: ${err?.message || err}`, 'error');
        context.__pmVoice.listening = false;
        mic.classList.remove('recording');
        _setReadyVoiceState();
      }
