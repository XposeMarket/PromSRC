...
export function mountStreamChatPanel({ host, loadMessages, streamMessage, placeholder='Message…', emptyLabel='Start a conversation.' }){
  ...
  function paint(){
    if(disposed)return;
    host.innerHTML=`<div class="pm-v2-mini-chat"><div class="pm-chat-thread pm-v2-mini-chat-thread">${rows.length?rows.map((row)=>`<div class="pm-msg ${roleOf(row)==='user'?'from-user':'from-ai'}"><div class="pm-bubble"><div class="markdown-body">${escapeHtml(normalizeText(row)||row.summary||'').replace(/\n/g,'<br>')}</div></div></div>`).join(''):`<div class="pm-v2-empty"><span>${escapeHtml(emptyLabel)}</span></div>`}</div><form class="pm-composer pm-v2-mini-composer"><div class="pm-composer-row"><div class="pm-v2-input-wrap"><textarea rows="1" placeholder="${escapeHtml(placeholder)}"></textarea></div><button class="pm-send" type="submit" aria-label="Send">↑</button></div></form></div>`;
