...
function parseRoute(){const raw=String(location.hash||'').replace(/^#\/?/,'');const parts=raw.split('/');if(parts[0]==='mobile'&&parts[1]==='pair')return{name:'pair',id:decode(parts[2]||'add'),sub:''};if(new URLSearchParams(location.search).has('pair'))return{name:'pair',id:'add',sub:''};return{name:parts[0]||'chat',id:decode(parts[1]||''),sub:decode(parts[2]||'')};}
const LOADERS={
...
export function createMobileV2Router({shell,gateways,features,chatStore}){
  ...
  async function render(){const current=++generation;try{cleanup?.();}catch{}cleanup=()=>{};const route=parseRoute();shell.setPageMode?.(route.name);
    const backRoute = route.name === 'teams' && route.id ? 'teams'
      : route.name === 'subagents' && route.id ? 'subagents'
      : route.name === 'schedule' && route.id ? 'schedule'
      : route.name === 'proposals' && route.id ? 'proposals'
      : route.name === 'settings' && route.id ? 'settings'
      : ['audit', 'memory'].includes(route.name) ? 'more'
      : route.name === 'pair' && route.id && route.id !== 'add' ? 'gateways'
      : 'chat';
    const showBack = (['teams', 'subagents', 'schedule', 'proposals', 'settings'].includes(route.name) && Boolean(route.id))
      || route.name === 'audit'
      || route.name === 'memory'
      || (route.name === 'pair' && Boolean(route.id) && route.id !== 'add');
    shell.renderHeader?.({ leftIcon: showBack ? 'back' : 'menu', backRoute });
    try{
  ...
  }catch(error){if(current!==generation)return;shell.page.innerHTML=`<div class="pm-v2-error-card pm-v2-route-error"><strong>Mobile V2 couldn’t open this page.</strong><span>${escapeHtml(error?.message||error||'Unknown error')}</span><button class="pm-btn ghost" data-v2-home>Open Chat</button></div>`;shell.page.querySelector('[data-v2-home]')?.addEventListener('click',()=>navigate('chat'));}}
