...
async function getSupabaseConfig() {
  ...
  const { ok, data } = await gatewayRequest('/api/account/config');
  if (!ok || !data.supabaseUrl) throw new Error('Could not load auth configuration');
  _sbConfig = data;
  return _sbConfig;
}

// ─── Direct Supabase REST call (from browser — not blocked by gateway firewall) ─
async function sbBrowserFetch(supabaseUrl, supabaseAnonKey, urlPath, opts = {}, accessToken) {
  const headers = {
  ...
  const res = await fetch(`${supabaseUrl}${urlPath}`, { ...opts, headers });
  const body = await res.json().catch(() => ({}));
  if (!res.ok) {
    const msg = body.error_description || body.error || body.message || 'Auth error';
    throw new Error(msg);
  }
  return body;
}
...
export async function login(email, password) {
  ...
  const auth = await sbBrowserFetch(supabaseUrl, supabaseAnonKey, '/auth/v1/token?grant_type=password', {
    method: 'POST',
  ...
  if (!userId || !accessToken) throw new Error('Authentication failed — no token returned');

  // 2. Check admin status
  let isAdmin = false;
  try {
    const adminData = await sbBrowserFetch(supabaseUrl, supabaseAnonKey,
      `/rest/v1/profiles?id=eq.${userId}&select=is_admin&limit=1`, {}, accessToken);
