...
assert.match(mobilePages, /currentIndex:\s*Number\.isFinite/, 'the active question index must survive normalization into the renderer');
...
assert.match(mobilePages, /if \(!isLastQuestion\)/, 'intermediate answers must advance locally before backend submission');
assert.match(mobilePages, /_syncMobileQuestionComposerPopover\(sessionId, \{ \[qid\]: state \}\)/, 'next question must rebuild the composer card at the next step');
