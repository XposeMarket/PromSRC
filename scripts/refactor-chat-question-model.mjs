...
const sourceModelPath = path.join(root, 'web-ui/src/features/chat/questions/model.js');
const generatedModelPath = path.join(root, 'generated/public-web-ui/static/features/chat/questions/model.js');
...
const target = locateFunction(sourceChat, 'normalizePrometheusQuestionRecord');
const extractedBytes = Buffer.byteLength(target.text);
assert.ok(extractedBytes >= 700 && extractedBytes <= 12000, `question model extraction size ${extractedBytes} is implausible`);

const modelSource = `/** Canonical Prometheus Question normalization. */\n\nexport ${target.text.trim()}\n`;
const importLine = "import { normalizePrometheusQuestionRecord } from '../features/chat/questions/model.js';\n";
const anchor = "import { encodeInlineJsString } from '../features/chat/rendering/inline-escape.js';\n";
assert.equal(sourceChat.split(anchor).length, 2, 'question model import anchor must be unique');
...
assert.equal(nextChat.includes('function normalizePrometheusQuestionRecord('), false, 'old question normalization declaration must be removed');
assert.ok((nextChat.match(/normalizePrometheusQuestionRecord\(/g) || []).length >= 3, 'existing question normalization call sites must remain');

const nextBytes = Buffer.byteLength(nextChat);
assert.ok(nextBytes < priorBytes, 'ChatPage must shrink');
baseline.legacySurfaces['web-ui/src/pages/ChatPage.js'] = nextBytes;

fs.mkdirSync(path.dirname(sourceModelPath), { recursive: true });
fs.mkdirSync(path.dirname(generatedModelPath), { recursive: true });
fs.writeFileSync(sourceModelPath, modelSource);
fs.writeFileSync(generatedModelPath, modelSource);
fs.writeFileSync(sourceChatPath, nextChat);
fs.writeFileSync(generatedChatPath, nextChat);
fs.writeFileSync(baselinePath, `${JSON.stringify(baseline, null, 2)}\n`);

console.log(`Question model extracted: ${extractedBytes} bytes; ChatPage ${priorBytes} -> ${nextBytes}`);
