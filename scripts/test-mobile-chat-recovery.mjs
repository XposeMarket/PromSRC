...
assert.match(pages, /mobileStreamReceiptLedger\.has\(requestedSession, evt\)/, 'duplicate durable frames must be rejected before allocating a new assistant turn');
assert.match(pages, /_findMobileRecentlyCompletedTurn\(activeThread, evt, requestedSession\)/, 'late frames must reconcile against the recently completed turn identity');
