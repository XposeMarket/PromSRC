...
assert.match(mobile, /if \(!lane\) \{[\s\S]{0,900}_mobileBackgroundAgentDetailMessage\(storedRecord\)/, 'cold background detail records must become renderable agent messages');
assert.match(mobile, /if \(!lane\) \{[\s\S]{0,1100}_upsertMobileBackgroundSpawnLane\(/, 'background detail refresh must rebuild a missing in-memory lane');
