...
assert.match(
  ...
  'a currently valid explicit namespaced gateway route must never be rewritten from older local binding state',
);
assert.match(
  repair,
  /const bound = getMobileSessionTarget\(parsed\.targetId\);\s*if \(!bound\?\.gatewayId\) return arg;/,
  'stale route repair must require an existing persisted session binding before considering any gateway rewrite',
);
assert.match(
  repair,
