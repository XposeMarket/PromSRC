...
float personSupportHeight(float x,float z){
  ...
  for(const ParkedCar&parked:PARKED_CARS){float yaw=parked.yaw*PI/180.0f,pdx=x-parked.x,pdz=z-parked.z;
...
void drawDestructibleBuilding(const CityBuilding&b,int buildingIndex,const AtlasUv&uv){
  ...
  for(int side=0;side<BUILDING_DAMAGE_SIDES;++side)for(int row=0;row<BUILDING_DAMAGE_ROWS;++row)for(int column=0;column<BUILDING_DAMAGE_COLS;++column){
    if(buildingChunkDestroyed(buildingIndex,side,row,column))continue;
  ...
  // Destroyed panels are true openings. The former solid black backing made
  // damage read like a texture bug instead of a missing section of wall.
}
...
void drawPond(float x,float z,float radiusX,float radiusZ){
  float waterY=cityGroundHeightAt(x,z)+.12f;
  glEnable(GL_BLEND);glBlendFunc(GL_SRC_ALPHA,GL_ONE_MINUS_SRC_ALPHA);glColor4f(.08f,.42f,.62f,.82f);glBegin(GL_TRIANGLE_FAN);glVertex3f(x,waterY,z);
