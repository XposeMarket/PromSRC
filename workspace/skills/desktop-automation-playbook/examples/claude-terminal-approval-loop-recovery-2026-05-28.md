...
1. Use `desktop_screenshot` for full-layout truth when Raul distinguishes between multiple similar windows.
2. If Raul says “terminal, not app,” prefer `desktop_find_window` / `desktop_focus_window` with the terminal title, then verify with `desktop_window_screenshot` or visible title/context before typing.
