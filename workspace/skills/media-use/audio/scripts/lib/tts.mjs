...
const PROMETHEUS_HYPERFRAMES_WRAPPER = fileURLToPath(
  new URL("../../../../../../scripts/run-hyperframes.js", import.meta.url),
);

function bundledMediaBinary(envName, packageName) {
  ...
  try {
    ...
    // Standalone installs may intentionally rely on ambient PATH.
  }
  return null;
}

const BUNDLED_FFMPEG = bundledMediaBinary("FFMPEG_PATH", "@ffmpeg-installer/ffmpeg");
const BUNDLED_FFPROBE = bundledMediaBinary("FFPROBE_PATH", "@ffprobe-installer/ffprobe");
...
function ffmpegDurationFallback(absPath) {
  const ambient = spawnSync("ffmpeg", ["-i", absPath], { encoding: "utf8" });
  const ambientDuration = parseFfmpegDurationBanner(ambient.stderr);
  if (isFinite(ambientDuration) || ambient.error?.code !== "ENOENT") return ambientDuration;
  if (!BUNDLED_FFMPEG) return NaN;
  const bundled = spawnSync(BUNDLED_FFMPEG, ["-i", absPath], { encoding: "utf8" });
  return parseFfmpegDurationBanner(bundled.stderr);
}
...
export function ffprobeDuration(absPath) {
  ...
  if (r.error?.code === "ENOENT") {
    // Respect an ambient ffmpeg-only install first (and preserve the isolated
    // fallback contract tests), then use Prometheus's bundled ffprobe.
    const fallbackDuration = ffmpegDurationFallback(absPath);
    if (isFinite(fallbackDuration)) return fallbackDuration;
    if (!BUNDLED_FFPROBE) return NaN;
    const bundled = spawnSync(
      BUNDLED_FFPROBE,
      ["-v", "error", "-show_entries", "format=duration", "-of", "default=nw=1:nk=1", absPath],
      { encoding: "utf8" },
    );
    return bundled.status === 0 ? parseFloat(String(bundled.stdout).trim()) : NaN;
  }
...
function spawnHyperframes(args, opts = {}) {
  if (existsSync(PROMETHEUS_HYPERFRAMES_WRAPPER)) {
    return spawnP(process.execPath, [PROMETHEUS_HYPERFRAMES_WRAPPER, ...args], opts);
  }
  return spawnP("npx", ["hyperframes", ...args], opts);
}
...
function transcodeToWav(bytes, destWav) {
  ...
  if (ff.error?.code === "ENOENT" && BUNDLED_FFMPEG) {
    ff = spawnSync(
      BUNDLED_FFMPEG,
      ["-y", "-loglevel", "error", "-i", tmp, "-ar", "44100", "-ac", "1", destWav],
      { stdio: "ignore" },
...
export async function synthesizeOne({
  ...
  const r = await spawnHyperframes(args, { cwd: hyperframesDir });
...
export async function transcribeWav({ wavRel, lang = "en", hyperframesDir }) {
  const model =
    process.env.HYPERFRAMES_WHISPER_MODEL || (lang === "en" ? "small.en" : "large-v3");
