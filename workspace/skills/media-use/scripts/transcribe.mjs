...
const { values: args } = parseArgs({
  options: {
    ...
    "whisper-model": { type: "string", default: "small.en" },
...
function resolveParakeet() {
  for (const p of [
    ...
    join(homedir(), ".venvs", "parakeet", "Scripts", "parakeet-mlx.exe"),
  ]) {
...
function runWhisper() {
  ...
  try {
    // In Prometheus, prefer the committed wrapper: it exposes bundled
    // FFmpeg/ffprobe and discovers whisper.cpp's Windows Release output. Keep
    // npx as the portable fallback when this skill is installed elsewhere.
    const prometheusWrapper = fileURLToPath(
      new URL("../../../../scripts/run-hyperframes.js", import.meta.url),
    );
    const command = existsSync(prometheusWrapper) ? process.execPath : "npx";
    const commandArgs = existsSync(prometheusWrapper)
      ? [
          prometheusWrapper,
          "transcribe",
          inputPath,
    ...
    try {
      ...
      words = Array.isArray(t) ? t.length : Array.isArray(t?.words) ? t.words.length : undefined;
    } catch {
      /* leave undefined */
    }
