...
## Optional setup — HeyGen free-usage path

Download the installer from `https://static.heygen.ai/cli/install.sh`, inspect the saved script, and run it explicitly only after review. Then run `heygen update` and `heygen auth login --oauth`. OAuth uses subscription/free credits; `--api-key` bills API credits.
...
Single-shot helpers: `audio/scripts/heygen-tts.mjs` (one voice file). Transcription / background removal / captions use the `hyperframes` CLI (`transcribe`, `remove-background`), see the per-topic guides in `audio/references/` (`tts.md`, `bgm.md`, `sfx.md`, `transcribe.md`, `remove-background.md`, `captions/`). Inside Prometheus, these paths prefer the committed wrapper and bundled FFmpeg/ffprobe/whisper.cpp runtime; do not diagnose them from an ambient `npx` or `PATH` check alone.
...
| `heygen`           | catalog (bgm/sfx/image/icon) + TTS (voice) + avatar video — the free-usage path | Download and inspect the official installer, run it explicitly, then `heygen auth login --oauth` (needs >= v0.3.0) |
