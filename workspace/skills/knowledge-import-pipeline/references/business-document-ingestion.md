# Migrated doc-ingestion guidance

This reference preserves detailed guidance from the former `doc-ingestion` entrypoint. Read it only for the matching operation.
...
If any are missing: `node workspace\doc-skills-setup.js` from `<prometheus-workspace>`
...
All paths relative to `<prometheus-workspace>\workspace\`:
