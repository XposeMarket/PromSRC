...
- **Multiple plain X status URLs → start with `web_fetch_batch`.**
  - Use browser only for URLs whose fetched result is empty, blocked, or where the user asks for live interaction/context.
...
Use this when the user asks to **find my bookmarks**, **open X bookmarks**, **go to bookmarks**, or review saved/bookmarked posts.
