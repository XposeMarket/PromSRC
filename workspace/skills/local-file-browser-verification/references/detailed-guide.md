...
Use this skill when the user wants a local file, static page, generated HTML app, mini-game, prototype, or local browser artifact tested in a real browser instead of only inspected as code.
...
When the user is on mobile and asks for visible proof, send screenshots to origin/mobile after meaningful visible steps if the task involves UI state. Use `delivery_send_screenshot` when available.
