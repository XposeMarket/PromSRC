# Migrated docx-writer guidance

This reference preserves detailed guidance from the former `docx-writer` entrypoint. Read it only for the matching operation.
...
```
Run from `<prometheus-workspace>`. If it fails: `node workspace\doc-skills-setup.js`
...
Save files to `<prometheus-workspace>\workspace\` so the user can access them. Tell the user the exact path.
