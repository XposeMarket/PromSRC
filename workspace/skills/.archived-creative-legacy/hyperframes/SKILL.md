...
When this skill is used inside Prometheus Creative Video mode, treat it as a reference for HTML timing, layout, GSAP choreography, captions, transitions, and visual craft. The controlling workflow is still `prometheus-hyperframes-bridge` plus Prometheus Creative HTML Motion tools.

- Create Prometheus clips with `creative_create_html_motion_clip` or `creative_apply_html_motion_template`, not by creating a standalone HyperFrames Studio project.
- Validate Prometheus clips with `creative_lint_html_motion_clip`, `creative_render_html_motion_snapshot`, `creative_validate_layout`, and `creative_quality_report` where available.
- Export Prometheus clips with `creative_export_html_motion_clip` or `creative_export`, not `npx hyperframes render`, unless the user explicitly asked for a standalone HyperFrames project.
- Use complete self-contained HTML with inline CSS/JS. Avoid external scripts, external media URLs, sub-composition files, or `data-composition-src` unless the user is working in a standalone HyperFrames project.
- If this skill conflicts with Prometheus Creative tool docs, follow the Prometheus Creative tool docs.
