...
Inside Prometheus, use this skill as a translation reference, not as an instruction to create a separate render pipeline by default.

- Convert Remotion ideas into Prometheus-compatible self-contained HTML/CSS/JS when the goal is Creative Video mode.
- Build and QA through Prometheus Creative HTML Motion tools: `creative_create_html_motion_clip`, `creative_lint_html_motion_clip`, `creative_render_html_motion_snapshot`, and export tools.
- Run Remotion and HyperFrames CLI comparison/eval commands only when the user explicitly asks for a standalone conversion project, exact regression testing, or external CLI output.
- If the Remotion source relies on React state, async effects, third-party UI libraries, Lambda, or runtime interop, surface the limitation instead of forcing it into Prometheus HTML.
