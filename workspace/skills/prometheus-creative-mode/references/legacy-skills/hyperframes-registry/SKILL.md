...
This skill is for standalone HyperFrames registry blocks/components. Inside Prometheus Creative Video mode, prefer Prometheus HTML motion blocks first:

1. `creative_list_html_motion_blocks`
2. `creative_render_html_motion_block`
3. `creative_patch_html_motion_clip`
4. `creative_render_html_motion_snapshot`

Use `hyperframes add` only when the user explicitly asks for a standalone HyperFrames project or a registry item that must be installed outside the Prometheus Creative runtime.
