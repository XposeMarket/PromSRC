...
This skill is for standalone HyperFrames projects and direct CLI troubleshooting. Inside Prometheus Creative Video mode, do not use this as the normal render/preview path.

- For Prometheus HTML Motion structure and timing rules, read `prometheus-html-motion-spec`; the CLI skill is compatibility guidance only.
- For Prometheus clips, prefer `creative_lint_html_motion_clip`, `creative_render_html_motion_snapshot`, and `creative_export_html_motion_clip`.
- Use `npx hyperframes ...` only when the user explicitly asks for a standalone HyperFrames project, direct HyperFrames CLI operation, or debugging an external HyperFrames install.
- Do not assume `npx hyperframes`, FFmpeg, Chrome, or the HyperFrames Studio are installed just because this skill is bundled.
