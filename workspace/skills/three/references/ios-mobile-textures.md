...
texture.flipY = false; // when using billboard/canvas-style sprites that expect DOM orientation
...
If safe NPOT settings are not enough, do not upload the decoded image element directly. Draw the image into a same-origin power-of-two canvas first, then create a `CanvasTexture`.
...
function freshUrl(url) {
  return url + (url.includes("?") ? "&" : "?") + "pmtex=" + assetBust;
}
...
```js
for (const mesh of spriteMeshes) {
  if (mesh.material?.map === texture) mesh.material.needsUpdate = true;
}
```
...
```js
iframe.src = workspaceUrl + (workspaceUrl.includes("?") ? "&" : "?") + "pmcv=" + Date.now().toString(36);
```

Do the same for sprite asset URLs during diagnosis, as shown above with `pmtex`.
