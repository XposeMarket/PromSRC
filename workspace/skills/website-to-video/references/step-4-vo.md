...
If yes, captions are built as a separate composition (`compositions/captions.html`) in Step 5. The `transcript.json` drives the timing—each word appears or highlights as it is spoken. Read [the caption-authoring reference](../../media-use/audio/references/captions/authoring.md) for timing, styling, and positioning rules.
