# Migrated xlsx-writer guidance

This reference preserves detailed guidance from the former `xlsx-writer` entrypoint. Read it only for the matching operation.
...
```
Run from `<prometheus-workspace>`. If missing: `node workspace\doc-skills-setup.js`
...
Always write to `<prometheus-workspace>\workspace\`. Tell the user the exact filename after creation.
