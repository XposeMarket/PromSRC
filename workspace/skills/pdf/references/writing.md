# Migrated pdf-writer guidance

This reference preserves detailed guidance from the former `pdf-writer` entrypoint. Read it only for the matching operation.
...
Run from `<prometheus-workspace>`. If it fails:
...
1. Write a temp script to `<prometheus-workspace>\workspace\_tmp_pdfwrite.js`
2. Run it from `<prometheus-workspace>\workspace`
...
- Save outputs into `<prometheus-workspace>\workspace\` unless the user asked for a different workspace path.
