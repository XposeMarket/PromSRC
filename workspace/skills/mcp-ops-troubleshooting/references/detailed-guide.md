...
- `Unterminated string in JSON` or similar parsing errors on large `tools/list` responses.
...
6. **Streamable HTTP framing**
   - Inspect `Content-Type`; servers may return JSON or `text/event-stream` to POST requests.
   - For SSE, buffer until a complete blank-line-delimited event is available.
   - Join all `data:` lines in that event before parsing JSON.
   - Do not parse each network chunk independently; chunks may split quoted strings or UTF-8 sequences.
   - Preserve and resend `Mcp-Session-Id` when the initialize response supplies it.
...
- Finish by verifying the durable connection attempt; lower-level `connect` success alone is insufficient.
