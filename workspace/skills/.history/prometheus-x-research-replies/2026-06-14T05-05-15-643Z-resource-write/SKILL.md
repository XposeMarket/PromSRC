...
description: Use this skill when the scheduled @raulinvests X/Twitter research, reply, quote-repost, or regular repost engagement workflow needs to hit Mara's growth targets: 25+ high-quality replies/day, 6-10 quote reposts/day, 8-15 regular reposts/day, browser-only execution, no em dashes, memory logging, and browser_close cleanup.
version: 1.4.0
triggers: X research replies, scheduled research posting, Mara reply run, Mara engagement run, quote-repost AI posts, regular repost workflow, repost more on X, 25 replies a day, increase X replies, raulinvests replies, X engagement workflow, quote repost workflow, AI Twitter replies
---

# Prometheus X Research & Replies Operator

Scheduled research and engagement workflow for @raulinvests. The goal is not to wedge every conversation into agent memory. The goal is to find relevant posts and respond naturally to what that post is actually about, in Raul's builder/operator voice, while keeping enough volume to grow the account.
...
These are quality-weighted targets, not permission to spam. Replies are the priority. Quote reposts and regular reposts should be used more often than before, but only when they make Raul look sharp, plugged in, and high-taste.
...
- **0-1 quote repost**. Use only when Raul can add a real angle.
...
Use a quote repost when Raul can add one of:
...
Regular reposts are for alignment and taste. Repost posts that reinforce Raul's account identity around:
...
- hostile dunking
- content that dilutes Raul's positioning
...
8. Quote repost up to 1 strong candidate if Raul can add a meaningful angle.
