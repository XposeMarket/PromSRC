...
Modern file tools should help recover from common drift instead of returning dead-end errors:

- Exact-text misses from `find_replace` and source variants may include up to three likely intended locations based on normalized whitespace, prefix/suffix anchors, and closest token blocks.
- Bad read/list/stats/grep/edit paths may include likely intended file or directory paths.
- Stale line-number edits may include the file tail and a concrete reread/retry hint.
- Treat these candidates as recovery aids, not permission to guess. Pick a candidate only after the snippet/path clearly matches the requested edit, then re-read the exact window if there is any doubt.
...
If exact text is not found and the tool returns likely locations, do not retry the same stale text. Use the best candidate snippet to re-anchor the edit, or read around the suggested line/path and make a smaller replacement.
...
2. Use any tool-provided recovery candidates first: likely paths, nearby exact-text locations, normalized-whitespace matches, or stale-line tails.
3. `read_file` the affected area or full file when the candidate is not already enough to prove the next edit.
4. Identify exactly what is missing, duplicated, or malformed.
5. Use the smallest corrective edit that restores structure.
6. Re-read and verify.
7. If available, use git only for inspection/recovery commands after understanding the file state; do not blindly overwrite user work.
