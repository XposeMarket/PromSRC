...
description: Use this skill when running Windows shell commands, PowerShell, cmd, npm scripts, git commands, path-sensitive operations, process checks, build/test commands, or command-line workflows in Prometheus on Windows. Triggers on phrases like run command, PowerShell, Windows shell, npm build, git status, path with spaces, cmd command, terminal command, execute script, check process, and shell quoting. Use it to choose PowerShell/cmd safely, handle Windows paths, avoid Bash-only syntax, and keep shell use for process execution rather than file editing.
emoji: "🪟"
version: 1.1.0
triggers: run command, PowerShell, Windows shell, npm build, git status, path with spaces, cmd command, terminal command, execute script, check process, shell quoting, Windows path, run npm, command failed
---
