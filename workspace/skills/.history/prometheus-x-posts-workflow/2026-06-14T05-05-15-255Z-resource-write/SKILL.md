...
description: Use this skill when Mara or a scheduled/delegated @raulinvests X/Twitter posting workflow needs to publish one original post that complements the broader engagement strategy: 25+ replies/day, more quote reposts, regular reposts, browser-only posting, schedule-owner memory, duplicate checks, no em dashes, browser_close cleanup, and post-run logging.
version: 1.3.0
triggers: autonomous X posting, scheduled X post, Mara post run, tweet from @raulinvests, broaden X topics, stop repeating agent memory, X content rotation, update X post schedule, original tweet workflow, raulinvests posting, scheduled tweet, AI Twitter posting, Mara engagement strategy
---
