...
description: Use this skill when the user asks to edit JSON, config files, manifests, scheduled job JSON, model routing config, team config, heartbeat config, integration state, skill manifests, or any structured file where commas/brackets can break runtime state. Triggers on phrases like edit config, fix JSON, update manifest, skill manifest, config surgery, model routing JSON, cron jobs JSON, managed teams JSON, validate JSON, and rewrite config safely. Use it to read the full structure, modify minimal fields, validate syntax, write the whole file when needed, and verify readback.
emoji: "🧩"
version: 1.1.0
triggers: edit config, fix JSON, update manifest, skill manifest, config surgery, model routing JSON, cron jobs JSON, managed teams JSON, validate JSON, rewrite config safely, config file edit, JSON surgery, update skill manifest, edit jobs.json
---
