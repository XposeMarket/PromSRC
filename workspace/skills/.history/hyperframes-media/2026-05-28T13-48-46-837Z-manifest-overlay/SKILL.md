...
Inside Prometheus Creative Mode, prefer the first-class Creative audio tools when audio belongs to a project, generation history, rough cut, overlay, or final export:

- `creative_import_audio`: uploaded/local audio.
- `creative_download_audio`: supported media-page audio via yt-dlp.
- `creative_extract_audio_from_video`: audio from a local video.
- `creative_generate_voiceover`: configured OpenAI/OpenAI Realtime-compatible or xAI voiceover.
- `creative_transcribe_audio`: OpenAI/xAI transcription.
- `creative_sync_captions_to_audio`: editable timed caption HTML layer.
- `creative_add_audio_track`, `creative_mix_audio_tracks`, `creative_add_music_bed`, `creative_add_sound_effects`: final timeline/mix work.

Use the HyperFrames CLI media commands as a local/offline fallback or standalone HyperFrames-project workflow. If both paths are available, Prometheus tools win for lineage, project state, asset import, waveform analysis, and final muxing.
