...
2. **Reusable lesson discovered during normal work:** Do not silently modify a skill and do not silently submit a Curator candidate. Identify the existing related skill, state the concrete workflow/tooling/recovery improvement discovered, and ask the user through `ask_prometheus_questions` whether they want that skill updated. If no relevant skill exists, ask whether they want a new skill created. Only create a Brain Curator candidate after the user asks to capture the lesson for later review rather than authorizing a direct update.

Brain Curator is the only automatic writer. User-directed skill edits are allowed immediately; learned behavior changes require an explicit user choice before either direct editing or candidate submission.
...
Only user messages prove user preferences or direct authorization. Assistant summaries, tool-call volume, and completion text do not. When a reusable workflow improvement is discovered, bring it to the user with the affected skill and exact proposed addition. Use `ask_prometheus_questions` for that choice; do not quietly edit the skill or quietly route it into Curator.
