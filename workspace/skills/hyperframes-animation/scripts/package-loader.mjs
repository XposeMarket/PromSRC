...
export function hyperframesPackageSpec(packageName) {
  ...
  const installedVersion = readResolvedPackageVersion(packageName);
  if (installedVersion) return `${packageName}@${installedVersion}`;
...
function readResolvedPackageVersion(packageName) {
  ...
  for (const ancestor of ancestors(dirname(entry))) {
    try {
      const manifest = JSON.parse(readFileSync(join(ancestor, "package.json"), "utf8"));
      if (manifest.name === packageName && typeof manifest.version === "string") {
        return manifest.version;
      }
    } catch {
      // Keep searching toward the filesystem root.
    }
  }
  return null;
}
