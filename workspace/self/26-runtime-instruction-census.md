...
| Shared Prometheus runtime contract | `src/config/prometheus-runtime-contract.md` | Direct/background/team/manager agent prompt assembly and Reactor agent runtime | Distinct agents and managers | Role conditional | ~1,200 chars / ~300 | Former subagent soul and main soul action/capability language |
| Legacy subagent soul | `src/config/subagent-soul.md` | Not normally injected; fallback only if the runtime contract asset is absent | Legacy/rollback path | Exceptional fallback | 3,359 chars / ~908 | Replaced by runtime contract + AGENT.md |
| Agent private memory | Per-agent `MEMORY.md` through `runtime-actor.ts` | Named agent/manager turns | That actor only | Role/file conditional, capped | ~0–6,000 chars; manager cap 8,000 | Role history that was previously mixed into caller/team notes |
...
| Direct subagent | Named actor identity + shared runtime contract + AGENT.md/caller + private agent MEMORY + tools + skills; no main USER/SOUL/MEMORY/BUSINESS/CIS/retrieval |
| Background agent | Same isolation plus spawning prompt/reference hint and task caller |
| Team subagent | Same isolation plus team-scoped AGENT.md/MEMORY.md, workspace/room/assignment caller, and bounded shared team memory snapshots |
| Team manager | Dedicated manager AgentDefinition + manager AGENT.md + private manager MEMORY.md + bounded shared team memory; no main USER/SOUL/MEMORY |
...
6. The active team caller contradiction is removed. Historical docs, stored chats, Brain prompts, voice prompts, provider fallbacks, and main-only helper prompts can still contain `You are Prometheus`; those are not team-agent identity injections.
...
## 2026-07-13 identity and memory correction

The current canonical contract is documented in [29-agent-identity-and-memory-runtime.md](29-agent-identity-and-memory-runtime.md). It supersedes the subagent/manager rows and contradiction notes above wherever older wording remains. The key runtime invariant is now: capability posture is shared, identity and memory ownership are not.
