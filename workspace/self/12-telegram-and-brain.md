...
- **Direct six-hour activity package (2026-08-08):** immediately before every Thought model call, `src/gateway/brain/activity-package.ts` assembles `prometheus.thoughts.activity-package.v1` from canonical runtime stores. The window is deterministic UTC `[start,end)`, with stable event IDs and merged provenance refs. It covers timestamped chat messages/session activity, task journals/evidence, runs/schedules/heartbeats, managed threads/teams, tool observations/errors, browser registry/observations, file/workspace changes, agents/subagents, runtime/config changes, important events, and current unresolved work. The package is injected directly into the Thought prompt; the Thought tool allowlist no longer exposes directory/list/search tools for reconstructing covered activity.
- Package artifacts are written under `workspace/Brain/activity-packages/<date>/<window>/activity-package.json` with direct-read-only continuation JSONL parts when the inline ledger would exceed the model budget. `sourceCoverage`, redaction policy, omissions, pagination/file-scan caps, and continuation hashes are part of the package; a partial source is never presented as an empty source. Metrics are appended to `workspace/Brain/state/activity-package-metrics.jsonl` (assembly latency, source/file/record/event counts, dedupe, package chars/tokens, continuation writes, and post-run search-call counts).
- The package is redacted before prompt injection: credentials, tokens, cookies, secret-key values, raw tool payloads, binary/screenshot payloads, and raw provider output are omitted or replaced. Stable IDs and safe workspace-relative provenance remain. Continuations are direct paths from the package, not a new search surface.
...
- the activity package carries the six-hour operational evidence directly; Thought reads exact continuation refs only when the inline ledger says continuation is required. Brain skill episodes/gardener evidence remains included as timestamped package activity, while durable memory roots continue to arrive through the normal cron/personality context.
