...
## Tool performance telemetry and first-message recovery — 2026-08-08

`src/gateway/chat/tool-performance-telemetry.ts` now records an opaque per-tool ID beneath the existing opaque turn trace. The safe lifecycle is `call_emitted → dispatch_start → executor_start → first_output → complete → result_serialized → result_delivered → result_to_model → next_visible_token`, with cancelled/abandoned terminal states. `chat.router.ts` attaches only IDs, family/name categories, counts, byte lengths, estimates, and timings to tool SSE events; it never records arguments, prompts, message text, tokens, file contents, page contents, credentials, or result contents. `model-call-worker-pool.ts` adds queue/provider/event-batch/request-result-byte/PID/RSS fields, and model usage rows carry the same trace ID.

The repeatable safe family harness is `scripts/benchmark-tool-performance.mjs` (`npm run benchmark:tool-performance`). It covers desktop, browser, workspace, terminal, web/search, MCP/connector, subagent/task, and core representatives with disposable sessions pinned to the requested model. Because categories/tools can be lazy on the first message, the local benchmark path uses the existing allowlist only when `origin.source=local_benchmark`, activates required safe categories, and sends one bounded same-session follow-up if the required tool was not emitted. The follow-up is labeled as recovery and is included in wall time; it is not hidden as a speedup.

The focused 2026-08-08 follow-up used Luna-low with explicit safe fixtures. Dispatch/result transport remained small (roughly 0–5 ms p50); provider emission and post-result visibility remained the dominant user-visible cost. The normal desktop doctor path now uses `deep=false` and measured 2.25 s p50 / 2.52 s p95 with 0 errors across three samples; the explicitly requested deep path has a doctor-only 5 s OCR budget and measured 10.85 s in the stable bounded sample. Browser Prometheus-target samples produced 6/6 required calls with 0 errors; cold CDP attach remains the main browser-local tail. See `docs/P0-1-TOOL-PERFORMANCE-REPORT-2026-08-08.md` for raw conditions, before/after aggregates, excluded restart-window samples, and remaining work.

The gateway now prewarms context-build workers and the chat/tool router before opening its listener, and completes the startup wiring that previously ran after listen. This closes the port-open/event-loop-busy window that could make the first message arrive before tools were ready. The benchmark still sends one bounded same-session follow-up when a required tool is absent, and retries only disposable setup/cleanup requests during listener rotation; recovery is retained in the measurements rather than hidden.

Per-turn tool-surface and full-system-prompt caches make subsequent provider/tool rounds cache hits (0 ms in the controlled traces). Context worker ready waits were 0–1 ms in the final stable samples; context assembly itself remained roughly 0.33–0.68 s, so provider/model round variance—not local dispatch—is the next optimization target.
