...
- `src/gateway/process/runtime-worker-broker.ts` and `runtime-worker-protocol.ts` own the reusable bounded child-process boundary; the first production consumer is memory-index maintenance through `memory-index-worker.ts`
- `src/gateway/turn-workers/` owns the bounded turn-worker protocol/pools; production model calls made through `OllamaClient` run in `model-call-turn-worker.ts` children, while finalization git/stat/read summaries run in `turn-file-change-worker.ts` children; both return blob references
- `src/gateway/turn-jobs/` owns the SQLite-WAL turn journal, content-addressed blobs, session/resource leases, fenced checkpoints/tool effects, final-persistence state, and child-process retention/GC
- `src/gateway/turn-delivery/` owns exact-byte progress/final bounding and replay-buffer primitives; `chat.router.ts` applies them to main-chat SSE/WebSocket/replay delivery

Main-chat turn safety:

- `src/gateway/chat/turn-coordinator.ts` remains the immediate in-process admission lock. The durable journal adds a fenced `session:<id>` lease across journal jobs. `runInteractiveTurn(...)` callers queue FIFO for the same session, while different sessions remain parallel.
- Provider/model calls, finalization file-change scans, stored-thread footprint diagnostics, and tool-observation persistence now run in bounded child processes, and memory-index maintenance remains isolated in its own recycled child. Session final writes are cooperative/atomic and are awaited before durable final publication; optional observation metadata may finish and attach after terminal delivery. The complete `handleChat(...)` orchestration/tool loop, most prompt/session work, tools, approvals, tasks, teams, and subagents are still gateway-owned; this is targeted hotspot isolation, not yet one OS process per complete turn. See [30-runtime-process-isolation.md](30-runtime-process-isolation.md).
...
Durable runtime startup/lifecycle:

- Boot opens `<configDir>/runtime/turn-jobs.sqlite`, enables WAL, and reconciles expired job/delivery/resource leases before listeners become the normal runtime. It also settles an already-durable `final_persisted` job only when that job has zero outbox rows; the exact client request can still replay the persisted final. A serialized, bounded 15-second timer repeats stale-lease and narrow final-state reconciliation, reporting any remaining batch backlog without claiming work for execution. Journal initialization failure fails startup rather than silently disabling durability.
- Turn payloads/checkpoints/results and oversized delivery content live under `<configDir>/runtime/turn-blobs/`; the SQLite database stores references.
- Turn attachments/admission payloads, central tool results, large model envelopes, and bounded finals use the cooperative/asynchronous blob path for serialization, compression, write, and fsync. Stream replay only reuses content already made durable, and immutable reuse checks a bounded header instead of rereading a complete multi-megabyte body on the gateway event loop.
- Restart/shutdown lifecycle awaits cooperative session persistence and all internal worker pools. Do not reintroduce a synchronous `flushSession(...)` preflight before restart; large current sessions are exactly where that would freeze the control plane.
- A lazy daily retention timer runs pruning in a recycled child: terminal jobs default to 30 days/500 per pass, unreferenced blobs to 90 days/2,000 scanned/500 deleted per pass. Saturated passes continue after a five-minute delay until the bounded backlog/cursor drains. Active, waiting, review, and final-persisted jobs are never pruned; direct database blob references and recently read/reused blobs are protected.
- `/api/health.turnRuntime` reports the model-process pool, the two-worker-by-default file-change pool and its V8 old-space cap, context-footprint and observation-persistence children, cooperative session-persistence state, journal queue/active/waiting counts, lease-and-final recovery schedule/backlog/result, and retention worker/schedule/last-result state; `/api/health.memoryMaintenance` remains the memory-maintenance worker view. Recovery is not queued/checkpoint redispatch and is not channel redelivery.
- Restart lifecycle and `SIGINT`/`SIGTERM` shutdown stop ingress first, durably mark live work interrupted, then await memory/model/file-change/context/observation/retention worker drains and cooperative session flushes before closing the journal. This preserves worker callbacks and distinguishes an orderly restart from an explicit user cancellation. The signal path has an operator-configurable hard deadline (30 seconds by default). `PROMETHEUS_DISABLE_MODEL_WORKERS=1`, `PROMETHEUS_DISABLE_FILE_CHANGE_WORKERS=1`, `PROMETHEUS_DISABLE_TURN_JOURNAL=1`, and `PROMETHEUS_DISABLE_TURN_RETENTION=1` are diagnostic fallbacks, not normal workflow modes.
