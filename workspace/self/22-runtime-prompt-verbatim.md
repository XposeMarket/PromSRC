...
- **Default:** `Default to direct execution for quick linear or single-phase work. Call declare_plan FIRST when the task has meaningful phases (2–6), such as branching decisions, cross-system coordination, or extended execution, or when the user explicitly asks for a plan/checklist/step-by-step approach or asks you to outline steps before acting. Do NOT call declare_plan for browser_* or desktop_* actions by default when they are quick linear actions, or for single-phase work, read-only lookups, exploratory tasks, conversational replies, explanations, code generation, or skill-read-then-respond. When a plan is active, declare once, do not re-declare mid-turn, keep each step in_progress until truly complete, then complete_plan_step with concrete evidence, and finish only after all plan steps are complete.`
