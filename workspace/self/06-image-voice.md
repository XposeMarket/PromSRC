...
## 12A-1A) Mobile Voice Room, orb, and preview behavior (2026-08-01)

The mobile Voice surface now has two deliberate presentation paths. Inline voice in chat keeps the compact waveform/strand treatment. Standalone `#mobile/voice` uses a face-on SVG sphere with depth shading, surface latitude/longitude lines, flow paths, and a transparent canvas particle field. The shared warm-mic analyser drives the sphere's lift, scale, grid squash/rotation, brightness, and particle motion; theme variables keep the palette in Prometheus gold, Olympian blue, or violet, and reduced-motion users receive a static-safe path. The real Prometheus logo/base was not replaced: the earlier transparent `assets/voice-flame/` variants remain reference assets, while the active Voice visual is the orb.

Standalone Voice response text is rendered as stable finalized chat text, not the old rolling lyric overlay. Artifact/source/product previews use a mounted, preloaded wheel: every card remains in the DOM while the active card rolls forward in 3D, cards are borderless and headerless in this surface, and the shared drag helper has a small dead-zone so a tap does not flash a new card.

Mobile multi-agent Voice Room state is durable and roster-scoped. `POST /api/voice-rooms/resolve` resolves the deterministic `voice_room_*` session and stores its `voice_room` channel/roster; `POST /api/voice-rooms/:id/transcript` appends bounded, deduplicated user/assistant transcript entries. The mobile host routes explicit spoken names/aliases before a turn reaches a participant, suppresses ambiguous/unknown handoffs, tracks a short focus window, and persists per-participant quiet/wake state. Opening ordinary chat or a fresh standalone Voice draft leaves an old room binding; only an explicit Voice Room transcript route restores that room.

For Codex Voice/Live v3, a room handoff switches the app-server thread for the selected participant through managed `appendText`/speech paths, parks the prior peer, and keeps a bounded warm pool (up to four connections) with parallel prewarm and shared-mic track cloning/replacement. The mobile bridge owner is rebound to the durable room session before thread operations. The room-only `voice_room_handoff` function is a runtime safety fallback injected only while a mobile room is active; it is not part of the 39 static voice schemas and does not widen the Worker boundary.

The latest voice/Worker parity pass also keeps voice desktop screenshots on the worker's vision-first skip-OCR default, makes post-action captures opt-in, accepts canonical window selectors, uses the configured preferred search provider unless multi-provider search is explicit, avoids hidden synchronous URL enrichment in source/product cards, and gives mobile realtime tool calls a 60-second HTTP budget with per-tool elapsed telemetry. Canonical source remains `web-ui/src/mobile/*` with the generated public mirror under `generated/public-web-ui/static/mobile/*`; meaningful PWA changes still require the service-worker version/cache-bust rule documented in `16-mobile-app.md`.
...
- Voice desktop click schemas also expose `modifier:"none"` as the default path. `executeVoiceAgentTool(...)` strips `none` and only forwards `shift|ctrl|alt`, so accidental modified desktop clicks should not leak through the compact voice wrapper.
- Voice browser/desktop wrappers mirror the main wrapper philosophy: expose compact `action`-based surfaces to the Voice Agent, keep granular compatibility handlers executable internally, and preserve the same browser/desktop safety, screenshot, coordinate, and policy checks behind the wrapper normalization.
