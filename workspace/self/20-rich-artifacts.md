...
Last verified against `src/gateway/rich-artifacts.ts`, `src/tools/{market,stocks,weather,polymarket,mapcard}.ts`, `src/gateway/tools/defs/cis-system.ts`, `src/gateway/agents-runtime/subagent-executor.ts`, `src/gateway/routes/chat.router.ts`, `src/gateway/scheduling/schedule-admin-tools.ts`, `src/runtime/build-status.ts`, `web-ui/src/pages/ChatPage.js`, `web-ui/src/mobile/mobile-pages.js`, `web-ui/src/styles/{components,mobile}.css`, and the `task-lifecycle` / `interactive-visuals` / `polymarket-research` skills on: 2026-06-05
