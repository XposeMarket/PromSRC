...
Gateway-freeze diagnostic (2026-07-15): if mobile, SSE/WS, and Telegram all stop and then flush together, treat it as control-plane event-loop starvation, not a PWA cache problem and not proof that Telegram “pinged” the gateway awake. The measured memory-index offender now runs in a maintenance child, and provider/model calls now run in the model-worker pool; inspect `/api/health.memoryMaintenance` and `/api/health.turnRuntime`. Full `handleChat(...)` orchestration/tool-loop isolation is still pending, so synchronous work in a remaining gateway-owned path can still produce the same symptom. See [30-runtime-process-isolation.md](30-runtime-process-isolation.md).
...
- Active mobile run recovery is **incremental-first** on reconnect/focus/WebSocket-open recovery: `refreshMobileRunRecovery` preserves an existing live assistant bubble when it already has text, `processEntries`, or `liveTraceEntries`, fetches `/api/mobile/chat/stream/:sessionId?after=<lastSeq>`, and appends only missed/new frames. It resets and replays from `after=0` for cold recovery or when replay metadata shows an eviction gap (`firstSeq > lastSeq + 1`). The server now retains at most 12,000 frames **and** 16 MiB per session; the byte cap is authoritative for large tool/media events, so an hour-long stream may evict before the count cap. Service worker version for the incremental client behavior: `pm-v58-2026-06-17-incremental-stream-recovery`. [updated 2026-07-15]
...
- CHAT VOICE EXIT FREEZE GOTCHA (2026-08-02): exiting inline chat voice can leave the page frozen except the hamburger if cleanup throws before overlay classes drop. Active voice makes `.pm-composer.is-voice-active` a fixed full-viewport layer (`pm-chat-voice-active`); the header/hamburger sits above it, so only that remains tappable. Rule: `_closeChatVoiceMode()` must clear `is-voice-active`, `pm-chat-voice-active`, `pm-chat-voice-focus`, `pm-chat-voice-docked`, and host mount state in a `finally` block even when `chatVoiceHost._pmCleanup` fails. Voice-page `_pmCleanup` must drop layout classes first and isolate later teardown steps so one failure cannot abort the rest. Page leave should call the hardened close path rather than a bare cleanup + partial class remove.
