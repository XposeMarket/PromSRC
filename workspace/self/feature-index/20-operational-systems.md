...
- Before the Thought model call, `src/gateway/brain/activity-package.ts` creates the authoritative six-hour activity package from canonical stores. It includes chat/task/run/team/tool/browser/file/agent/runtime/event provenance and unresolved work using stable IDs and UTC `[start,end)` boundaries; the package is injected directly rather than reconstructed through the audit mirror.
- The package is bounded for model context by an inline ledger plus direct continuation JSONL. Redaction, source failures, dedupe, pagination/file caps, continuation hashes, package size/token estimates, and assembly/search metrics are persisted under `workspace/Brain/activity-packages/` and `workspace/Brain/state/activity-package-metrics.jsonl`.
