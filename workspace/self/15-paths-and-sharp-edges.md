...
- runtime workers are internal gateway children, not managed user command runs; inspect `/api/health.memoryMaintenance`, `/api/status.gatewayQueues`, and gateway logs rather than expecting a record under `<configDir>/processes/`
- current chat/session authority is the configured session store plus live-runtime/recovery state (`src/gateway/session.ts`, `src/gateway/live-runtime-registry.ts`, `src/gateway/runtime-recovery.ts`). Do not treat the retired `<configDir>/runtime/turn-jobs.sqlite` / `turn-blobs` architecture as current storage.
...
- HTTP JSON ingress is still a gateway-owned sharp edge: `core/app.ts` globally accepts up to 50 MiB through `express.json(...)`, so parsing/allocation happens in the gateway before normal route handling. Do not claim complete large-payload isolation until affected routes have appropriately bounded or streaming-compatible admission.
- current process-isolated hotspots include memory-index maintenance, memory search, context construction, provider/model calls, Brain activity packaging, and other source-backed worker clients. File-change summaries and compact tool observations are currently gateway-owned (`src/gateway/file-change-summary.ts`, `src/gateway/tool-observations.ts`); do not describe the deleted file-change/context-footprint/observation worker pools as current. Use [30-runtime-process-isolation.md](30-runtime-process-isolation.md) before claiming one process per full turn.
- runtime recovery is conservative: `runtime-recovery.ts` reconstructs state from current durable/session/process evidence and file-change summaries, but a recovered record is not proof that an uncertain external effect is safe to execute again. Verify live state before retrying a side effect after interruption.
...
If Prometheus gains or loses modes, tools, providers, connectors, account gates, proposal repair behavior, approval behavior, context/compaction behavior, tool observation behavior, frontend context indicators, memory/index layers, or worker/process boundaries, update this file only after reading the exact source files that implement the change.
Do not refresh this file from memory, from UI copy alone, or from older workspace notes. Repository-relative source paths are canonical; machine-specific checkout roots and deleted implementation directories are documentation bugs, not harmless examples.
