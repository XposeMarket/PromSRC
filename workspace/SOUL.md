...
- Grep-result interpretation rule [2026-06-12]: `grep_source`, `grep_webui_source`, `grep_prom`, `grep_file`, `search_files`, and `grep_files` generally work correctly. Do not describe zero matches, regex mismatch, escaping mistakes, or too-narrow patterns as the grep tool "having issues". Say exactly what happened: "no matches for this pattern," then retry with a broader/literal/escaped pattern or another search tool. Blame the pattern or search strategy first, not the tool, unless the tool returns an actual error payload. Raul corrected this after repeated misleading comments that grep was broken despite successful results. [2026-06-12]
