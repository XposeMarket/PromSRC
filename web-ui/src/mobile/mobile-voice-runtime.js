// Voice transport runtime. Kept behind the Voice feature boundary so the Chat route does not statically own WebRTC, camera, or realtime event handling.
export function createMobileVoiceRuntime(context = {}) {
  const {
    ICONS,
    MOBILE_CHAT_SESSION_ID,
    PM_MOBILE_CHAT_MESSAGE_PAGE_SIZE,
    PM_VOICE_ROOM_FOCUS_MS,
  ...
  function _handleMobileRealtimeAgentQuietTranscript(transcript) {
    ...
    if (!heard || !heard.includes(wake)) {
      ...
      return true; // not woken — suppress transcript display and keep the surface silent
    }
  ...
  function _stopMobileRealtimeAgentSession(options = {}) {
    ...
    // AVAS can have a decoded frame queued in the media element.  Silence it
  ...
  function _shouldIgnoreMobileRealtimeTranscriptForCurrentTurn(event = {}, type = '') {
    ...
    if (__pmRealtimeAgent.quiet?.active && current !== itemId) {
      ...
      // Treat each AVAS item as the new wake candidate instead of rejecting it
