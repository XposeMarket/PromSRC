import { ICONS as V2_ICONS } from '../../ui/icons.js';
...
// This route keeps the existing mobile settings UI and actions, while all
// gateway reads and writes are injected by the V2 route owner.
const ICONS = V2_ICONS;
let activeSettingsApi = null;

function mobileGatewayFetch(path, options) {
  if (!activeSettingsApi?.request) return Promise.reject(new Error('Settings gateway is unavailable.'));
  return activeSettingsApi.request(path, options);
}

function loadGatewayStatus(options) {
  return activeSettingsApi?.gatewayStatus
    ? activeSettingsApi.gatewayStatus(options)
    : mobileGatewayFetch('/api/status', { timeoutMs: options?.timeoutMs || 30000 });
}

function loadVoiceStatus() {
  if (activeSettingsApi?.voiceStatus) return activeSettingsApi.voiceStatus();
  return Promise.all([
    mobileGatewayFetch('/api/realtime/status').catch(() => null),
    mobileGatewayFetch('/api/realtime/xai/status').catch(() => null),
    mobileGatewayFetch('/api/settings/credentialed-model-providers').catch(() => null),
  ]).then(([realtime, xaiRealtime, credentials]) => ({
    realtime: realtime || { configured: false },
    xaiRealtime: xaiRealtime || { configured: false },
    providers: Array.isArray(credentials?.providers) ? credentials.providers : (credentials?.ids || []),
    voice: { sttProviders: [], ttsProviders: [] },
  }));
}

function prettifyModelName(model, provider) {
...
const BUILTIN_STATIC_MODELS = {
  ...
  anthropic: ['claude-fable-5-1','claude-fable-5','claude-opus-5','claude-opus-4-8','claude-opus-4-7','claude-opus-4-6','claude-sonnet-5','claude-sonnet-4-6','claude-sonnet-4-5-20250514','claude-haiku-4-5-20251001'],
...
function modelOptions(provider, models) {
  return (models || []).map((model) => ({ value: model, label: prettifyModelName(model, provider) }));
}
...
export function renderMobileV2SettingsSurface(slot, {
  section = '',
  navigate,
  api,
  overviewExtraHtml = '',
} = {}) {
  if (!api?.request) throw new TypeError('Mobile V2 settings require a gateway client.');
  activeSettingsApi = api;
  const current = SECTION_IDS.has(section) ? section : '';
  slot.innerHTML = `
    <div class="pm-settings-body pm-v2-settings-body">
      ${current ? `<div class="pm-settings-topnav"><button class="pm-btn ghost" data-settings-home>${ICONS.back} All settings</button></div>` : ''}
      <div id="pm-settings-content">${current ? `<div class="pm-card">${status('Loading settings...')}</div>` : renderSettingsOverview(overviewExtraHtml)}</div>
    </div>
  `;
  slot.querySelector('[data-settings-home]')?.addEventListener('click', () => navigate?.('settings'));
  slot.querySelectorAll('[data-settings-section]').forEach(btn => {
    btn.addEventListener('click', () => navigate?.(`settings/${btn.getAttribute('data-settings-section')}`));
  });
  ...
  return () => {
    if (activeSettingsApi === api) activeSettingsApi = null;
  };
}

function renderSettingsOverview(overviewExtraHtml = '') {
  return `
  ...
  page.querySelector('#pm-refresh-models')?.addEventListener('click', async () => {
    ...
    try {
      ...
      if (modelEl && models.length && modelEl.tagName === 'SELECT') {
        ...
        modelEl.innerHTML = modelOptions(provider, models).map(m => `<option value="${escapeHtml(m.value)}">${escapeHtml(m.label)}</option>`).join('');
...
async function renderVoice(content, navigate) {
  ...
  `;
  content.querySelector('[data-go-models]')?.addEventListener('click', () => navigate?.('settings/models'));
  content.querySelector('[data-go-creds]')?.addEventListener('click', () => navigate?.('settings/credentials'));
}
