...
/**
 * Minimal identity stub injected when a subagent has no identity files.
 * Ensures the model knows it is an autonomous tool-using agent, not a plain
...
function inferToolProfile(userMessage: string, requestedProfile?: ToolProfile): ToolProfile {
  // Explicit agent config always wins — no inference needed
  if (requestedProfile) return requestedProfile;
  // Sub-agents and spawned tasks with no profile get full access by default.
  // Inference only applies to interactive chat turns.
  return 'full';
}
...
export class Reactor {
  ...
  async run(userMessage: string, options: ReactOptions = {}): Promise<string> {
    ...
    try {
      ...
      // eslint-disable-next-line @typescript-eslint/no-var-requires
      const { getConfig } = require('../config/config.js');
      const globalWs = String(getConfig().getConfig()?.workspace?.path || process.cwd());
      // If an explicit workspacePath is provided (e.g. from spawnAgent), use it.
      // Otherwise check if AsyncLocalStorage already has a scoped workspace set
      // (from an outer runWithWorkspace call). Fall back to global workspace.
      workspacePath = String(options.workspacePath || getActiveWorkspace(globalWs));
    } catch {
    ...
    console.log(`${label} WORKSPACE_SCOPE  ${workspacePath}`);
    ...
    // ── Try native function-call path first (secondary channel) ─────────────
    // Cloud providers (openai, openai_codex) always support native tool calls.
    // Ollama: only enabled when NATIVE_TOOL_CALLS_ENABLED (model is large enough).
    // If the model emits proper tool_calls[] objects, great — use them.
    // If not, fall through to the node_call<> primary channel.
    if (NATIVE_TOOL_CALLS_ENABLED || this.isCloudProvider) {
      try {
