...
const SCHEMA_HIDDEN_COMPAT_TOOL_NAMES = new Set([
  ...
  // Connector/MCP runtime tools are intentionally not listed here: connected
  // extension registries are the source of truth for their live schemas.
...
export function buildTools(deps: BuildToolsDeps, activatedCategories?: Set<string>, options: BuildToolsOptions = {}) {
  ...
  toolDefs.push({
    ...
    function: {
      name: 'connector_list',
      description:
        'List all extension-backed connectors discovered by the runtime, their connection status, and the tools exposed by each connected connector. ' +
        'The result is generated from installed manifests and live connector registrations; it is not a provider allowlist. ' +
        'Use this when connection status or the available tool surface matters.',
  ...
  toolDefs.push(
    ...
    },
    {
      ...
      function: {
        name: 'vercel_ops',
        description: 'Unified Vercel connector wrapper for the complete first-class lifecycle surface, plus a constrained native API escape hatch.',
        parameters: {
          type: 'object',
          required: ['action'],
          properties: {
            action: { type: 'string', enum: ['status', 'list_teams', 'list_projects', 'get_project', 'create_project', 'update_project', 'delete_project', 'list_deployments', 'get_deployment', 'create_deployment', 'redeploy', 'deployment_events', 'cancel_deployment', 'delete_deployment', 'list_aliases', 'assign_alias', 'env', 'domains', 'get_project_domain', 'manage_project_domain', 'api_request'] },
            team_id: { type: 'string' },
            ...
            git_ref: { type: 'string' },
            git_sha: { type: 'string' },
            git_repo_id: { type: 'string' },
            git_pr_id: { type: 'string' },
            git_metadata: { type: 'object' },
            project_settings: { type: 'object' },
            custom_environment_slug_or_id: { type: 'string' },
            custom_environment_id: { type: 'string' },
            ...
            with_git_repo_info: { type: 'boolean' },
            with_latest_commit: { type: 'boolean' },
            force_new: { type: 'boolean' },
            build_id: { type: 'string' },
            direction: { type: 'string' },
            since: { type: 'number' },
            until: { type: 'number' },
            redirect_status_code: { type: 'number' },
            limit: { type: 'number' },
          },
        },
      },
