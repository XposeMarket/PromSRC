...
/** @deprecated Optional legacy slice — prefer `file_ops` at config root */
...
/**
 * Reads `file_ops` from config root, with fallback to legacy `orchestration.file_ops`.
 */
export function resolveFileOpSettings(legacy?: OrchestrationLikeConfig | null): FileOpSettings {
  const raw = getConfig().getConfig() as any;
  const f = raw.file_ops ?? legacy?.file_ops ?? raw.orchestration?.file_ops ?? {};
  return {
...
export function canPrimaryApplyFileTool(input: {
  ...
  if (isFileEditTool(toolName)) {
    if (looksRefactorishIntent(input.message)) {
      return { allowed: false, reason: 'refactor-ish request exceeds primary-only limits', estimate };
    }
