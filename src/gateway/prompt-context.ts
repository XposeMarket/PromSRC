...
MANDATORY EDIT ROUTE: For workspace file edits, native workspace wrappers are the default and expected path: workspace_read(action:"stats"/"read"/"grep") first, then workspace_edit(action:"find_replace"/"replace_lines"/"insert_after"/"delete_lines"/"write"/"create") or workspace_edit(action:"patchset") for grouped edits. Do not use terminal/Python/PowerShell/sed/node scripts to edit files unless the user explicitly asks for shell editing or native tools cannot perform the transformation.
WORKSPACE ONLY: In the public app, file tools operate on the user workspace and generated app data only. Always read before editing.`
	...
	SRC/WEB-UI SURFACES (read-only by default): use dev_source_read(action:"list"|"stats"|"stats_batch"|"read"|"batch_read"|"grep"|"search", surface:"src"|"web-ui"|"prom-root", file/path/pattern, ...) to inspect Prometheus src/, web-ui/, and allowlisted prom-root files. For self-edit token savings, prefer dev_source_read(action:"batch_read") for multi-file reads, file_tree(path:"src/..."|"web-ui/...") for compact structure, and search_files(directory:"src/..."|"web-ui/...") when you want search_files-style args that delegate to source grep.
	Write (approved dev source sessions only): use request_dev_source_edit for fast user approval in the current dev chat, or write_proposal execution_mode="code_change" for the full proposal lane. request_dev_source_edit must include a grounded plan: user_request, reasoning, evidence with file/line findings, current_state, fix, steps, expected_workflow, verification, and completion_note_tag (default dev_edit_complete). expected_workflow should explain exactly what happens after approval and after edits apply live: scoped tool unlock, verification/preflight, restart/reload/checkpoint behavior, completion note, and final response shape. After approval, follow that declared plan and prefer dev_source_read(action:"batch_read") plus dev_source_edit(action:"patchset") for src/web-ui file tasks; use scoped workspace wrappers for the approved self docs (self/ and workspace/self/) and tiny source tools only for one-off emergency source edits. Approved source write wrapper actions include dev_source_edit(action:"find_replace"|"replace_lines"|"insert_after"|"delete_lines"|"write"|"create"|"delete_file", surface:"src"|"web-ui", file, ...). Prometheus Mobile is part of the web UI source tree: edit mobile app source under web-ui/src/mobile/*, never hand-edit generated/public-web-ui/static/mobile/* except for emergency verification, and finalize quick live mobile edits with dev_source_edit(action:"apply_live", changed_surfaces:["mobile"], ...). For quick live dev edits, finalize with dev_source_edit(action:"verify_only") then dev_source_edit(action:"apply_live") rather than raw npm run build. After apply_live/restart/reload succeeds, call write_note with the approved completion_note_tag and dev_edit_id so the dev-edit plan can close before the final user summary. Proposal/code_exec lanes are isolated sandboxes: verify them with the canonical terminal build command inside the sandbox, and do not call apply_live there.
	MANDATORY EDIT ROUTE: For workspace file edits, native workspace wrappers are the default and expected path: workspace_read(action:"stats"/"read"/"grep") first, then workspace_edit(action:"find_replace"/"replace_lines"/"insert_after"/"delete_lines"/"write"/"create") or workspace_edit(action:"patchset") for grouped edits. Do not use terminal/Python/PowerShell/sed/node scripts to edit files unless the user explicitly asks for shell editing or native tools cannot perform the transformation.
	EDIT PRIORITY: Always inspect before editing. For source-controlled Prometheus code, prefer file_tree for orientation, dev_source_read(action:"grep"|"search") for call sites, dev_source_read(action:"batch_read") for multi-file inspection, then dev_source_edit(action:"patchset") for approved edits. Use tiny source handlers only as hidden compatibility internals. For normal workspace files, prefer workspace_read(action:"batch_read") for multi-file inspection, then workspace_edit(action:"patchset") for grouped edits; use workspace_read(action:"stats"|"search"|"grep"|"read") and workspace_edit(...) for one-off work.`,
...
SEARCH MODES: quick(default)=fast focused retrieval; deep=broad recall; project=project-scoped; timeline=chronological history.`,
  ...
  agents: `AGENTS: chat_with_subagent(agent_id,message) is core for normal persistent chat/check-ins with a standalone non-team subagent. Activate agents_and_teams for wrappers: agent_ops(action:"list"|"info"|"spawn"|"update"|"delete"|"deploy_analysis_team"), agent_chat_ops(action:"talk"|"message"|"send"|"turn_request"|"reply_wait"|"thread_watch"), team_ops_wrapper(...), and team_collab_ops(...).
RULES: Use chat_with_subagent for check-ins, status questions, planning, or casual collaboration when you know the agent_id. Activate agents_and_teams and call agent_ops(action:"list") first when you need to discover/inspect/create agents. Use agent_chat_ops(action:"message") only when you want a background task handoff whose working conversation/result stays in the subagent task panel.`,
...
[FILE EDIT ROUTING] For workspace edits, activate/use workspace_write/file_ops and follow: workspace_read(action:"stats"/"read"/"grep") first, then workspace_edit(action:"find_replace"/"replace_lines"/"insert_after"/"delete_lines"/"write"/"create") or workspace_edit(action:"patchset"). Do not use workspace_run/terminal/Python/PowerShell/sed/node scripts as the default file editor.
