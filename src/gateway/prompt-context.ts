...
const BG_AGENT_RUNTIME_HINT = `BACKGROUND AGENTS: background_spawn(task_prompt, id?) runs a parallel agent. For ASAP/urgent independent work that needs to start now, use background_spawn instead of spawning a subagent and waiting on its response. Prefer spawning when work is independent and can run concurrently, especially for sidecar tasks (memory updates, repo scans, web lookups, data gathering) while you continue the primary flow. task_prompt must be fully self-contained (no references to "the conversation"). The finalization gate waits for and merges same-turn background_spawn results before the final reply. Use background_wait(wait_ms:30000) when you intentionally want to pause this foreground turn while active background agents work before continuing. Don't call background_join/status unless explicitly needed. Background agents use bg_plan_declare/bg_plan_advance for their own planning.`;
