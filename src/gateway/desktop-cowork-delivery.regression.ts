...
async function main(): Promise<void> {
  ...
  const weakTarget = await runDesktopDeliveryWithFallback({
    ...
    background: async () => ({ value: 'should-not-run' }),
    foreground: async () => ({ value: 'compat' }),
  });
  ...
  await assert.rejects(
    () => runDesktopDeliveryWithFallback({
      ...
      background: async () => ({ value: 'nope' }),
      foreground: async () => ({ value: 'nope' }),
    }),
    DesktopBackgroundDeliveryUnsupportedError,
  );
  ...
  const explicitForeground = await runDesktopDeliveryWithFallback({
    ...
    background: async () => ({ value: 'wrong' }),
    foreground: async () => ({ value: 'foreground', verified: true }),
  });
