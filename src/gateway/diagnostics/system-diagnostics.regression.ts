...
function main(): void {
  ...
  const rows = [
    ...
    // Live runtimes are also present in the durable ledger under the same ID.
    // The duplicate must not inflate count/items, and the first (live) row wins.
    { id: 'active-1', status: 'running', updatedAt: now - 10_000 },
  ];

  const summary = summarizeRuntimeDiagnostics(rows, now, 2, 'summary');
  assert.equal(summary.count, 3, 'live + durable copies of the same runtime must count once');
  assert.equal(summary.interruptedCount, 1, 'fault counts must cover every unique observed runtime, not only displayed rows');
  ...
  assert.equal(full.items[0]?.ageMs, 1_000, 'deduplication must keep the first live snapshot instead of the stale durable copy');
