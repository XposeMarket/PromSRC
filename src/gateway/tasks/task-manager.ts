...
export async function callManagerBrief(input: {
  ...
  const xTaskHint = isXTask
    ? `X.COM / TWITTER NOTE FOR BRIEF: To compose a tweet, the worker MUST use this exact 3-step sequence: (1) browser_open("https://x.com/home") — the result will include a SITE SHORTCUTS block showing keyboard shortcuts including "n → Open new tweet composer modal". (2) browser_press_key({"key":"n"}) — this opens the composer modal immediately on any X page, no scrolling needed. (3) browser_fill(ref, tweet_text) — find the contenteditable composer ref in the snapshot after pressing n, and fill it. The fill auto-submits via the Post button. Forbidden actions: browser_snapshot after browser_open (snapshot is already in the result), ANY PageDown/scroll before filling, opening a profile URL like x.com/SmallClaw (that is NOT the home page and has no composer). The exact_objective MUST say: "Press n to open composer, then browser_fill with the tweet text." The stop_condition MUST be: "browser_fill returned confirmation that the tweet was posted."`
    : '';
