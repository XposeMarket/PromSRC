  ...
  const tools = [
    {
      ...
      function: {
        ...
        description: 'Unified Creative project/workspace wrapper for mode, project state, references, history, scene reset/save/export, templates, and storyboard/project lifecycle actions.',
        parameters: {
          type: 'object',
          required: ['action'],
          properties: {
            action: { type: 'string', enum: ['get_mode', 'switch_mode', 'list_references', 'get_state', 'create_project', 'project_history', 'create_storyboard', 'storyboard_history', 'save_scene', 'export', 'export_trace', 'checkpoint', 'history_status', 'undo', 'redo', 'reset_scene', 'purge_scene', 'create_library_pack', 'list_library_packs', 'toggle_library_pack'] },
            mode: { type: ['string', 'null'], enum: [...CREATIVE_MODE_ENUM, null] },
            reason: { type: 'string' },
    ...
    },
    {
      ...
      function: {
        name: 'creative_scene',
        description: 'Unified Creative scene/element/style wrapper for image/canvas operations, element inventory, styling, layout, effects, templates, and scene composition.',
        parameters: {
          type: 'object',
          required: ['action'],
          properties: {
            action: { type: 'string', enum: ['apply_ops', 'set_canvas', 'add_element', 'update_element', 'delete_element', 'select_element', 'element_inventory', 'apply_animation', 'arrange', 'apply_style', 'apply_template', 'add_effect', 'add_mask', 'set_blend_mode', 'apply_brand_kit', 'search_animations', 'chain_scene'] },
    ...
    },
    {
      type: 'function',
      function: {
        name: 'creative_image_ops',
        description: 'Unified Creative image/asset wrapper for importing, searching, generating, analyzing, fitting, extracting layers, icons, ASCII assets, model status, and generation history.',
        parameters: {
          type: 'object',
          required: ['action'],
          properties: {
            action: { type: 'string', enum: ['add_asset', 'import_asset', 'analyze_asset', 'extract_layers', 'extract_layers_for_generation', 'model_status', 'search_assets', 'generate_asset', 'register_generation', 'generation_history', 'fit_asset', 'search_icons', 'render_ascii_asset', 'normalize_layer_specs'] },
            assetId: { type: 'string' },
            asset_id: { type: 'string' },
            source: { type: 'string', description: 'Source image/media for import_asset, analyze_asset, extract_layers, and extract_layers_for_generation. Aliases: path, url, sourceImage, image, inputs.sourceImage, inputs.image.' },
            path: { type: 'string', description: 'Local workspace path alias for source.' },
            url: { type: 'string', description: 'Remote URL alias for source where supported.' },
            sourceImage: { type: 'string', description: 'Image source alias for layer extraction/generation workflows.' },
            source_image: { type: 'string', description: 'Snake-case image source alias.' },
            image: { type: 'string', description: 'Image source alias.' },
            query: { type: 'string' },
            prompt: { type: 'string' },
            id: { type: 'string' },
            inputs: { type: 'object', description: 'Optional nested args. Source may be supplied as inputs.source, inputs.path, inputs.url, inputs.sourceImage, or inputs.image.' },
    ...
    },
    {
      type: 'function',
      function: {
        name: 'creative_video_ops',
        description: 'Unified Creative video wrapper for shots, sequences, clips, timeline/composition, audio, captions, motion templates, rough cuts, snapshots, and rendering.',
        parameters: {
          type: 'object',
          required: ['action'],
          properties: {
            action: { type: 'string', enum: ['write_shot_prompt', 'extract_video_frame', 'extract_video_frames', 'generate_image_shot', 'generate_video_shot', 'refine_video_shot', 'generate_sequence', 'pick_continuity_frame', 'wrap_video_as_html_motion_clip', 'add_generated_clip_to_composition', 'render_generated_sequence', 'stitch_clips', 'auto_assemble_rough_cut', 'import_audio', 'download_audio', 'extract_audio_from_video', 'generate_voiceover', 'transcribe_audio', 'sync_captions_to_audio', 'add_audio_track', 'mix_audio_tracks', 'add_music_bed', 'add_sound_effects', 'generate_motion_graphics_layer', 'composite_video_layers', 'attach_audio_from_file', 'attach_audio_from_url', 'trim_clip', 'list_motion_templates', 'preview_motion_template', 'apply_motion_template', 'generate_motion_variants', 'composition_get', 'composition_save', 'composition_add_track', 'composition_add_clip', 'composition_select_clip', 'composition_move_clip', 'composition_trim_clip', 'composition_split_at', 'composition_set_transition', 'composition_delete_clip', 'composition_render', 'timeline', 'render_snapshot'] },
            id: { type: 'string' },
            clipId: { type: 'string' },
            clip_id: { type: 'string' },
            trackId: { type: 'string' },
    ...
    },
    {
      type: 'function',
      function: {
        name: 'creative_hyperframes_ops',
        description: 'Unified HyperFrames/HTML Motion wrapper for catalog, clip insertion, patching, variables, QA, materialize/export, and HTML motion templates/blocks.',
        parameters: {
          type: 'object',
          required: ['action'],
          properties: {
            action: { type: 'string', enum: ['browse_catalog', 'insert_clip', 'apply_patch', 'set_text', 'set_color', 'set_timing', 'set_variable', 'set_asset', 'add_animation', 'lint', 'qa', 'materialize', 'export', 'list_components', 'import_component', 'sync_catalog', 'apply_component', 'overlay_on_video', 'list_templates', 'apply_template', 'create_clip', 'save_template', 'save_block', 'promote_scene_to_template', 'list_blocks', 'render_block', 'read_clip', 'patch_clip', 'restore_revision', 'render_snapshot', 'export_clip'] },
            id: { type: 'string' },
            clipId: { type: 'string' },
            clip_id: { type: 'string' },
    ...
    },
    {
      type: 'function',
      function: {
        name: 'creative_quality_ops',
        description: 'Unified Creative quality/QA wrapper for image, video, layout, composition, text fit, frame, and generated-video checks.',
        parameters: {
          type: 'object',
          required: ['action'],
          properties: {
            action: { type: 'string', enum: ['quality_report', 'validate_layout', 'validate_composition_layers', 'preflight_overlay', 'sample_composite_frames', 'frame_trace', 'frame_diff', 'analyze_generated_video', 'compare_shots', 'select_best_take', 'retry_shot_until_pass', 'lint_html_motion_clip', 'measure_text', 'text_fit_report', 'composition_lint', 'image_get_element_at_point', 'image_get_overlaps', 'image_get_bounds_summary', 'image_check_text_overflow', 'image_check_contrast', 'image_detect_empty_regions', 'video_render_frame', 'video_render_contact_sheet', 'video_analyze_frame', 'video_analyze_timeline', 'video_check_keyframes', 'video_check_caption_timing', 'video_check_audio_sync', 'video_extract_clip_frames', 'video_analyze_imported_video'] },
            id: { type: 'string' },
            path: { type: 'string' },
            x: { type: 'number' },
            y: { type: 'number' },
      ...
      },
    {
      ...
      function: {
        ...
        description: 'Import a local workspace image/video/audio/SVG/Lottie asset into the Creative asset library, analyze it, and add it to the searchable asset index. Use before placing reusable footage, screenshots, overlays, mockups, or brand files.',
          parameters: {
            type: 'object',
            properties: {
              source: { type: 'string', description: 'Workspace path, absolute workspace-contained path, or remote URL to register. Aliases: path, url, inputs.source, inputs.path.' },
              path: { type: 'string', description: 'Local workspace path alias for source.' },
              url: { type: 'string', description: 'Remote URL alias for source.' },
              assetId: { type: 'string', description: 'Existing Creative asset id/name/path to re-import or reuse as source.' },
              asset_id: { type: 'string', description: 'Snake-case alias for assetId.' },
              inputs: { type: 'object', description: 'Optional nested source args.' },
      ...
      },
      {
        ...
        function: {
          ...
          parameters: {
            type: 'object',
            properties: {
              source: { type: 'string', description: 'Workspace path or absolute workspace-contained path to the source image. Aliases: path, sourceImage, image, inputs.sourceImage, inputs.image, assetId.' },
              path: { type: 'string', description: 'Local workspace path alias for source.' },
              sourceImage: { type: 'string', description: 'Image source alias.' },
              source_image: { type: 'string', description: 'Snake-case image source alias.' },
              image: { type: 'string', description: 'Image source alias.' },
              assetId: { type: 'string', description: 'Existing Creative asset id/name/path to use as source.' },
              asset_id: { type: 'string', description: 'Snake-case alias for assetId.' },
              inputs: { type: 'object', description: 'Optional nested source args.' },
              ...
              replaceScene: { type: 'boolean', description: 'Reset the current Image scene before applying extracted layers. Defaults true.' },
                ...
                saveLayerAssets: { type: 'boolean', description: 'After extraction, automatically save each extracted image/cutout layer as a separate PNG asset under Creative assets/layers/<batch>. Use this for sprite/game asset workflows. Defaults false.' },
                autoSaveLayerAssets: { type: 'boolean', description: 'Alias for saveLayerAssets.' },
                layerAssetBatchName: { type: 'string', description: 'Optional folder/batch name for auto-saved layer PNG assets.' },
      ...
      },
      {
        ...
        function: {
          ...
          parameters: {
            type: 'object',
            properties: {
              source: { type: 'string', description: 'Workspace path, absolute workspace-contained path, or remote URL to analyze. Aliases: path, url, inputs.source, inputs.path, assetId.' },
              path: { type: 'string', description: 'Local workspace path alias for source.' },
              url: { type: 'string', description: 'Remote URL alias for source.' },
              assetId: { type: 'string', description: 'Existing Creative asset id/name/path to analyze.' },
              asset_id: { type: 'string', description: 'Snake-case alias for assetId.' },
              inputs: { type: 'object', description: 'Optional nested source args.' },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_extract_video_frame',
          description: 'Extract one PNG frame from a local workspace video into Creative storage. Supports frame:first/middle/last/best_continuity_frame, timestamp seconds, timestampMs, or percent. Optionally imports the frame into the Creative asset library.',
          parameters: {
            type: 'object',
            required: ['source'],
            properties: {
              source: { type: 'string', description: 'Workspace path or Creative asset path for the source video.' },
              video: { type: 'string', description: 'Alias for source.' },
              frame: { type: 'string', enum: ['first', 'middle', 'last', 'best_continuity_frame'] },
              timestamp: { type: 'number', description: 'Timestamp in seconds.' },
              timestampMs: { type: 'number', description: 'Timestamp in milliseconds.' },
              percent: { type: 'number', description: 'Position in the video, either 0-100 or 0-1.' },
              outputName: { type: 'string' },
              registerAsAsset: { type: 'boolean', description: 'Import the extracted frame into the Creative asset index. Defaults true.' },
              tags: { type: ['array', 'string'], items: { type: 'string' } },
            },
            additionalProperties: false,
          },
      ...
      },
      {
        ...
        function: {
          ...
          description: 'Create a persistent Creative Generation project manifest that owns brief, target format/duration, storyboard reference, selected takes, assets, extracted layers, rough cuts, exports, and QA reports.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
              ...
              aspectRatio: { type: 'string', enum: ['landscape', 'square', 'portrait'] },
              ...
              storyboard: { type: 'object', description: 'Optional inline storyboard seed with title/shots/styleGuide/characterBible.' },
              sourceAssets: { type: ['array', 'string'], items: { type: 'string' } },
              notes: { type: 'object' },
            },
            additionalProperties: false,
          },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_project_history',
          description: 'List Creative Generation projects, or load one full project manifest by projectId/path.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_create_storyboard',
          description: 'Create a persistent Creative storyboard JSON with shot ids, prompts, camera/action notes, QA criteria, style guide, and character bible notes.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_storyboard_history',
          description: 'List Creative storyboards, or load one storyboard by storyboardId/path.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_write_shot_prompt',
          description: 'Turn structured shot notes into a cleaner video-generation prompt, negative prompt, and per-shot QA criteria.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
              ...
              endingFrameGoal: { type: 'string' },
              negatives: { type: ['array', 'string'], items: { type: 'string' } },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_extract_video_frames',
          description: 'Extract multiple PNG frames from a local workspace video into Creative storage, optionally generating a contact sheet and importing the frames into the Creative asset library.',
          parameters: {
            type: 'object',
            required: ['source'],
            properties: {
              source: { type: 'string' },
              video: { type: 'string', description: 'Alias for source.' },
              frames: { type: 'array', items: {}, description: 'Array of frame selectors: strings like first/last, timestamps as numbers, or objects with frame/timestamp/timestampMs/percent.' },
              timestamps: { type: 'array', items: { type: 'number' }, description: 'Timestamps in seconds.' },
              percents: { type: 'array', items: { type: 'number' }, description: 'Positions in the video, 0-100 or 0-1.' },
              count: { type: 'number', description: 'Uniform frame count when explicit frames are not provided. Defaults 6.' },
              rangeStartPercent: { type: 'number' },
              rangeEndPercent: { type: 'number' },
              outputNamePrefix: { type: 'string' },
              registerAsAssets: { type: 'boolean', description: 'Import extracted frames into the Creative asset index. Defaults true.' },
              contactSheet: { type: 'boolean', description: 'Generate a contact sheet. Defaults true.' },
              tags: { type: ['array', 'string'], items: { type: 'string' } },
            },
            additionalProperties: false,
          },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_register_generation',
          description: 'Record generation lineage for an image/video/frame/audio asset: prompt, provider, model, source assets, parent generation, shot id, attempt, output asset, and metadata.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
              kind: { type: 'string', enum: ['image', 'video', 'frame', 'audio', 'other'] },
              shotId: { type: 'string' },
              attempt: { type: 'number' },
              prompt: { type: 'string' },
              ...
              outputPath: { type: 'string', description: 'Generated file path to import/register.' },
              metadata: { type: 'object' },
              tags: { type: ['array', 'string'], items: { type: 'string' } },
            },
            additionalProperties: false,
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_generation_history',
          description: 'List recorded Creative generation lineage records, optionally filtered by shotId.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_generate_image_shot',
          description: 'Generate a production-ready image shot/keyframe/background/asset, import it into Creative assets, and register shot lineage with prompt, references, output role, and parent generation links.',
          parameters: {
            type: 'object',
            required: ['prompt'],
            properties: {
              ...
              aspectRatio: { type: 'string', enum: ['landscape', 'square', 'portrait'] },
              referenceImages: { type: 'array', items: { type: 'string' } },
              characterReferences: { type: 'array', items: { type: 'string' } },
              locationReferences: { type: 'array', items: { type: 'string' } },
              negativePrompt: { type: 'string' },
              seed: { type: ['string', 'number'] },
              continuityId: { type: 'string' },
              outputRole: { type: 'string', enum: ['opening_frame', 'keyframe', 'ending_frame', 'asset', 'background_plate'] },
              provider: { type: 'string', enum: ['auto', 'openai', 'openai_codex', 'xai'] },
              model: { type: 'string' },
              count: { type: 'number' },
              outputDir: { type: 'string' },
              parentGenerationId: { type: 'string' },
              parentAssetId: { type: 'string' },
              importToCreative: { type: 'boolean' },
            },
            additionalProperties: false,
          },
        },
      },
      {
        type: 'function',
        function: {
          name: 'creative_generate_video_shot',
          description: 'Generate a video shot through the existing video provider, import/analyze it as a Creative asset, and record prompt/provider/source lineage in the Creative generation index.',
          parameters: {
            type: 'object',
            required: ['prompt'],
            properties: {
              prompt: { type: 'string' },
              shotId: { type: 'string' },
              image: { type: 'string', description: 'Optional start image for image-to-video.' },
              referenceImages: { type: 'array', items: { type: 'string' } },
              video: { type: 'string', description: 'Optional source video for edit/extend.' },
              mode: { type: 'string', enum: ['generate', 'edit', 'extend'] },
              aspectRatio: { type: 'string', enum: ['landscape', 'square', 'portrait'] },
              duration: { type: 'number' },
              resolution: { type: 'string', enum: ['480p', '720p'] },
              provider: { type: 'string', enum: ['auto', 'xai'] },
              model: { type: 'string' },
              outputDir: { type: 'string' },
              parentGenerationId: { type: 'string' },
              parentAssetId: { type: 'string' },
              importToCreative: { type: 'boolean', description: 'Import generated MP4 into the Creative asset library. Defaults true.' },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_analyze_generated_video',
          description: 'Analyze a generated video by sampling frames, creating a contact sheet, scoring basic usability, and returning warnings/retry recommendations. This is Phase 2 QA plumbing; semantic review may still require vision/human review.',
          parameters: {
            type: 'object',
            required: ['source'],
            properties: {
              source: { type: 'string' },
              video: { type: 'string', description: 'Alias for source.' },
              intendedPrompt: { type: 'string' },
              continuityTarget: { type: 'string' },
              qaCriteria: { type: ['array', 'string'], items: { type: 'string' } },
              frameCount: { type: 'number' },
              useVision: { type: 'boolean', description: 'Use active vision-capable model for semantic QA. Defaults true.' },
            },
            additionalProperties: false,
          },
        },
      },
      {
        type: 'function',
        function: {
          name: 'creative_compare_shots',
          description: 'Analyze two or more generated video takes and select the strongest take using the Phase 2 generated-video QA scoring and contact sheets.',
          parameters: {
            type: 'object',
            required: ['videos'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_select_best_take',
          description: 'Select the best registered video generation take for a shotId by analyzing generation history and returning the highest-scoring take.',
          parameters: {
            type: 'object',
            required: ['shotId'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_retry_shot_until_pass',
          description: 'Generate/retry a video shot up to maxRetries, analyze each take, and return the first passing take or the highest-scoring attempt.',
          parameters: {
            type: 'object',
            required: ['prompt'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_refine_video_shot',
          description: 'Targeted video repair loop for bad hands/faces, weak motion, wrong character, style mismatch, camera drift, bad ending frame, extension, loopability, or cinematic polish. Extracts/uses a keyframe, generates a corrected clip, and QA-checks it.',
          parameters: {
            type: 'object',
            required: ['sourceVideo'],
            properties: {
              sourceVideo: { type: 'string' },
              video: { type: 'string', description: 'Alias for sourceVideo.' },
              issueMode: { type: 'string', enum: ['motion_too_weak', 'wrong_character', 'bad_hands', 'bad_face', 'camera_drift', 'not_following_action', 'style_mismatch', 'extend_action', 'replace_ending', 'make_loopable', 'make_more_cinematic'] },
              issueDescription: { type: 'string' },
              desiredCorrection: { type: 'string' },
              shotId: { type: 'string' },
              keyframe: { type: 'string' },
              frameStrategy: { type: 'string', enum: ['last', 'best_ending_frame', 'timestamp', 'percent'] },
              timestamp: { type: 'number' },
              percent: { type: 'number' },
              preserveStyle: { type: 'boolean' },
              preserveCharacter: { type: 'boolean' },
              preserveLocation: { type: 'boolean' },
              duration: { type: 'number' },
              resolution: { type: 'string', enum: ['480p', '720p'] },
              aspectRatio: { type: 'string', enum: ['landscape', 'square', 'portrait'] },
              provider: { type: 'string', enum: ['auto', 'xai'] },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_generate_sequence',
          description: 'Generate all shots in a storyboard/project using image-first, video-only, or chained-continuity mode; QA/retry each shot, select takes, update storyboard/project state, and return failures. Supports dryRun for planning without spending generation calls.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
              ...
              storyboardId: { type: 'string' },
              storyboardPath: { type: 'string' },
              generationMode: { type: 'string', enum: ['image-first', 'video-only', 'chained-continuity'] },
              maxRetriesPerShot: { type: 'number' },
              qaThreshold: { type: 'number' },
              provider: { type: 'string', enum: ['auto', 'xai'] },
              model: { type: 'string' },
              duration: { type: 'number' },
              resolution: { type: 'string', enum: ['480p', '720p'] },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_extract_layers_for_generation',
          description: 'Run creative_extract_layers on a generated/uploaded image and register the resulting scene plus reusable cutout layers into generation lineage for avatar, game asset, and promo workflows.',
          parameters: {
            type: 'object',
            properties: {
              source: { type: 'string', description: 'Source image path for layer extraction. Aliases: path, sourceImage, image, inputs.sourceImage, inputs.image, assetId.' },
              path: { type: 'string', description: 'Local workspace path alias for source.' },
              sourceImage: { type: 'string', description: 'Image source alias.' },
              source_image: { type: 'string', description: 'Snake-case image source alias.' },
              image: { type: 'string', description: 'Image source alias.' },
              ...
              extractObjects: { type: 'boolean' },
              maxTextLayers: { type: 'number' },
              maxShapeLayers: { type: 'number' },
              useVision: { type: 'boolean' },
              useOcr: { type: 'boolean' },
              useSam: { type: 'boolean' },
              inpaintBackground: { type: 'boolean' },
              vectorTraceShapes: { type: 'boolean' },
              saveLayerAssets: { type: 'boolean', description: 'After extraction, automatically save each extracted image/cutout layer as a separate PNG asset under Creative assets/layers/<batch>. Use this for sprite/game asset workflows. Defaults false.' },
              autoSaveLayerAssets: { type: 'boolean', description: 'Alias for saveLayerAssets.' },
              layerAssetBatchName: { type: 'string', description: 'Optional folder/batch name for auto-saved layer PNG assets.' },
            },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_pick_continuity_frame',
          description: 'Extract candidate frames from a video and pick a practical continuity frame for scene chaining. Current scoring is deterministic and heuristic; it also produces a contact sheet for review.',
          parameters: {
            type: 'object',
            required: ['source'],
            properties: {
              source: { type: 'string' },
              video: { type: 'string', description: 'Alias for source.' },
              strategy: { type: 'string', enum: ['ending', 'uniform'] },
              candidatePercents: { type: 'array', items: { type: 'number' } },
              registerAsAsset: { type: 'boolean', description: 'Import selected frame into the Creative asset index. Defaults true.' },
              continuityPrompt: { type: 'string', description: 'Next action/continuity target used by vision scoring.' },
              useVision: { type: 'boolean', description: 'Use model-assisted frame selection. Defaults true.' },
            },
            additionalProperties: false,
          },
        },
      },
      {
        type: 'function',
        function: {
          name: 'creative_chain_scene',
          description: 'Generate the next AI video scene from a previous clip by extracting a continuity frame, using it as the next image-to-video start frame, registering lineage, and optionally rendering a stitched rough cut.',
          parameters: {
            type: 'object',
            required: ['previousVideo', 'nextPrompt'],
            properties: {
              previousVideo: { type: 'string' },
              ...
              model: { type: 'string' },
              parentGenerationId: { type: 'string' },
              stitch: { type: 'boolean', description: 'Render a two-clip rough cut after generation. Defaults false.' },
              outputDir: { type: 'string' },
            },
            additionalProperties: false,
          },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_wrap_video_as_html_motion_clip',
          description: 'Wrap a raw MP4/WebM/MOV as a self-contained HTML Motion clip so generated videos can enter the existing Creative/HTML Motion/HyperFrames render path without adding a native video lane yet.',
          parameters: {
            type: 'object',
            required: ['source'],
            properties: {
              ...
              importToCreative: { type: 'boolean', description: 'Copy/import source video into Creative asset library before wrapping. Defaults true.' },
            },
            additionalProperties: false,
          },
        },
      },
      {
        type: 'function',
        function: {
          name: 'creative_add_generated_clip_to_composition',
          description: 'Wrap a generated/raw video as HTML Motion and add it to the active Video composition timeline as an html-motion clip.',
          parameters: {
            type: 'object',
            required: ['source'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_render_generated_sequence',
          description: 'Render a list of generated/raw video clips into one rough-cut MP4/WebM by wrapping each video as HTML Motion and using the backend composition renderer. Does not require direct canvas work.',
          parameters: {
            type: 'object',
            required: ['videos'],
            properties: {
              ...
              transition: { type: 'string', enum: ['cut', 'crossfade', 'fade', 'dip_to_black'] },
              transitionDurationMs: { type: 'number' },
              audioHandling: { type: 'string', enum: ['mute', 'first'] },
              renderMode: { type: 'string', enum: ['ffmpeg', 'html-motion'] },
            },
            additionalProperties: false,
          },
        },
      },
      {
        type: 'function',
        function: {
          name: 'creative_stitch_clips',
          description: 'Editor-grade FFmpeg stitching for generated/raw clips with per-clip trims, normalization, cut/crossfade/dip-to-black transitions, optional first-clip audio, and Creative asset registration.',
          parameters: {
            type: 'object',
            required: ['clips'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_auto_assemble_rough_cut',
          description: 'Assemble selected takes from a project/storyboard or explicit videos into a rough-cut MP4/WebM, applying simple editorial trims, normalization, transitions, timeline JSON export, and project rough-cut/export registration.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
              ...
              transition: { type: 'string', enum: ['cut', 'crossfade', 'fade', 'dip_to_black'] },
              transitionDurationMs: { type: 'number' },
              defaultTrimStartMs: { type: 'number' },
              defaultTrimEndMs: { type: 'number' },
              format: { type: 'string', enum: ['mp4', 'webm'] },
              audioHandling: { type: 'string', enum: ['mute', 'first'] },
            },
            additionalProperties: false,
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_normalize_layer_specs',
          description: 'Normalize raw overlay/composite layer specs into strict CreativeLayerSpec objects with global width/height/duration, timing, z-index, opacity, fit, blend mode, bounds, slots, and metadata.',
          parameters: {
            type: 'object',
            required: ['layers'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_validate_composition_layers',
          description: 'Validate normalized overlay/composite layers before render: missing assets, duplicate IDs, timing overflow, safe-area issues, unsupported/nondeterministic HTML, editability metadata, and complexity score.',
          parameters: {
            type: 'object',
            required: ['layers'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_preflight_overlay',
          description: 'Preflight overlay/composite layers and optionally write a deterministic browser-preview HTML file before rendering/export.',
          parameters: {
            type: 'object',
            required: ['layers'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_sample_composite_frames',
          description: 'Render deterministic sample PNG frames/contact sheet from overlay/composite layer specs using the Creative browser path, so layout/timing/path problems are caught before export.',
          parameters: {
            type: 'object',
            required: ['layers'],
            properties: {
              ...
              height: { type: 'number' },
              durationMs: { type: 'number' },
              timestampsMs: { type: 'array', items: { type: 'number' } },
              count: { type: 'number' },
            },
            additionalProperties: false,
          },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_import_audio',
          description: 'Import a local audio file into Creative storage, analyze waveform/duration, register lineage, and optionally attach it to a Creative project.',
          parameters: {
            type: 'object',
            required: ['source'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_download_audio',
          description: 'Download audio from a supported media page URL using yt-dlp audio-only mode, import it as a Creative audio asset, analyze it, and register lineage.',
          parameters: {
            type: 'object',
            required: ['url'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_extract_audio_from_video',
          description: 'Extract the audio stream from a local video into MP3/WAV/M4A, import it, analyze it, and register Creative lineage.',
          parameters: {
            type: 'object',
            required: ['source'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_generate_voiceover',
          description: 'Generate a voiceover audio file through the configured OpenAI/OpenAI Realtime-compatible TTS or xAI voice provider, save it, import it, analyze it, and register lineage. Provider auto uses available API keys or OAuth bridges and falls back to xAI when OpenAI is unavailable.',
          parameters: {
            type: 'object',
            required: ['text'],
            properties: {
              text: { type: 'string' },
              provider: { type: 'string', enum: ['auto', 'openai', 'openai_realtime', 'xai'] },
              voice: { type: 'string' },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_transcribe_audio',
          description: 'Transcribe a local audio file with OpenAI or xAI STT so captions can be generated and synced.',
          parameters: {
            type: 'object',
            required: ['source'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_sync_captions_to_audio',
          description: 'Create an editable HTML Motion caption layer from transcript text or timed segments, optionally tied to an audio source and project.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_add_audio_track',
          description: 'Prepare a timed Creative audio track with trim, volume, mute, and fade settings, and optionally attach it to a project manifest.',
          parameters: {
            type: 'object',
            required: ['source'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_mix_audio_tracks',
          description: 'Mix multiple timed Creative audio tracks into a single mastered audio asset using FFmpeg, with start offsets, trims, volume, and fades.',
          parameters: {
            type: 'object',
            required: ['tracks'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_add_music_bed',
          description: 'Import or download a music bed, analyze it, and create a low-volume faded Creative audio track for a project or composition.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_add_sound_effects',
          description: 'Add timed sound effect tracks and optionally mix them into a single SFX audio asset.',
          parameters: {
            type: 'object',
            required: ['effects'],
            properties: {
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_generate_motion_graphics_layer',
          description: 'Generate an editable HTML Motion/HyperFrames-style overlay layer for captions, lower thirds, callouts, HUDs, data cards, logo intros, CTA outros, subtitles, app annotations, or title cards. Runs preflight and optional frame sampling.',
          parameters: {
            type: 'object',
            required: [],
            properties: {
              mode: { type: 'string', enum: ['captions', 'lower_third', 'callout', 'hud', 'data_card', 'logo_intro', 'cta_outro', 'subtitle', 'app_annotation', 'title_card'] },
              text: { type: 'string' },
      ...
      },
      {
        type: 'function',
        function: {
          name: 'creative_overlay_hyperframes_on_video',
          description: 'Overlay editable HTML Motion/HyperFrames markup onto a base video with timing, bounds, opacity/blend, preflight validation, sample frames, and final MP4/WebM export.',
          parameters: {
            type: 'object',
            required: ['baseVideo'],
            properties: {
              baseVideo: { type: 'string' },
              video: { type: 'string', description: 'Alias for baseVideo.' },
              overlayHtml: { type: 'string' },
              overlayPath: { type: 'string' },
              startMs: { type: 'number' },
              durationMs: { type: 'number' },
              position: { type: 'string', enum: ['full', 'top', 'bottom', 'custom'] },
              bounds: { type: 'object' },
              blendMode: { type: 'string', enum: ['normal', 'multiply', 'screen', 'overlay', 'lighten', 'darken'] },
              opacity: { type: 'number' },
              width: { type: 'number' },
              height: { type: 'number' },
              frameRate: { type: 'number' },
              ...
              filename: { type: 'string' },
              format: { type: 'string', enum: ['mp4', 'webm'] },
              sampleBeforeRender: { type: 'boolean' },
            },
            additionalProperties: false,
          },
        },
      },
      {
        type: 'function',
        function: {
          name: 'creative_composite_video_layers',
          description: 'Composite arbitrary video, image, HTML Motion, HyperFrames, caption, shape, and audio layer specs into a final MP4/WebM using the shared layer guardrails, preflight, sample frames, manifest, and Creative render path.',
          parameters: {
            type: 'object',
            required: ['layers'],
            properties: {
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            height: { type: 'number' },
            frameRate: { type: 'number' },
            activate: { type: 'boolean', description: 'Also save an HTML Motion copy. Defaults false so the HyperFrames canvas is preserved.' },
            activateHtmlMotion: { type: 'boolean', description: 'Alias for activate.' },
            replaceCanvas: { type: 'boolean', description: 'When activating, replace the current canvas with the HTML Motion copy. Defaults false.' },
            updateScene: { type: 'boolean', description: 'Alias for replaceCanvas.' },
          },
          additionalProperties: false,
        },
      },
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            filename: { type: 'string' },
            frameRate: { type: 'number' },
            maxFrames: { type: 'number', description: 'Optional smoke-test cap for debug renders.' },
            forceHighFps: { type: 'boolean', description: 'Allow high-frame-count exports when intentionally rendering at high fps.' },
            perFrameTimeoutMs: { type: 'number' },
            skipSpatialQa: { type: 'boolean' },
            renderer: { type: 'string', enum: ['html-motion', 'producer'] },
            engine: { type: 'string', enum: ['html-motion', 'producer'] },
            producer: { type: 'boolean', description: 'Use @hyperframes/producer explicitly. Defaults true for native HyperFrames export; set false or engine:"html-motion" for the legacy fallback renderer.' },
          },
          additionalProperties: false,
        },
      },
    ...
    },
    {
      ...
      function: {
        ...
        description: 'Place an HTML Motion, Remotion, or native HyperFrames clip on a track at a specific master-timeline position. Scene-graph clips are not part of Video mode.',
        parameters: {
          type: 'object',
          required: ['lane', 'source'],
          properties: {
            trackId: { type: 'string' },
            lane: { type: 'string', enum: ['html-motion', 'remotion', 'hyperframes'] },
            source: { type: 'object', description: 'Source descriptor matching the lane: { kind:"html-motion", clipPath }, { kind:"remotion", templateId, presetId?, input }, or { kind:"hyperframes", html?, projectPath?, entryFile?, compositionId?, variables? }.' },
