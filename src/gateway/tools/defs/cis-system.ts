...
export function getCisSystemTools(): any[] {
  ...
  const proposalDescription = devToolsVisible
    ? 'Submit a human-approved proposal. Choose one execution_mode: general (read/research/internal orchestration), action (user-world work), or code_change (Prometheus src/web-ui self-edit). General plan proposals may stay broad and concise; include execution_steps only when approval should launch follow-up work. Action/code_change proposals retain their strict implementation contracts.'
    : 'Submit a human-approved proposal for user-work or internal orchestration. Prometheus self-edit/code-change proposals are hidden in workspace-first mode. General plan proposals may stay broad and concise; include execution_steps only when approval should launch follow-up work.';
  ...
  const tools = [
    ...
    },
    {
      ...
      function: {
        ...
        description:
          ...
          'Use to enrich third-party skills with version, legacy triggers, structured prompt signals, categories, required tools, permissions, and curated resources.',
        parameters: {
          type: 'object',
          ...
          properties: {
            ...
            promptSignalPositivePrompts: { type: 'array', items: { type: 'string' }, description: 'Optional evaluation prompts for structured prompt-signal changes.' },
            promptSignalNegativePrompts: { type: 'array', items: { type: 'string' }, description: 'Optional negative evaluation prompts for structured prompt-signal changes.' },
          },
        },
      },
    },
      ...
      function: {
        ...
        parameters: {
          ...
          properties: {
            ...
            promptSignals: { type: 'object', description: 'Structured routing policy: phrases, allOf term groups, anyOf terms, noneOf exclusions, and minScore.' },
            triggerPositivePrompts: { type: 'array', items: { type: 'string' }, description: 'Required when triggers or promptSignals are provided.' },
            triggerNegativePrompts: { type: 'array', items: { type: 'string' }, description: 'Required when triggers or promptSignals are provided.' },
            categories: { type: 'string', description: 'Comma-separated categories.' },
            requiredTools: { type: 'string', description: 'Comma-separated required tool/category names.' },
    ...
    },
    {
      ...
      function: {
        ...
        description:
          'Update one skill\'s discovery metadata (description, legacy triggers, structured prompt signals, categories, requiredTools, lifecycle, name) via a non-destructive Prometheus manifest overlay. Snapshots and a change-ledger entry are recorded automatically; the original SKILL.md/downloaded folder is not modified. Returns the new quality score. Prefer this over hand-editing SKILL.md frontmatter for metadata fixes.',
        parameters: {
          type: 'object',
          required: ['id'],
          properties: {
            ...
            promptSignals: { type: 'object', description: 'Replace structured routing signals with phrases, allOf, anyOf, noneOf, and minScore.' },
            categories: { type: 'string', description: 'Comma-separated categories.' },
            requiredTools: { type: 'string', description: 'Comma-separated required tool/category names.' },
            ...
            triggerPositivePrompts: { type: 'array', items: { type: 'string' }, description: 'Required when triggers or promptSignals change.' },
            triggerNegativePrompts: { type: 'array', items: { type: 'string' }, description: 'Required when triggers or promptSignals change.' },
            promptSignalPositivePrompts: { type: 'array', items: { type: 'string' }, description: 'Optional evaluation prompts for structured prompt-signal changes.' },
            promptSignalNegativePrompts: { type: 'array', items: { type: 'string' }, description: 'Optional negative evaluation prompts for structured prompt-signal changes.' },
