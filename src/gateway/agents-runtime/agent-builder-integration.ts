...
// URL resolved from: AGENT_BUILDER_URL env var → config.json agent_builder.url → default
function resolveAgentBuilderUrl(): string {
  if (process.env.AGENT_BUILDER_URL) return process.env.AGENT_BUILDER_URL;
  try {
    const { getConfig } = require('../../config/config');
    const url = (getConfig().getConfig() as any)?.agent_builder?.url;
    if (typeof url === 'string' && url.startsWith('http')) return url;
  } catch { /* ignore */ }
  return 'http://localhost:3005';
}
const AGENT_BUILDER_URL = resolveAgentBuilderUrl();
...
async function create_node_subagent(args: {
  ...
  const topicsMd = [
    `# Topics Rotation`,
    ``,
    `Rotate through these topics. Track which you've covered in history.md.`,
    ``,
    answers.topics
      .split(/[,\n]+/)
      .map((t: string) => t.trim())
  ...
  const systemInstructions = [
    ...
    `4. Read your latest notes in history.md for running context`,
    ``,
    `After writing, append a short note to history.md with what topic you covered.`,
  ...
  const result = await abCallWithRetry('POST', '/api/v1/subagents/create', {
    ...
    constraints: [
      ...
      'Update history.md after every task',
