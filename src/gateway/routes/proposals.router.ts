...
export async function dispatchApprovedProposal(
  ...
  const fullPrompt = usesOperationalExecutionMode
    ...
    needsBuild ? `\nSANDBOX VERIFICATION REQUIRED: Yes — run ${canonicalBuildCommand} after edits inside this isolated proposal workspace. MANDATORY. Stop if build fails — task will pause for assistance.` : '',
    ``,
    `GLOBAL RULES:`,
    `- Execute only the approved proposal scope and approved steps.`,
    ...
    `- Run the canonical sandbox verification command before completion.`,
    `- Do not call prom_apply_dev_changes inside this isolated proposal workspace; any live sync/build/restart must happen from the live repo after verified promotion, not from the sandbox.`,
    ...
    `4. ${needsBuild ? `Run sandbox verification: run_command({command:"${canonicalBuildCommand}", shell:true}). If it fails, STOP — do not retry.` : 'Verify your changes are correct.'}`,
