...
function formatFriendlyToolAction(toolNameRaw: string, args: any = {}): string {
  ...
  const exact: Record<string, string> = {
    ...
    chat_with_subagent: withParens('Chatting with Subagent', pickArg(args, ['message', 'content', 'task'], 160)),
    agent_message_send: withParens('Sending Agent Message', pickArg(args, ['message', 'content', 'task'], 160)),
    agent_turn_request: withParens('Requesting Agent Turn', pickArg(args, ['prompt', 'message', 'content'], 160)),
    agent_reply_wait: withSubject('Waiting for Agent Reply', pickArg(args, ['agent_id', 'team_id', 'thread_id'])),
    agent_thread_watch: withSubject('Watching Agent Thread', pickArg(args, ['agent_id', 'team_id', 'thread_id'])),
