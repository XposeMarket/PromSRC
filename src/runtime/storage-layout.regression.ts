...
import { resolvePrometheusLayout, standaloneSubagentWorkspace, teamSharedWorkspace, teamSubagentWorkspace } from './storage-layout.js';
...
function run(): void {
  ...
  const explicitRuntime = resolvePrometheusLayout({
    env: env({
      PROMETHEUS_DATA_DIR: data,
      PROMETHEUS_RUNTIME_DIR: path.join(data, 'isolated-runtime'),
    }),
    homedir: home,
    cwd: repo,
    platform: 'linux',
  ...
  const legacyElectronShape = resolvePrometheusLayout({
    env: env({
      PROMETHEUS_DATA_DIR: data,
      PROMETHEUS_WORKSPACE_DIR: path.join(data, 'workspace'),
    }),
    homedir: home,
    cwd: repo,
    platform: 'linux',
