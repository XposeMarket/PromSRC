...
function main(): void {
  ...
  if (!manifest.readyToActivate) {
    console.error('\nMigration copied what it could but is NOT safe to activate. Resolve conflicts/errors/skipped symlinks from the manifest first.');
    process.exitCode = 2;
    return;
  }
  console.log('\nVerified canonical copy is ready for the activation phase. Legacy source data was left intact.');
}
