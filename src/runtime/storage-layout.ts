...
function sanitizeStorageId(value: string, fallback: string): string {
  const sanitized = String(value || '').trim().replace(/[^a-zA-Z0-9_.-]/g, '_');
  // `path.join(root, '.')` aliases the root and `path.join(root, '..')` escapes it.
  // Treat dot-only path segments as invalid IDs while preserving dots elsewhere.
...
export function standaloneSubagentWorkspace(layout: PrometheusLayout, agentId: string): string {
  return path.join(layout.workspace.standaloneSubagents, sanitizeStorageId(agentId, 'agent'));
}

export function teamRoot(layout: PrometheusLayout, teamId: string): string {
  return path.join(layout.workspace.teams, sanitizeStorageId(teamId, 'team'));
}
...
export function teamSubagentWorkspace(layout: PrometheusLayout, teamId: string, agentId: string): string {
  return path.join(teamRoot(layout, teamId), 'subagents', sanitizeStorageId(agentId, 'agent'));
}
